...
class _CompositeConfigHelper:
    ...
    def _get_compositor_loader_from_config(composite_name, composite_info):
        try:
            ...
            raise ValueError(
                "'compositor' key missing or empty for '{}'. Option keys = {}".format(
        ...
        for prereq_type in ["prerequisites", "optional_prerequisites"]:
            ...
            for dep_info in composite_info.get(prereq_type, []):
                ...
                if not isinstance(dep_info, dict):
                    ...
                    self._handle_inline_comp_dep(dep_info, dep_num, composite_info["name"])
    ...
    def parse_config(self, configured_composites, composite_configs):
        ...
        try:
            ...
            raise RuntimeError("Failed to load composites from configs '{}'".format(composite_configs))
...
class _ModifierConfigHelper:
    ...
    def _get_modifier_loader_from_config(modifier_name, modifier_info):
        try:
            ...
            raise ValueError(
                "'modifier' key missing or empty for '{}'. Option keys = {}".format(
    ...
    def parse_config(self, configured_modifiers, composite_configs):
        ...
        try:
            ...
            raise RuntimeError("Failed to load modifiers from configs '{}'".format(composite_configs))
...
def load_compositor_configs_for_sensor(sensor_name: str) -> tuple[dict[str, dict], dict[str, dict], dict]:
    ...
    composite_configs = config_search_paths(
        os.path.join("composites", config_filename), search_dirs=paths, check_exists=True
    )
    if not composite_configs:
        logger.debug("No composite config found called %s", config_filename)
...
def all_composite_sensors():
    ...
    paths = get_entry_points_config_dirs("satpy.composites")
    composite_configs = glob_config(os.path.join("composites", "*.yaml"), search_dirs=paths)
    yaml_names = set([os.path.splitext(os.path.basename(fn))[0] for fn in composite_configs])
