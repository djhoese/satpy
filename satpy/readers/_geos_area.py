...
def make_ext(ll_x, ur_x, ll_y, ur_y, h):
    ...
    """
    aex = (np.deg2rad(ll_x) * h, np.deg2rad(ll_y) * h, np.deg2rad(ur_x) * h, np.deg2rad(ur_y) * h)
...
def get_area_extent(pdict):
    ...
    ll_x, ll_y = get_xy_from_linecol(
        lines * scanmult, cols, (pdict["loff"], pdict["coff"]), (pdict["lfac"], pdict["cfac"])
    )

    cols += pdict["ncols"]
    lines += pdict["nlines"]
    # Upper right x, y scanning angles in degrees
    ur_x, ur_y = get_xy_from_linecol(
        lines * scanmult, cols, (pdict["loff"], pdict["coff"]), (pdict["lfac"], pdict["cfac"])
    )
    if pdict["scandir"] == "S2N":
        ll_y *= -1
...
def get_area_definition(pdict, a_ext):
    ...
    a_def = geometry.AreaDefinition(
        pdict["a_name"], pdict["a_desc"], pdict["p_id"], proj_dict, int(pdict["ncols"]), int(pdict["nlines"]), a_ext
    )
...
def get_geos_area_naming(input_dict):
    ...
    area_naming_dict["area_id"] = "{}_{}_{}_{}{}".format(
        input_dict["platform_name"].lower(),
    ...
    area_naming_dict["description"] = "{} {} {} area definition with {} {} resolution".format(
        input_dict["platform_name"].upper(),
...
def get_resolution_and_unit_strings(resolution):
    ...
    """
    if resolution >= 1000:
        return {"value": "{:.0f}".format(resolution * 1e-3), "unit": "km"}

    return {"value": "{:.0f}".format(resolution), "unit": "m"}
