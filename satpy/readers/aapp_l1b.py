...
class AAPPL1BaseFileHandler(BaseFileHandler):
    ...
    def start_time(self):
        ...
        return datetime(self._data["scnlinyr"][0], 1, 1) + timedelta(
            days=int(self._data["scnlindy"][0]) - 1, milliseconds=int(self._data["scnlintime"][0])
        )
    ...
    def end_time(self):
        ...
        return datetime(self._data["scnlinyr"][-1], 1, 1) + timedelta(
            days=int(self._data["scnlindy"][-1]) - 1, milliseconds=int(self._data["scnlintime"][-1])
        )
...
class AVHRRAAPPL1BFile(AAPPL1BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self._get_all_interpolated_angles = functools.lru_cache(maxsize=10)(self._get_all_interpolated_angles_uncached)
    ...
    def _convert_binary_channel_status_to_activation_dict(status):
        bits_channels = ((13, "1"), (12, "2"), (11, "3a"), (10, "3b"), (9, "4"), (8, "5"))
        activated = dict()
    ...
    def _interpolate_arrays(self, *input_arrays, geolocation=False):
        ...
        try:
            ...
            output_arrays = [
                da.from_delayed(out_array, (lines, 2048), in_array.dtype)
    ...
    def _create_40km_interpolator(lines, *arrays_40km, geolocation=False):
        ...
        cross_track_order = 3
        satint = Interpolator(
            arrays_40km, (rows40km, cols40km), (rows1km, cols1km), along_track_order, cross_track_order
        )
        return satint
    ...
    def calibrate(self, dataset_id, pre_launch_coeffs=False, calib_coeffs=None):
        ...
        if dataset_id["name"] in ("3a", "3b") and self._is3b is None:
            ...
            self._is3a = da.bitwise_and(da.from_array(self._data["scnlinbit"], chunks=line_chunks), 3) == 0
            self._is3b = da.bitwise_and(da.from_array(self._data["scnlinbit"], chunks=line_chunks), 3) == 1
        ...
        if vis_idx is not None:
            ...
            ds = create_xarray(
                _vis_calibrate(self._data, vis_idx, dataset_id["calibration"], pre_launch_coeffs, coeffs, mask=mask)
            )
        else:
            if dataset_id["name"] == "3b":
                mask = self._is3b[:, None]
            ds = create_xarray(_ir_calibrate(self._header, self._data, ir_idx, dataset_id["calibration"], mask=mask))
...
def _vis_calibrate(data, chn, calib_type, pre_launch_coeffs=False, calib_coeffs=None, mask=True):
    ...
    *calib_type* in count, reflectance, radiance.
    ...
    if calib_type == "radiance":
        logger.info("Radiances are not yet supported for " + "the VIS/NIR channels!")
    ...
    if pre_launch_coeffs:
        ...
        if np.all(data["calvis"][:, chn, 0, 4] == 0):
            logger.info("No valid operational coefficients, fall back to pre-launch")
            coeff_idx = 2
        else:
            coeff_idx = 0

    intersection = da.from_array(data["calvis"][:, chn, coeff_idx, 4], chunks=line_chunks)
    ...
    if calib_coeffs is not None:
        ...
        slope1 = da.from_array(data["calvis"][:, chn, coeff_idx, 0], chunks=line_chunks) * 1e-10
        intercept1 = da.from_array(data["calvis"][:, chn, coeff_idx, 1], chunks=line_chunks) * 1e-7
        slope2 = da.from_array(data["calvis"][:, chn, coeff_idx, 2], chunks=line_chunks) * 1e-10
        intercept2 = da.from_array(data["calvis"][:, chn, coeff_idx, 3], chunks=line_chunks) * 1e-7
