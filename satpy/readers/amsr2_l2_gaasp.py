...
class GAASPFileHandler(BaseFileHandler):
    ...
    def nc(self):
        ...
        chunks = {dim_name: CHUNK_SIZE for dim_name in self.y_dims + self.x_dims + self.time_dims}
        nc = xr.open_dataset(self.filename, decode_cf=True, mask_and_scale=False, chunks=chunks)
...
class GAASPGriddedFileHandler(GAASPFileHandler):
    ...
    def get_area_def(self, dataid):
        ...
        area_def = AreaDefinition(area_name, area_name, area_name, crs, data_shape[1], data_shape[0], extent)
        return area_def
