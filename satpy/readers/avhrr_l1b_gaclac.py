...
class GACLACFile(BaseFileHandler):
    ...
    def __init__(
        ...
        **reader_kwargs,
        ...
        """
        super(GACLACFile, self).__init__(filename, filename_info, filetype_info)
        ...
        self._end_time = datetime.combine(filename_info["start_time"].date(), filename_info["end_time"].time())
        if self._end_time < self._start_time:
            self._end_time += timedelta(days=1)
        self.platform_id = filename_info["platform_id"]
        if self.platform_id in ["NK", "NL", "NM", "NN", "NP", "M1", "M2", "M3"]:
    ...
    def get_dataset(self, key, info):
        ...
        res = xr.DataArray(da.from_array(data, chunks=(chunk_lines, chunk_cols)), dims=["y", xdim], attrs=info)
        if xcoords:
            res[xdim] = xcoords
