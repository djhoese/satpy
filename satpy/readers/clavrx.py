...
class _CLAVRxHelper:
    ...
    def _remove_attributes(attrs: dict) -> dict:
        ...
        old_attrs = ["unscaled_missing", "SCALED_MIN", "SCALED_MAX", "SCALED_MISSING"]
    ...
    def _read_axi_fixed_grid(filename: str, l1b_attr) -> geometry.AreaDefinition:
        ...
        """
        LOG.debug(f"looking for corresponding input file for {l1b_attr} to act as fixed grid navigation donor")
        ...
        area = geometry.AreaDefinition(
            "ahi_geos", "AHI L2 file area", "ahi_geos", proj, ncols, nlines, np.asarray(area_extent)
        )
...
class CLAVRXHDF4FileHandler(HDF4FileHandler, _CLAVRxHelper):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        """Init method."""
        super(CLAVRXHDF4FileHandler, self).__init__(filename, filename_info, filetype_info)
    ...
    def get_dataset(self, dataset_id, ds_info):
        ...
        data.attrs = _CLAVRxHelper.get_metadata(self.sensor, self.platform, data.attrs, ds_info)
        return data
    ...
    def _is_polar(self):
        l1b_att, inst_att = (
            str(self.file_content.get("/attr/L1B", None)),
...
class CLAVRXNetCDFFileHandler(_CLAVRxHelper, BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        """Init method."""
        super(CLAVRXNetCDFFileHandler, self).__init__(
            filename,
            filename_info,
            filetype_info,
        )

        self.nc = xr.open_dataset(filename, decode_cf=True, mask_and_scale=False, decode_coords=True, chunks=CHUNK_SIZE)
        # y,x is used in satpy, bands rather than channel using in xrimage
        self.nc = self.nc.rename_dims(
            {"scan_lines_along_track_direction": "y", "pixel_elements_along_scan_direction": "x"}
        )

        self.platform = _get_platform(self.filename_info.get("platform_shortname", None))
        ...
        self.nc.coords["latitude"] = _CLAVRxHelper._get_data(self.nc.coords["latitude"], {"name": "latitude"})
        self.nc.coords["longitude"] = _CLAVRxHelper._get_data(self.nc.coords["longitude"], {"name": "longitude"})
    ...
    def _is_polar(self):
        l1b_att, inst_att = (str(self.nc.attrs.get("L1B", None)), str(self.nc.attrs.get("sensor", None)))
