...
def read_records(filename):
    ...
    with open(filename, "rb") as fdes:
        ...
        for dtype, count, rec_class in zip(dtypes, counts, classes):
            fdes.seek(offset)
            if rec_class == ("mdr", 2):
                record = da.from_array(
                    np.memmap(fdes, mode="r", dtype=dtype, shape=count, offset=offset), chunks=(max_lines,)
                )
            else:
...
class EPSAVHRRFile(BaseFileHandler):
    ...
    units = {"reflectance": "%", "brightness_temperature": "K"}
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.pixels = None
        self.sections = None
        self.get_full_angles = functools.lru_cache(maxsize=1)(self._get_full_angles_uncached)
        self.get_full_lonlats = functools.lru_cache(maxsize=1)(self._get_full_lonlats_uncached)
    ...
    def _interpolate(self, lons_like, lats_like):
        ...
        if nav_sample_rate == 20 and self.pixels == 2048:
            ...
            lons_like_1km = da.from_delayed(lons_like_1km, dtype=lons_like.dtype, shape=(self.scanlines, self.pixels))
            lats_like_1km = da.from_delayed(lats_like_1km, dtype=lats_like.dtype, shape=(self.scanlines, self.pixels))
            return lons_like_1km, lats_like_1km

        raise NotImplementedError(
            "Lon/lat and angle expansion not implemented for "
            + "sample rate = "
            + str(nav_sample_rate)
            + " and earth views = "
            + str(self.pixels)
    ...
    def _get_full_angles_uncached(self):
        ...
        return self._get_full_angles(solar_zenith, sat_zenith, solar_azimuth, sat_azimuth)
    ...
    def _get_calibrated_dataarray(self, key):
        ...
        if key["calibration"] not in ["reflectance", "brightness_temperature", "radiance"]:
            raise ValueError("calibration type " + str(key["calibration"]) + " is not supported!")
        ...
        if channel_name in ["1", "2", "3A"]:
            if key["calibration"] == "reflectance":
                array = radiance_to_refl(array, self[f"CH{channel_name}_SOLAR_FILTERED_IRRADIANCE"])
            if channel_name == "3A":
    ...
    def get_lonlats(self):
        ...
        if self.area is None:
            ...
            self.area.name = "_".join([self.platform_name, str(self.start_time), str(self.end_time)])
        return self.area
