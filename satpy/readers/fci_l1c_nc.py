...
class FCIL1cNCFileHandler(NetCDF4FsspecFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        super().__init__(filename, filename_info, filetype_info, cache_var_size=0, cache_handle=True)
    ...
    def get_segment_position_info(self):
        ...
        segment_position_info = {
            HIGH_RES_GRID_INFO[file_type]["grid_type"]: {
                ...
                "segment_height": (
                    self.get_and_cache_npxr(vis_06_measured_path + "/end_position_row").item()
            ...
            LOW_RES_GRID_INFO[file_type]["grid_type"]: {
                ...
                "segment_height": (
                    self.get_and_cache_npxr(ir_105_measured_path + "/end_position_row").item()
    ...
    def _get_dataset_measurand(self, key, info=None):
        ...
        fv = attrs.pop("FillValue", default_fillvals.get(data.dtype.str[1:], np.nan))
        ...
        res.attrs["platform_name"] = self._platform_name_translate.get(self["attr/platform"], self["attr/platform"])
    ...
    def calibrate_rad_to_bt(self, radiance, key):
        ...
        for v in (vc, a, b, c1, c2):
            if v == v.attrs.get("FillValue", default_fillvals.get(v.dtype.str[1:])):
                logger.error(
    ...
    def calibrate_rad_to_refl(self, radiance, key):
        ...
        if cesi == cesi.attrs.get("FillValue", default_fillvals.get(cesi.dtype.str[1:])):
            logger.error(
        ...
        sun_earth_distance = (
            np.mean(self.get_and_cache_npxr("state/celestial/earth_sun_distance")) / 149597870.7
        )  # [AU]
