...
class FciL2CommonFunctions(object):
    ...
    def ssp_lon(self):
        ...
        try:
            ...
            logger.warning(
                "ssp_lon could not be obtained from file content, using default value "
    ...
    def _slice_dataset(self, variable, dataset_info, dimensions):
        ...
        slice_dict = {
            dim: dataset_info[dim_id]
            for (dim, dim_id) in dimensions.items()
...
class FciL2NCFileHandler(FciL2CommonFunctions, BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, with_area_definition=True):
        ...
        self.nc = xr.open_dataset(
            ...
            decode_cf=True,
            mask_and_scale=True,
            chunks={"number_of_columns": CHUNK_SIZE, "number_of_rows": CHUNK_SIZE},
        )
    ...
    def _compute_area_def(self, dataset_id):
        ...
        area_def = geometry.AreaDefinition(
            area_naming["area_id"], area_naming["description"], "", proj_dict, self.ncols, self.nlines, area_extent
        )
    ...
    def _get_proj_area(self, dataset_id):
        ...
        area_naming = get_geos_area_naming({**area_naming_input_dict, **get_service_mode("fci", self.ssp_lon)})

        proj_dict = {"a": a, "lon_0": self.ssp_lon, "h": h, "rf": rf, "proj": "geos", "units": "m", "sweep": "y"}
    ...
    def get_total_cot(variable):
        ...
        variable = variable.fillna(0.0)
        variable = variable.sum(dim="maximum_number_of_layers", keep_attrs=True)
        variable = variable.where(variable != 0.0, np.nan)
...
class FciL2NCSegmentFileHandler(FciL2CommonFunctions, BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, with_area_definition=False):
        ...
        self.nc = xr.open_dataset(
            ...
            decode_cf=True,
            mask_and_scale=True,
            chunks={"number_of_FoR_cols": CHUNK_SIZE, "number_of_FoR_rows": CHUNK_SIZE},
        )
        ...
        self.multi_dims = {
            "number_of_categories": "category_id",
            "number_of_channels": "channel_id",
            "number_of_vis_channels": "vis_channel_id",
            "number_of_ir_channels": "ir_channel_id",
            "number_test": "test_id",
        }
    ...
    def get_dataset(self, dataset_id, dataset_info):
        ...
        if self.with_adef and var_key not in [
            "longitude",
            "latitude",
            "product_quality",
