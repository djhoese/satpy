...
class GEOCATFileHandler(NetCDF4FileHandler):
    ...
    **Loading data with decode_times=True**
    ...
    def __init__(self, filename, filename_info, filetype_info, **kwargs):
        ...
        kwargs.setdefault("xarray_kwargs", {}).setdefault("engine", "netcdf4")
        kwargs.setdefault("xarray_kwargs", {}).setdefault("decode_times", False)

        super(GEOCATFileHandler, self).__init__(
            filename, filename_info, filetype_info, xarray_kwargs=kwargs["xarray_kwargs"]
        )
    ...
    def _get_proj(self, platform, ref_lon):
        if platform == "GOES-16" and -76.0 < ref_lon < -74.0:
    ...
    def _calc_area_resolution(self, ds_res):
        elem_res = round(ds_res / 1000.0)  # mimic 'Element_Resolution' attribute from above
        sensor = self.get_sensor(self["/attr/Sensor_Name"])
        return self.resolutions.get(sensor, {}).get(int(elem_res), elem_res * 1000.0)
    ...
    def _get_extents(self, proj, res, lon_arr, lat_arr):
        ...
        half_x = res / 2.0
        half_y = res / 2.0
        return (left_x - half_x, bot_y - half_y, right_x + half_x, top_y + half_y)
    ...
    def get_area_def(self, dsid):
        ...
        area_name = "{} {} Area at {}m".format(platform, self.metadata.get("sector_id", ""), int(res))
