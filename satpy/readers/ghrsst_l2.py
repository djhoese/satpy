...
class GHRSSTL2FileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, engine=None):
        ...
        self.filename_info["start_time"] = datetime.strptime(self.nc.start_time, "%Y%m%dT%H%M%SZ")
        self.filename_info["end_time"] = datetime.strptime(self.nc.stop_time, "%Y%m%dT%H%M%SZ")
    ...
    def nc(self):
        ...
        nc = xr.open_dataset(
            ...
            mask_and_scale=True,
            engine=self._engine,
            chunks={"ni": CHUNK_SIZE, "nj": CHUNK_SIZE},
        )
    ...
    def _open_tarfile(self):
        ...
        sst_filename = next((name for name in self._tarfile.getnames() if self._is_sst_file(name)))
