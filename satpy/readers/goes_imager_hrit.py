...
goms_variable_length_headers = {image_segment_line_quality: "image_segment_line_quality"}
...
goes_hdr_map.update({7: key_header, 128: segment_identification, 129: image_segment_line_quality})
...
def make_sgs_time(sgs_time_array):
    """Make sgs time."""
    year = (
        (sgs_time_array["century"] >> 4) * 1000
        + (sgs_time_array["century"] & 15) * 100
        + (sgs_time_array["year"] >> 4) * 10
        + (sgs_time_array["year"] & 15)
    )
    doy = (sgs_time_array["doy1"] >> 4) * 100 + (sgs_time_array["doy1"] & 15) * 10 + (sgs_time_array["doy_hours"] >> 4)
    hours = (sgs_time_array["doy_hours"] & 15) * 10 + (sgs_time_array["hours_mins"] >> 4)
    mins = (sgs_time_array["hours_mins"] & 15) * 10 + (sgs_time_array["mins_secs"] >> 4)
    secs = (sgs_time_array["mins_secs"] & 15) * 10 + (sgs_time_array["secs_msecs"] >> 4)
    msecs = (
        (sgs_time_array["secs_msecs"] & 15) * 100 + (sgs_time_array["msecs"] >> 4) * 10 + (sgs_time_array["msecs"] & 15)
    )
    return datetime(int(year), 1, 1) + timedelta(
        days=int(doy - 1), hours=int(hours), minutes=int(mins), seconds=int(secs), milliseconds=int(msecs)
    )


satellite_status = np.dtype(
    [
        ("TagType", "<u4"),
        ("TagLength", "<u4"),
        ("SatelliteID", "<u8"),
...
image_acquisition = np.dtype(
    [("TagType", "<u4"), ("TagLength", "<u4"), ("Status", "<u4"), ("StartDelay", "<i4"), ("Cel", "<f8")]
)
...
class HRITGOESPrologueFileHandler(HRITFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        super(HRITGOESPrologueFileHandler, self).__init__(
            filename, filename_info, filetype_info, (goes_hdr_map, goms_variable_length_headers, goms_text_headers)
        )
        self.prologue = {}
        self.read_prologue()
...
radiometric_processing = np.dtype(
    [
        ...
        (
            "RPSummary",
            [("Impulse", "<u4"), ("IsStrNoiseCorrection", "<u4"), ("IsOptic", "<u4"), ("IsBrightnessAligment", "<u4")],
        ),
        ("OpticCorrection", [("Degree", "<i4"), ("A", "<f8", (16,))]),
        (
            "RPQuality",
            [
                ("EffDinRange", "<f8"),
...
class HRITGOESFileHandler(HRITFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, prologue):
        """Initialize the reader."""
        super(HRITGOESFileHandler, self).__init__(
            filename, filename_info, filetype_info, (goes_hdr_map, goms_variable_length_headers, goms_text_headers)
        )
