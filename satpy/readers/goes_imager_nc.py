...
CALIB_COEFS = {
    "GOES-15": {
        "00_7": {
            ...
            "offset": [-16.9707, -17.0513, -16.9847, -16.9773, -17.0143, -16.9251, -16.9550, -16.9469],
            "x0": 29,
            "k": 1.88852e-3,
        },
        "03_9": {
    ...
    "GOES-14": {
        "00_7": {
            ...
            "offset": [-17.037, -17.010, -17.002, -17.006, -16.986, -16.971, -16.996, -16.941],
            "x0": 29,
            "k": 1.88772e-3,
        },
        "03_9": {
    ...
    "GOES-13": {
        "00_7": {
            ...
            "offset": [-17.749, -17.744, -17.769, -17.653, -17.785, -17.743, -17.755, -17.594],
            "x0": 29,
            "k": 1.89544e-3,
        },
        "03_9": {
        ...
        "13_3": {
            ...
            "btmin": 180.0,
            "btmax": 340.0,
        },  # Has only one detector on GOES-13
    },
    "GOES-12": {
        "00_7": {
            "slope": [
            ...
            "offset": [-16.736, -16.709, -16.750, -16.793, -16.782, -16.692, -16.687, -16.681],
            "x0": 29,
            "k": 1.97658e-3,
        },
        "03_9": {
        ...
        "13_3": {
            ...
            "btmin": 180.0,
            "btmax": 340.0,
        },  # Has only one detector on GOES-12
    },
    "GOES-11": {
        "00_7": {
            "slope": [
            ...
            "offset": [-16.129, -16.104, -16.121, -16.175, -16.116, -16.205, -16.201, -16.034],
            "x0": 29,
            "k": 2.01524e-3,
        },
        "03_9": {
    ...
    "GOES-10": {
        "00_7": {
            ...
            "offset": [-16.256, -16.134, -16.143, -16.188, -16.192, -16.158, -16.133, -16.279],
            "x0": 29,
            "k": 1.98808e-3,
        },
        "03_9": {
    ...
    "GOES-9": {
        "00_7": {"slope": [0.5492361], "offset": [-15.928], "x0": 29, "k": 1.94180e-3},
        "03_9": {
            "scale": SCALE_03_9,
            "offset": OFFSET_03_9,
            "n": [2555.18, 2555.18],
    ...
    "GOES-8": {
        "00_7": {"slope": [0.5501873], "offset": [-15.955], "x0": 29, "k": 1.92979e-3},
        "03_9": {
            "scale": SCALE_03_9,
            "offset": OFFSET_03_9,
            "n": [2556.71, 2558.62],
...
class GOESNCBaseFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, geo_data=None):
        ...
        super(GOESNCBaseFileHandler, self).__init__(filename, filename_info, filetype_info)
        self.nc = xr.open_dataset(
            self.filename, decode_cf=True, mask_and_scale=False, chunks={"xc": CHUNK_SIZE, "yc": CHUNK_SIZE}
        )
        self.sensor = "goes_imager"
        self.nlines = self.nc.dims["yc"]
        self.ncols = self.nc.dims["xc"]
        self.platform_name = self._get_platform_name(self.nc.attrs["Satellite Sensor"])
        self.platform_shortname = self.platform_name.replace("-", "").lower()
        self.gvar_channel = int(self.nc["bands"].values)
        self.sector = self._get_sector(channel=self.gvar_channel, nlines=self.nlines, ncols=self.ncols)
        self._meta = None
    ...
    def resolution(self):
        ...
        """
        return 1000.0 * self.nc["lineRes"].values
    ...
    def meta(self):
        ...
        if self._meta is None:
            ...
            crow, ccol = self._get_nadir_pixel(earth_mask=earth_mask, sector=self.sector)
            ...
            area_def_uni = self._get_area_def_uniform_sampling(lon0=lon0, channel=self.gvar_channel)
            del lon
    ...
    def _counts2radiance(self, counts, coefs, channel):
        ...
        if is_vis_channel(channel):
            ...
            return self._viscounts2radiance(counts=counts, slope=slope, offset=offset)

        return self._ircounts2radiance(counts=counts, scale=coefs["scale"], offset=coefs["offset"])
    ...
    def _calibrate(self, radiance, coefs, channel, calibration):
        ...
        if is_vis_channel(channel):
            if not calibration == "reflectance":
                raise ValueError("Cannot calibrate VIS channel to {}".format(calibration))
            return self._calibrate_vis(radiance=radiance, k=coefs["k"])
        else:
            if not calibration == "brightness_temperature":
                raise ValueError("Cannot calibrate IR channel to {}".format(calibration))
    ...
    def _calibrate_ir(radiance, coefs):
        ...
        # Apply BT threshold
        return bt.where(np.logical_and(bt >= coefs["btmin"], bt <= coefs["btmax"]))
...
class GOESNCFileHandler(GOESNCBaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        """Initialize the reader."""
        super(GOESNCFileHandler, self).__init__(filename, filename_info, filetype_info)
    ...
    def get_dataset(self, key, info):
        ...
        if "longitude" in key["name"]:
            ...
            tic = datetime.now()
            data = self.calibrate(self.nc["data"].isel(time=0), calibration=key["calibration"], channel=key["name"])
    ...
    def calibrate(self, counts, calibration, channel):
        ...
        if calibration in ["radiance", "reflectance", "brightness_temperature"]:
            radiance = self._counts2radiance(counts=counts, coefs=coefs, channel=channel)
            if calibration == "radiance":
                return radiance

            return self._calibrate(radiance=radiance, coefs=coefs, channel=channel, calibration=calibration)
...
class GOESEUMNCFileHandler(GOESNCBaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, geo_data):
        """Initialize the reader."""
        super(GOESEUMNCFileHandler, self).__init__(filename, filename_info, filetype_info, geo_data)
    ...
    def calibrate(self, data, calibration, channel):
        ...
        if not is_vis and calibration == "brightness_temperature":
            return self._calibrate(radiance=data, calibration=calibration, coefs=coefs, channel=channel)
...
class GOESEUMGEONCFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        """Initialize the reader."""
        super(GOESEUMGEONCFileHandler, self).__init__(filename, filename_info, filetype_info)
        self.nc = xr.open_dataset(
            self.filename, decode_cf=True, mask_and_scale=False, chunks={"xc": CHUNK_SIZE, "yc": CHUNK_SIZE}
        )
        self.sensor = "goes_imager"
        self.nlines = self.nc.dims["yc"]
        self.ncols = self.nc.dims["xc"]
        self.platform_name = GOESNCBaseFileHandler._get_platform_name(self.nc.attrs["Satellite Sensor"])
        self.platform_shortname = self.platform_name.replace("-", "").lower()
        self._meta = None
...
class GOESCoefficientReader(object):
    ...
    def __init__(self, ir_url, vis_url):
        ...
        self.ir_html = BeautifulSoup(self._load_url_or_file(ir_url), features="html5lib")
        self.vis_html = BeautifulSoup(self._load_url_or_file(vis_url), features="html5lib")
    ...
    def _get_ir_coefs(self, platform, channel):
        ...
        table11 = self._get_table(root=self.ir_html, heading="Table 1-1", heading_type="h3")
        for row in table11:
        ...
        # given platform
        table = self._get_table(root=self.ir_html, heading=self.ir_tables[platform], heading_type="h3")
    ...
    def _get_vis_coefs(self, platform):
        ...
        table = self._get_table(root=self.vis_html, heading=self.vis_tables[platform], heading_type="p")
    ...
    def _get_table(
        ...
        headings = [h for h in root.find_all(heading_type) if heading in h.text]
        if not headings:
            raise ValueError('Cannot find a coefficient table matching text "{}"'.format(heading))
        if len(headings) > 1:
            raise ValueError('Found multiple headings matching text "{}"'.format(heading))
        ...
        for channel, coefs in CALIB_COEFS[platform].items():
            ...
            for cname in coefs_expected.keys():
                if not np.allclose(coefs[cname], coefs_expected[cname]):
                    raise ValueError(
                        "Coefficient {} for {} channel {} does not match the reference".format(cname, platform, channel)
                    )
...
class AreaDefEstimator:
    ...
    def _get_area_description(self):
        return "{} geostationary projection (uniform sampling)".format(self.platform_name)
