...
class GRIBFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        try:
            with pygrib.open(self.filename) as grib_file:
                ...
                start_time = self._convert_datetime(first_msg, "validityDate", "validityTime")
                end_time = self._convert_datetime(last_msg, "validityDate", "validityTime")
    ...
    def _analyze_messages(self, grib_file):
        ...
        for idx, msg in enumerate(grib_file):
            msg_id = DataQuery(name=msg["shortName"], level=msg["level"], modifiers=tuple())
            ds_info = {
                "message": idx + 1,
                "name": msg["shortName"],
                "level": msg["level"],
    ...
    def _get_extents(min_x, min_y, max_x, max_y, shape):
        half_x = abs((max_x - min_x) / (shape[1] - 1)) / 2.0
        half_y = abs((max_y - min_y) / (shape[0] - 1)) / 2.0
    ...
    def _get_area_info(self, msg, proj_params):
        ...
        shape = lats.shape
        scans_positively = msg.valid_key("jScansPositively") and msg["jScansPositively"] == 1
        proj_params, lons, lats = self._get_corner_lonlat(proj_params, lons, lats)
    ...
    def get_metadata(self, msg, ds_info):
        """Get metadata."""
        model_time = self._convert_datetime(msg, "dataDate", "dataTime")
        start_time = self._convert_datetime(msg, "validityDate", "validityTime")
        end_time = start_time
        try:
