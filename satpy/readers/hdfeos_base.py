...
def _interpolate_with_angles(clons, clats, csatz, src_resolution, dst_resolution):
    ...
    return _find_and_run_interpolation(interpolation_functions, src_resolution, dst_resolution, (clons, clats, csatz))
...
def _interpolate_no_angles(clons, clats, src_resolution, dst_resolution):
    ...
    return _find_and_run_interpolation(interpolation_functions, src_resolution, dst_resolution, (clons, clats))
...
def _find_and_run_interpolation(interpolation_functions, src_resolution, dst_resolution, args):
    try:
        ...
        error_message = "Interpolation from {}m to {}m not implemented".format(src_resolution, dst_resolution)
...
class HDFEOSBaseFileReader(BaseFileHandler):
    ...
    def start_time(self):
        ...
        try:
            date = (
                self.metadata["INVENTORYMETADATA"]["RANGEDATETIME"]["RANGEBEGINNINGDATE"]["VALUE"]
                + " "
    ...
    def end_time(self):
        ...
        try:
            date = (
                self.metadata["INVENTORYMETADATA"]["RANGEDATETIME"]["RANGEENDINGDATE"]["VALUE"]
                + " "
    ...
    def get_dataset(self, dataset_id: DataID, dataset_info: dict) -> xr.DataArray:
        ...
        if resolution != self.geo_resolution:
            if in_file_dataset_name is not None:
                ...
                raise NotImplementedError("Interpolation for variable '{}' is not configured".format(dataset_name))

            # The data must be interpolated
            logger.debug("Loading %s", dataset_name)
            if dataset_name in ["longitude", "latitude"]:
                self.get_interpolated_dataset("longitude", "latitude", resolution)
                ...
                self.get_interpolated_dataset(
                    "satellite_azimuth_angle", "satellite_zenith_angle", resolution, offset=90
                )
            elif dataset_name in ["solar_azimuth_angle", "solar_zenith_angle"]:
                # Sensor dataset names differs between L1b and L2 products
                self.get_interpolated_dataset("solar_azimuth_angle", "solar_zenith_angle", resolution, offset=90)
