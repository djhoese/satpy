...
common_hdr = np.dtype([("hdr_id", "u1"), ("record_length", ">u2")])

primary_header = np.dtype([("file_type", "u1"), ("total_header_length", ">u4"), ("data_field_length", ">u8")])

image_structure = np.dtype(
    [
        ("number_of_bits_per_pixel", "u1"),
...
image_navigation = np.dtype(
    [("projection_name", "S32"), ("cfac", ">i4"), ("lfac", ">i4"), ("coff", ">i4"), ("loff", ">i4")]
)
...
timestamp_record = np.dtype([("cds_p_field", "u1"), ("timestamp", time_cds_short)])
...
class HRITFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, hdr_info):
        ...
        super(HRITFileHandler, self).__init__(filename, filename_info, filetype_info)
        ...
        with utils.generic_open(self.filename, mode="rb") as fp:
            ...
            while fp.tell() < total_header_length:
                ...
                if the_type in variable_length_headers:
                    field_length = int((hdr_id["record_length"] - 3) / the_type.itemsize)
                    ...
                    self.mda.update(dict(zip(current_hdr.dtype.names, current_hdr)))
    ...
    def get_area_extent(self, size, offsets, factors, platform_height):
        ...
        return (np.deg2rad(ll_x) * h, np.deg2rad(ll_y) * h, np.deg2rad(ur_x) * h, np.deg2rad(ur_y) * h)
    ...
    def get_area_def(self, dsid):
        ...
        area = geometry.AreaDefinition(
            "some_area_name", "On-the-fly area", "geosmsg", proj_dict, ncols, nlines, area_extent
        )
    ...
    def read_band(self, key, info):
        ...
        return da.from_delayed(_read_data(self.filename, self.mda), shape=output_shape, dtype=output_dtype)
    ...
    def _read_data_from_disk(self):
        ...
        with utils.unzip_context(self.filename) as fn:
            with decompressed(fn) if self.compressed else nullcontext(fn) as filename:
                return np.fromfile(filename, offset=self.offset, dtype=dtype, count=np.prod(shape))
    ...
    def _read_file_like(self):
        ...
        with utils.generic_open(self.filename, mode="rb") as fp:
            ...
            return np.frombuffer(
                fp.read(np.dtype(dtype).itemsize * no_elements), dtype=np.dtype(dtype), count=no_elements.item()
            ).reshape(shape)

    def _get_input_info(self):
        total_bits = int(self.lines) * int(self.cols) * int(self.bpp)
        input_shape = int(np.ceil(total_bits / 8.0))
        if self.bpp == 16:
            input_dtype = ">u2"
            input_shape //= 2
