...
segment_identification = np.dtype(
    [("image_segm_seq_no", ">u1"), ("total_no_image_segm", ">u1"), ("line_no_image_segm", ">u2")]
)
...
UNKNOWN_AREA = -1
AREA_NAMES = {
    FULL_DISK: {"short": "FLDK", "long": "Full Disk"},
...
SENSORS = {MTSAT1R: "jami", MTSAT2: "mtsat2_imager", HIMAWARI8: "ahi"}
...
class HRITJMAFileHandler(HRITFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, use_acquisition_time_as_start_time=False):
        ...
        super(HRITJMAFileHandler, self).__init__(
            filename, filename_info, filetype_info, (jma_hdr_map, jma_variable_length_headers, jma_text_headers)
        )
    ...
    def _get_platform(self):
        ...
        try:
            ...
            logger.error('Unable to determine platform: Unknown projection name "{}"'.format(self.projection_name))
    ...
    def _check_sensor_platform_consistency(self, sensor):
        ...
        if ref_sensor and not sensor == ref_sensor:
            logger.error(
                "Sensor-Platform mismatch: {} is not a payload of {}. Did you choose the correct reader?".format(
                    sensor, self.platform
                )
            )
    ...
    def _get_acq_time(self):
        ...
        """
        buf_b = np.frombuffer(self.mda["image_observation_time"], dtype=image_observation_time)
