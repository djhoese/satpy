...
def time_seconds(tc_array, year):
    ...
    msecs += word & 1023
    return (
        np.datetime64(str(year) + "-01-01T00:00:00Z", "s")
...
def geo_interpolate(lons32km, lats32km):
    ...
    satint = SatelliteInterpolator(
        (lons32km, lats32km), (rows32km, cols32km), (rows1km, cols1km), along_track_order, cross_track_order
    )
...
def _get_channel_index(key):
    ...
    avhrr_channel_index = {"1": 0, "2": 1, "3a": 2, "3b": 2, "4": 3, "5": 4}
...
class HRPTFile(BaseFileHandler):
    ...
    def calibrate_thermal_channel(self, data, key):
        ...
        line_numbers = (np.round((self.times - self.times[-1]) / np.timedelta64(166666667, "ns"))).astype(int)
        line_numbers -= line_numbers[0]
        prt, ict, space = self.telemetry
        index = _get_channel_index(key)
        data = calibrate_thermal(
            data, prt, ict[:, index - 2], space[:, index], line_numbers, index + 1, self.calibrator
        )
        return data
    ...
    def calibrate_solar_channel(self, data, key):
        ...
        julian_days = (
            np.datetime64(self.start_time) - np.datetime64(str(self.year) + "-01-01T00:00:00Z")
        ) / np.timedelta64(1, "D")
        data = calibrate_solar(data, _get_channel_index(key), self.year, julian_days, self.calibrator)
        return data
    ...
    def lons_lats(self):
        ...
        lons, lats = geo_interpolate(lons.reshape((scanline_nb, -1)), lats.reshape((scanline_nb, -1)))
        return lons, lats
    ...
    def start_time(self):
        """Get the start time."""
        return time_seconds(self._data["timecode"][0, np.newaxis, :], self.year).astype(datetime)[0]

    @property
    def end_time(self):
        """Get the end time."""
        return time_seconds(self._data["timecode"][-1, np.newaxis, :], self.year).astype(datetime)[0]
