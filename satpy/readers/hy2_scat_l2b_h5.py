...
class HY2SCATL2BH5FileHandler(HDF5FileHandler):
    ...
    def start_time(self):
        ...
        return datetime.strptime(self["/attr/Range_Beginning_Time"], "%Y%m%dT%H:%M:%S")

    @property
    def end_time(self):
        """Time for final observation."""
        return datetime.strptime(self["/attr/Range_Ending_Time"], "%Y%m%dT%H:%M:%S")
    ...
    def get_dataset(self, key, info):
        ...
        if key["name"] in "wvc_row_time":
            ...
            if key["name"] in "wvc_lon":
                _attrs = data.attrs
                data = xr.where(data > 180, data - 360.0, data)
