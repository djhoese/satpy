...
SHORT_NAMES = {"M01": "Metop-B", "M02": "Metop-A", "M03": "Metop-C"}
...
GEO_NAMES = {
    ...
    "sensing_time": {"day": "SensingTime_day", "msec": "SensingTime_msec"},
...
class IASIL2HDF5(BaseFileHandler):
    ...
    def end_time(self):
        """Get the end time."""
        end_time = dt.datetime.combine(self.start_time.date(), self.finfo["end_time"].time())
...
def read_geo(fid, key):
    ...
    if "time" in key["name"]:
        ...
        data = fid["/L1C/" + dsid][()]
        dtype = np.float32
    data = xr.DataArray(da.from_array(data, chunks=CHUNK_SIZE), name=key["name"], dims=["y", "x"]).astype(dtype)
