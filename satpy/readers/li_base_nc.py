...
class LINCFileHandler(NetCDF4FileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, cache_handle=True):
        ...
        super().__init__(filename, filename_info, filetype_info, cache_var_size=10000, cache_handle=cache_handle)
    ...
    def get_variable_search_paths(self, var_paths):
        ...
        if len(self.search_paths) == 0:
            ...
            all_var_paths = [f"{folder}/{var_path}" for folder in self.search_paths for var_path in var_paths]
    ...
    def register_available_datasets(self):
        ...
        logger.debug("Adding %d datasets for %s input product.", len(self.dataset_infos), self.product_type)
    ...
    def apply_broadcast_to(self, data_array, ds_info):
        ...
        data_array = xr.DataArray(
            new_array, coords=data_array.coords, dims=dims, name=data_array.name, attrs=data_array.attrs
        )
        return data_array
    ...
    def apply_accumulate_index_offset(self, data_array, ds_info):
        ...
        logger.debug("Adding %d elements for index offset, new value is: %d", ref_var.size, ds_info["__index_offset"])
