...
class MAIAFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.finfo["end_time"] = self.finfo["end_time"].replace(
            year=self.finfo["start_time"].year, month=self.finfo["start_time"].month, day=self.finfo["start_time"].day
        )
        if self.finfo["end_time"] < self.finfo["start_time"]:
            myday = self.finfo["end_time"].day
            self.finfo["end_time"] = self.finfo["end_time"].replace(day=myday + 1)
        self.selected = None
    ...
    def read(self, filename):
        ...
        missing = -9999.0
        self.Lat = da.from_array(self.h5["DATA/Latitude"], chunks=CHUNK_SIZE) / 10000.0
        self.Lon = da.from_array(self.h5["DATA/Longitude"], chunks=CHUNK_SIZE) / 10000.0
        self.selected = self.Lon > missing
        self.file_content = {}
        for key in self.h5["DATA"].keys():
            self.file_content[key] = da.from_array(self.h5["DATA/" + key], chunks=CHUNK_SIZE)
        for key in self.h5["HEADER"].keys():
            self.file_content[key] = self.h5["HEADER/" + key][:]

        # Cloud Mask on pixel
        mask = 2**0 + 2**1 + 2**2
        lst = self.file_content["CloudMask"] & mask
        lst = lst / 2**0
        self.file_content["cma"] = lst

        # Cloud Mask confidence
        mask = 2**5 + 2**6
        lst = self.file_content["CloudMask"] & mask
        lst = lst / 2**5
        self.file_content["cma_conf"] = lst

        # Cloud Mask Quality
        mask = 2**3 + 2**4
        lst = self.file_content["CloudMask"] & mask
        lst = lst / 2**3
        self.file_content["cma_qual"] = lst

        # Opaque Cloud
        mask = 2**21
        lst = self.file_content["CloudMask"] & mask
        lst = lst / 2**21
        self.file_content["opaq_cloud"] = lst

        # land /water Background
        mask = 2**15 + 2**16 + 2**17
        lst = self.file_content["CloudMask"] & mask
        lst = lst / 2**15
        self.file_content["land_water_background"] = lst

        # CT (Actual CloudType)
        mask = 2**4 + 2**5 + 2**6 + 2**7 + 2**8
        classif = self.file_content["CloudType"] & mask
        classif = classif / 2**4
