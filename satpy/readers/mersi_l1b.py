...
class MERSIL1B(HDF5FileHandler):
    ...
    def _get_coefficients(self, cal_key, cal_index):
        ...
        if slope is not None:
            slope, intercept = self._get_single_slope_intercept(slope, intercept, cal_index)
    ...
    def get_dataset(self, dataset_id, ds_info):
        ...
        if dataset_id.get("calibration") == "reflectance":
            coeffs = self._get_coefficients(ds_info["calibration_key"], ds_info["calibration_index"])
            ...
            wave_number = 1.0 / (dataset_id["wavelength"][1] / 1e6)
    ...
    def _mask_data(self, data, dataset_id, attrs):
        ...
        if valid_range is not None:
            ...
            # dead detector == 65533
            data = data.where((data >= valid_range[0]) & (data <= valid_range[1]), new_fill)
        return data
