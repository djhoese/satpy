...
class ModisL2HDFFileHandler(HDFEOSGeoReader):
    ...
    def _extract_and_mask_category_dataset(self, dataset_id, dataset_info, var_name):
        ...
        dataset = _extract_byte_mask(
            dataset, dataset_info["byte"], dataset_info["bit_start"], dataset_info["bit_count"]
        )
    ...
    def _mask_with_quality_assurance_if_needed(self, dataset, dataset_info, dataset_id):
        ...
        quality_assurance_dataset_id = dataset_id.from_dict(dict(name="quality_assurance", resolution=1000))
        ...
        quality_assurance = self.get_dataset(quality_assurance_dataset_id, quality_assurance_dataset_info)
        # Duplicate quality assurance dataset to create relevant filter
        duplication_factor = [
            int(dataset_dim / quality_assurance_dim)
            for dataset_dim, quality_assurance_dim in zip(dataset.shape, quality_assurance.shape)
        ]
