...
PLATFORMS = {"S2A": "Sentinel-2A", "S2B": "Sentinel-2B", "S2C": "Sentinel-2C", "S2D": "Sentinel-2D"}
...
class SAFEMSIL1C(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, mda, tile_mda, mask_saturated=True):
        ...
        super(SAFEMSIL1C, self).__init__(filename, filename_info, filetype_info)
        del mask_saturated
...
class SAFEMSITileMDXML(SAFEMSIXMLMetadata):
    ...
    def get_area_def(self, dsid):
        ...
        area = geometry.AreaDefinition(
            self.tile, "On-the-fly area", self.tile, self.projection, cols, rows, area_extent
        )
        return area
    ...
    def _get_values_from_tag(xml_tree, xml_tag):
        ...
        return np.array([[val for val in elt.text.split()] for elt in elts], dtype=np.float64)
