...
class NcNWCSAF(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.cache = {}
        self.nc = xr.open_dataset(self.filename, decode_cf=True, mask_and_scale=False, chunks=CHUNK_SIZE)
        ...
        self.upsample_geolocation = functools.lru_cache(maxsize=1)(self._upsample_geolocation_uncached)
    ...
    def _prepare_variable_for_palette(self, variable, info):
        ...
        variable.attrs["palette_meanings"] = [int(val) for val in variable.attrs["palette_meanings"].split()]
        ...
        if fill_value not in variable.attrs["palette_meanings"] and "fill_value_color" in variable.attrs:
            ...
            variable = xr.DataArray(
                da.vstack((np.array(variable.attrs["fill_value_color"]), variable.data)),
    ...
    def _adjust_variable_for_legacy_software(self, variable):
        if self.sw_version == "NWC/PPS version v2014" and variable.attrs.get("standard_name") == "cloud_top_altitude":
            ...
            variable.attrs["valid_range"] = (0.0, 9000.0)
        if (
            self.sw_version == "NWC/PPS version v2014"
            and variable.attrs.get("long_name") == "RGB Palette for ctth_alti"
        ):
    ...
    def _upsample_geolocation_uncached(self):
        ...
        satint = SatelliteInterpolator(
            (lon_reduced.values, lat_reduced.values), (row_indices, col_indices), (rows_full, cols_full)
        )
    ...
    def get_area_def(self, dsid):
        ...
        area = AreaDefinition("some_area_name", "On-the-fly area", "geosmsg", crs, ncols, nlines, area_extent)
    ...
    def _ensure_crs_extents_in_meters(crs, area_extent):
        ...
        if "kilo" in crs.axis_info[0].unit_name:
            ...
            if "R" in proj_dict:
                proj_dict["R"] *= 1000.0
            proj_dict["h"] *= 1000.0
            area_extent = tuple([val * 1000.0 for val in area_extent])
    ...
    def _get_projection(self):
        ...
        area_extent = (
            float(self.nc.attrs["gdal_xgeo_up_left"]) / scale,
