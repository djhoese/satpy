...
PLATFORM_NAMES = {"S3A": "Sentinel-3A", "S3B": "Sentinel-3B", "ENV": "Environmental Satellite"}
...
class BitFlags:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(data, xr.DataArray):
            ...
            res = xr.DataArray(res, coords=self._value.coords, attrs=self._value.attrs, dims=self._value.dims)
        else:
            res = ((data >> pos) % 2).astype(bool)
        return res
...
class NCOLCIBase(BaseFileHandler):
    ...
    def nc(self):
        ...
        dataset = xr.open_dataset(
            ...
            mask_and_scale=True,
            engine=self._engine,
            chunks={self.cols_name: CHUNK_SIZE, self.rows_name: CHUNK_SIZE},
        )
...
class NCOLCILowResData(NCOLCIBase):
    ...
    def _do_interpolate(self, data):
        ...
        cross_track_order = 3
        satint = Interpolator(
            [x.values for x in data], (tie_lines, tie_cols), (lines, cols), along_track_order, cross_track_order
        )
        int_data = satint.interpolate()

        return [xr.DataArray(da.from_array(x, chunks=(CHUNK_SIZE, CHUNK_SIZE)), dims=["y", "x"]) for x in int_data]
