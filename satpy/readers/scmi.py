...
class SCMIFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.nc = xr.open_dataset(
            self.filename, decode_cf=True, mask_and_scale=False, chunks={"x": LOAD_CHUNK_SIZE, "y": LOAD_CHUNK_SIZE}
        )
    ...
    def _calc_extents(self, proj_dict):
        ...
        h = float(proj_dict.get("h", 1.0))  # force to 64-bit float
        x = self["x"]
        y = self["y"]
    ...
    def get_area_def(self, key):
        ...
        return geometry.AreaDefinition(
            area_name, "SCMI file area", area_name, proj_dict, self.ncols, self.nlines, np.asarray(area_extent)
        )
