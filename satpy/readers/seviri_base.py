...
SATNUM = {321: "8", 322: "9", 323: "10", 324: "11"}
...
def get_cds_time(days, msecs):
    ...
    time = (
        np.datetime64("1958-01-01").astype("datetime64[ms]")
...
def add_scanline_acq_time(dataset, acq_time):
    ...
    dataset.coords["acq_time"].attrs["long_name"] = "Mean scanline acquisition time"
...
class MpefProductHeader(object):
    ...
    def get(self):
        ...
        record = [
            ...
            ("BaseAlgorithmVersion", issue_revision),
            ("ProductAlgorithmVersion", issue_revision),
...
class SEVIRICalibrationAlgorithm:
    ...
    def _tl15(self, data, wavenumber):
        ...
        return (C2 * wavenumber) / np.log((1.0 / data) * C1 * wavenumber**3 + 1.0)
...
class SEVIRICalibrationHandler:
    ...
    def __init__(self, platform_id, channel_name, coefs, calib_mode, scan_time):
        ...
        self._scan_time = scan_time
        self._algo = SEVIRICalibrationAlgorithm(platform_id=self._platform_id, scan_time=self._scan_time)

        valid_modes = ("NOMINAL", "GSICS")
        if self._calib_mode not in valid_modes:
            raise ValueError("Invalid calibration mode: {}. Choose one of {}".format(self._calib_mode, valid_modes))
    ...
    def calibrate(self, data, calibration):
        ...
        if calibration == "counts":
            ...
            res = self._algo.convert_to_radiance(data.astype(np.float32), gain, offset)
        else:
            raise ValueError("Invalid calibration {} for channel {}".format(calibration, self._channel_name))
        ...
        if calibration == "reflectance":
            ...
            res = self._algo.ir_calibrate(res, self._channel_name, self._coefs["radiance_type"])
...
class OrbitPolynomial:
    ...
    def evaluate(self, time):
        ...
        """
        domain = [np.datetime64(self.start_time).astype("int64"), np.datetime64(self.end_time).astype("int64")]
    ...
    def __eq__(self, other):
        ...
        return (
            np.array_equal(self.coefs, np.array(other.coefs))
...
def get_satpos(orbit_polynomial, time, semi_major_axis, semi_minor_axis):
    ...
    geocent = pyproj.CRS(proj="geocent", a=semi_major_axis, b=semi_minor_axis, units="m")
    latlong = pyproj.CRS(proj="latlong", a=semi_major_axis, b=semi_minor_axis, units="m")
...
class OrbitPolynomialFinder:
    ...
    def get_orbit_polynomial(self, time, max_delta=6):
        ...
        try:
            ...
            warnings.warn("No orbit polynomial valid for {}. Using closest match.".format(time), stacklevel=2)
    ...
    def _get_enclosing_interval(self, time):
        ...
        enclosing = np.where(np.logical_and(time >= self.valid_from, time < self.valid_to))[0]
    ...
    def _get_closest_interval_within(self, time, threshold):
        ...
        raise NoValidOrbitParams("Unable to find orbit coefficients valid for {} +/- {}hours".format(time, threshold))
    ...
    def _get_closest_interval(self, time):
        ...
        """
        intervals_centre = self.valid_from + 0.5 * (self.valid_to - self.valid_from)
...
def create_coef_dict(coefs_nominal, coefs_gsics, radiance_type, ext_coefs):
    ...
    return {
        "coefs": {
            ...
            "GSICS": {"gain": coefs_gsics[0], "offset": coefs_gsics[1]},
            "EXTERNAL": ext_coefs,
        },
