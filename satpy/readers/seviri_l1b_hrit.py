...
# MSG implementation:
key_header = np.dtype([("key_number", "u1"), ("seed", ">f8")])

segment_identification = np.dtype(
    [
        ("GP_SC_ID", ">i2"),
...
msg_variable_length_headers = {image_segment_line_quality: "image_segment_line_quality"}
...
msg_hdr_map.update({7: key_header, 128: segment_identification, 129: image_segment_line_quality})
...
cuc_time = np.dtype([("coarse", "u1", (4,)), ("fine", "u1", (3,))])
...
class HRITMSGPrologueFileHandler(HRITMSGPrologueEpilogueBase):
    ...
    def __init__(
        ...
        super(HRITMSGPrologueFileHandler, self).__init__(
            filename, filename_info, filetype_info, (msg_hdr_map, msg_variable_length_headers, msg_text_headers)
        )
        self.prologue = {}
        self.read_prologue()
    ...
    def satpos(self):
        ...
        poly_finder = OrbitPolynomialFinder(self.prologue["SatelliteStatus"]["Orbit"]["OrbitPolynomial"])
...
class HRITMSGEpilogueFileHandler(HRITMSGPrologueEpilogueBase):
    ...
    def __init__(
        ...
        """Initialize the reader."""
        super(HRITMSGEpilogueFileHandler, self).__init__(
            filename, filename_info, filetype_info, (msg_hdr_map, msg_variable_length_headers, msg_text_headers)
        )
        self.epilogue = {}
        self.read_epilogue()
...
class HRITMSGFileHandler(HRITFileHandler):
    ...
    **Metadata**
    ...
    def _get_header(self):
        ...
        # Misc
        self.platform_id = self.prologue["SatelliteStatus"]["SatelliteDefinition"]["SatelliteId"]
    ...
    def nominal_start_time(self):
        ...
        tm = self.prologue["ImageAcquisition"]["PlannedAcquisitionTime"]["TrueRepeatCycleStart"]
    ...
    def observation_start_time(self):
        ...
        return self.epilogue["ImageProductionStats"]["ActualScanningSummary"]["ForwardScanStart"]
    ...
    def get_area_def(self, dsid):
        ...
        if self.prologue["ImageDescription"]["Level15ImageProduction"]["ImageProcDirection"] == 0:
            pdict["scandir"] = "N2S"
        else:
            pdict["scandir"] = "S2N"
        ...
        current_first_line = (segment_number - self.mda["planned_start_segment_number"]) * pdict["nlines"]
        ...
        upper_south_line = bounds["LowerNorthLineActual"] - current_first_line - 1
    ...
    def pad_hrv_data(self, res):
        ...
        current_first_line = (segment_number - self.mda["planned_start_segment_number"]) * nlines
        ...
        if upper_south_line > 0:
            ...
            data_lower = pad_data_horizontally(
                res[:upper_south_line, :].data,
        ...
        if upper_south_line < nlines:
            ...
            data_upper = pad_data_horizontally(
                res[upper_south_line:, :].data,
    ...
    def _get_calib_coefs(self, channel_name):
        ...
        coefs_nominal = self.prologue["RadiometricProcessing"]["Level15ImageCalibration"]
        coefs_gsics = self.prologue["RadiometricProcessing"]["MPEFCalFeedback"]
        radiance_types = self.prologue["ImageDescription"]["Level15ImageProduction"]["PlannedChanProcessing"]
...
def pad_data(data, final_size, east_bound, west_bound):
    ...
    padding_east = da.zeros((nlines, east_bound - 1), dtype=data.dtype, chunks=CHUNK_SIZE)
    padding_west = da.zeros((nlines, (final_size[1] - west_bound)), dtype=data.dtype, chunks=CHUNK_SIZE)
