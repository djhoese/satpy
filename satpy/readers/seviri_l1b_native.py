...
class NativeMSGFileHandler(BaseFileHandler):
    ...
    **Metadata**
    ...
    def __init__(
        ...
        super(NativeMSGFileHandler, self).__init__(filename, filename_info, filetype_info)
    ...
    def observation_start_time(self):
        ...
        return self.trailer["15TRAILER"]["ImageProductionStats"]["ActualScanningSummary"]["ForwardScanStart"]
    ...
    def _get_data_dtype(self):
        ...
        pkhrec = [("GP_PK_HEADER", GSDTRecords.gp_pk_header), ("GP_PK_SH1", GSDTRecords.gp_pk_sh1)]
        ...
        number_of_visir_channels = len([s for s in self.mda["channel_list"] if not s == "HRV"])
    ...
    def _get_memmap(self):
        ...
        with open(self.filename) as fp:
            ...
            return np.memmap(fp, dtype=data_dtype, shape=(self.mda["number_of_lines"],), offset=hdr_size, mode="r")
    ...
    def _read_header(self):
        ...
        self.mda["channel_list"] = [i for i in CHANNEL_NAMES.values() if self.mda["available_channels"][i]]

        self.platform_id = data15hd["SatelliteStatus"]["SatelliteDefinition"]["SatelliteId"]
        self.mda["platform_name"] = "Meteosat-" + SATNUM[self.platform_id]
        self.mda["offset_corrected"] = data15hd["GeometricProcessing"]["EarthModel"]["TypeOfEarthModel"] == 2

        equator_radius = data15hd["GeometricProcessing"]["EarthModel"]["EquatorialRadius"] * 1000.0
        north_polar_radius = data15hd["GeometricProcessing"]["EarthModel"]["NorthPolarRadius"] * 1000.0
        south_polar_radius = data15hd["GeometricProcessing"]["EarthModel"]["SouthPolarRadius"] * 1000.0
        polar_radius = (north_polar_radius + south_polar_radius) * 0.5
        ssp_lon = data15hd["ImageDescription"]["ProjectionDescription"]["LongitudeOfSSP"]
        ...
        if cols_visir_hdr != cols_visir:
            logger.warning("Number of VISIR columns from the header is incorrect!")
        ...
        if "15_MAIN_PRODUCT_HEADER" not in self.header:
            ...
            warnings.warn(
                "The quality flag for this file indicates not OK. Use this data with caution!",
                UserWarning,
                stacklevel=2,
            )
    ...
    def _read_trailer(self):
        hdr_size = self.header_type.itemsize
        data_size = self._get_data_dtype().itemsize * self.mda["number_of_lines"]
    ...
    def get_area_def(self, dataset_id):
        ...
        area_naming = get_geos_area_naming({**area_naming_input_dict, **get_service_mode("seviri", pdict["ssp_lon"])})
    ...
    def get_area_extent(self, dataset_id):
        ...
        earth_model = data15hd["GeometricProcessing"]["EarthModel"]["TypeOfEarthModel"]
        if earth_model == 2:
            ns_offset = 0
            we_offset = 0
        elif earth_model == 1:
            ...
            raise NotImplementedError("Unrecognised Earth model: {}".format(earth_model))
        ...
        if grid_origin != 2:
            msg = "Grid origin not supported number: {}, {} corner".format(grid_origin, origins[grid_origin])
    ...
    def get_dataset(self, dataset_id, dataset_info):
        ...
        if self.fill_disk and not (dataset_id["name"] != "HRV" and self.mda["is_full_disk"]):
            padder = Padder(
                dataset_id, self.image_boundaries.get_img_bounds(dataset_id, self.is_roi()), self.mda["is_full_disk"]
            )
    ...
    def _get_calib_coefs(self, channel_name):
        ...
        coefs_nominal = self.header["15_DATA_HEADER"]["RadiometricProcessing"]["Level15ImageCalibration"]
        coefs_gsics = self.header["15_DATA_HEADER"]["RadiometricProcessing"]["MPEFCalFeedback"]
        radiance_types = self.header["15_DATA_HEADER"]["ImageDescription"]["Level15ImageProduction"][
            "PlannedChanProcessing"
        ]
        return create_coef_dict(
            coefs_nominal=(coefs_nominal["CalSlope"][band_idx], coefs_nominal["CalOffset"][band_idx]),
            coefs_gsics=(coefs_gsics["GSICSCalCoeff"][band_idx], coefs_gsics["GSICSOffsetCount"][band_idx]),
    ...
    def _get_acq_time_hrv(self):
        ...
        tline1 = tline[:, 1]
        tline2 = tline[:, 2]
        return da.stack((tline0, tline1, tline2), axis=1).reshape(self.mda["hrv_number_of_lines"]).compute()
    ...
    def _update_attrs(self, dataset, dataset_info):
        ...
        if self.include_raw_metadata:
            dataset.attrs["raw_metadata"] = reduce_mda(self.header, max_size=self.mda_max_array_size)
    ...
    def _get_orbital_parameters(self):
        orbital_parameters = {
            ...
            "satellite_nominal_longitude": self.header["15_DATA_HEADER"]["SatelliteStatus"]["SatelliteDefinition"][
                "NominalLongitude"
            ],
    ...
    def satpos(self):
        ...
        poly_finder = OrbitPolynomialFinder(
            self.header["15_DATA_HEADER"]["SatelliteStatus"]["Orbit"]["OrbitPolynomial"]
        )
...
def get_available_channels(header):
    ...
    channels_str = header["15_SECONDARY_PRODUCT_HEADER"]["SelectedBandIDs"]["Value"]
    available_channels = {}
