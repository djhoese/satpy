...
class NCSEVIRIFileHandler(BaseFileHandler):
    ...
    **Metadata**
    ...
    def __init__(self, filename, filename_info, filetype_info, ext_calib_coefs=None, mask_bad_quality_scan_lines=True):
    ...
    def nc(self):
        """Read the file."""
        return open_dataset(self.filename, decode_cf=True, mask_and_scale=False, chunks=CHUNK_SIZE).rename(
            {"num_columns_vis_ir": "x", "num_rows_vis_ir": "y"}
        )

    def get_metadata(self):
        """Get metadata."""
        # Obtain some area definition attributes
        equatorial_radius = self.nc.attrs["equatorial_radius"] * 1000.0
    ...
    def _get_calib_coefs(self, dataset, channel):
        ...
        return {
            "coefs": {"NOMINAL": {"gain": gain, "offset": offset}, "EXTERNAL": self.ext_calib_coefs.get(channel, {})},
    ...
    def _update_attrs(self, dataset, dataset_info):
        ...
        dataset.attrs["orbital_parameters"] = {
            ...
            "satellite_nominal_longitude": float(self.nc.attrs["nominal_longitude"]),
    ...
    def get_area_extent(self, dsid):
        ...
        if grid_origin != 2:
            raise NotImplementedError(
                "Grid origin not supported number: {}, {} corner".format(grid_origin, origins[grid_origin])
            )
    ...
    def satpos(self):
        ...
        end_times_poly = get_cds_time(
            days=self.nc["orbit_polynomial_end_time_day"].values, msecs=self.nc["orbit_polynomial_end_time_msec"].values
        )
        orbit_polynomials = {
