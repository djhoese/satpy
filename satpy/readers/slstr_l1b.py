...
PLATFORM_NAMES = {"S3A": "Sentinel-3A", "S3B": "Sentinel-3B"}
...
class NCSLSTRGeo(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.nc = xr.open_dataset(
            self.filename, decode_cf=True, mask_and_scale=True, chunks={"columns": CHUNK_SIZE, "rows": CHUNK_SIZE}
        )
...
class NCSLSTR1B(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, user_calibration=None):
        ...
        self.cal = xr.open_dataset(cal_file, decode_cf=True, mask_and_scale=True, chunks={"views": CHUNK_SIZE})
        indices_file = os.path.join(os.path.dirname(self.filename), "indices_{}{}.nc".format(self.stripe, self.view[0]))
        self.indices = xr.open_dataset(
            indices_file, decode_cf=True, mask_and_scale=True, chunks={"columns": CHUNK_SIZE, "rows": CHUNK_SIZE}
        )
    ...
    def _apply_radiance_adjustment(self, radiances):
        ...
        if adjust_fac is None:
            if chan_name in CHANCALIB_FACTORS:
                ...
                warnings.warn("Warning: No radiance adjustment supplied " + "for channel " + chan_name, stacklevel=3)
                return radiances
    ...
    def get_dataset(self, key, info):
        """Load a dataset."""
        if self.channel not in key["name"] or self.stripe != key["stripe"].name or self.view != key["view"].name:
            return
        logger.debug("Reading %s.", key["name"])
        ...
        info.update(dict(units=units, platform_name=self.platform_name, sensor=self.sensor, view=self.view))
...
class NCSLSTRAngles(BaseFileHandler):
    ...
    def _loadcart(self, fname):
        ...
        cartf = xr.open_dataset(
            fname, decode_cf=True, mask_and_scale=True, chunks={"columns": CHUNK_SIZE, "rows": CHUNK_SIZE}
        )
        return cartf
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        carta_file = os.path.join(os.path.dirname(self.filename), "cartesian_a{}.nc".format(self.view[0]))
        carti_file = os.path.join(os.path.dirname(self.filename), "cartesian_i{}.nc".format(self.view[0]))
        cartx_file = os.path.join(os.path.dirname(self.filename), "cartesian_tx.nc")
    ...
    def get_dataset(self, key, info):
        ...
        if c_step != 1 or l_step != 1:
            ...
            variable = xr.DataArray(
                da.from_array(values, chunks=(CHUNK_SIZE, CHUNK_SIZE)), dims=["y", "x"], attrs=variable.attrs
            )
...
class NCSLSTRFlag(BaseFileHandler):
    ...
    def get_dataset(self, key, info):
        ...
        info.update(variable.attrs)
        info.update(key.to_dict())
        info.update(dict(platform_name=self.platform_name, sensor=self.sensor))
