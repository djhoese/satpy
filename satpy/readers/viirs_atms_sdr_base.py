...
ATMS_DATASET_KEYS = {"SATMS": "ATMS-SDR", "GATMO": "ATMS-SDR-GEO", "TATMS": "ATMS-TDR"}
...
def _get_scale_factors_for_units(factors, file_units, output_units):
    if file_units == "W cm-2 sr-1" and output_units == "W m-2 sr-1":
        ...
        raise ValueError("Don't know how to convert '{}' to '{}'".format(file_units, output_units))
    return factors
...
class JPSS_SDR_FileHandler(HDF5FileHandler):
    ...
    def _parse_datetime(self, datestr, timestr):
        try:
            ...
            datetime_str = str(datestr.data.compute().astype(str)) + str(timestr.data.compute().astype(str))
    ...
    def sensor_name(self):
        ...
        sensor_path = self.filetype_info.get("sensor_name", default).format(dataset_group=dataset_group)
    ...
    def _map_and_apply_factors(data, factors, rows_per_gran):
        ...
        data = xr.DataArray(dask_data.rechunk(old_chunks), dims=data.dims, coords=data.coords, attrs=data.attrs)
        return data
    ...
    def _get_valid_scaling_factors(factors):
        if factors is None:
            ...
            factors = factors.where(factors != -999.0, np.float32(np.nan))
        return factors
