...
class VIIRSCompactFileHandler(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        super(VIIRSCompactFileHandler, self).__init__(filename, filename_info, filetype_info)
        ...
        self.switch_to_cart = (abs(self.max_lon - self.min_lon) > 90) or (
            max(abs(self.min_lat), abs(self.max_lat)) > 60
        )
        ...
        self.c_align = da.from_array(self.geography["AlignmentCoefficient"], chunks=tuple(self.nb_tiepoint_zones))
    ...
    def end_time(self):
        """Get the end time."""
        end_time = datetime.combine(self.start_time.date(), self.finfo["end_time"].time())
    ...
    def read_geo(self, key, info):
        """Read angles."""
        pairs = {
            ("satellite_azimuth_angle", "satellite_zenith_angle"): ("SatelliteAzimuthAngle", "SatelliteZenithAngle"),
            ("solar_azimuth_angle", "solar_zenith_angle"): ("SolarAzimuthAngle", "SolarZenithAngle"),
            ("dnb_solar_azimuth_angle", "dnb_solar_zenith_angle"): ("SolarAzimuthAngle", "SolarZenithAngle"),
            ("dnb_lunar_azimuth_angle", "dnb_lunar_zenith_angle"): ("LunarAzimuthAngle", "LunarZenithAngle"),
        }
        if self.lons is None or self.lats is None:
            self.lons, self.lats = self.navigate()
        for pair, fkeys in pairs.items():
            if key["name"] in pair:
                if self.cache.get(pair[0]) is None or self.cache.get(pair[1]) is None:
                    angles = self.angles(*fkeys)
                    self.cache[pair[0]], self.cache[pair[1]] = angles
                if key["name"] == pair[0]:
                    return xr.DataArray(self.cache[pair[0]], name=key["name"], attrs=self.mda, dims=("y", "x"))
                else:
                    return xr.DataArray(self.cache[pair[1]], name=key["name"], attrs=self.mda, dims=("y", "x"))
        ...
        if key["name"] == "dnb_moon_illumination_fraction":
            mda = self.mda.copy()
            mda.update(info)
            return xr.DataArray(da.from_array(self.geography["MoonIllumFraction"]), attrs=info)

    def read_dataset(self, dataset_key, info):
        """Read a dataset."""
        h5f = self.h5f
        channel = _channels_dict[dataset_key["name"]]
        chan_dict = dict([(key.split("-")[1], key) for key in h5f["All_Data"].keys() if key.startswith("VIIRS")])

        h5rads = h5f["All_Data"][chan_dict[channel]]["Radiance"]
        chunks = h5rads.chunks or CHUNK_SIZE
        rads = xr.DataArray(da.from_array(h5rads, chunks=chunks), name=dataset_key["name"], dims=["y", "x"]).astype(
            np.float32
        )
        ...
        try:
            rads = xr.where(
                ...
                rads * h5attrs["RadianceScaleLow"] + h5attrs["RadianceOffsetLow"],
                rads * h5attrs["RadianceScaleHigh"] + h5attrs["RadianceOffsetHigh"],
            )
        ...
        if dataset_key["calibration"] in ["reflectance", "brightness_temperature"]:
            ...
            try:
                ...
                try:
                    ...
                    rads *= 1e6
                    rads = (h * c) / (k * lambda_c * np.log(1 + (2 * h * c**2) / ((lambda_c**5) * rads)))
                    rads *= a_ir
                    rads += b_ir
                    unit = "K"
                except KeyError:
            ...
            raise ValueError("Calibration parameter should be radiance, reflectance or brightness_temperature")
        ...
        for array in arrays:
            res.append(
                da.map_blocks(
                    expand,
                    array[:, :, np.newaxis],
                    self.expansion_coefs,
                    scans=self.scans,
                    scan_size=self.scan_size,
                    dtype=array.dtype,
                    drop_axis=2,
                    chunks=self.expansion_coefs.chunks[:-1],
                )
            )
        return res
    ...
    def expansion_coefs(self):
        ...
        self._expansion_coefs = da.map_blocks(
            get_coefs,
            self.c_align,
            self.c_exp,
            self.tpz_sizes,
    ...
    def angles(self, azi_name, zen_name):
        ...
        switch_to_cart = (
            (np.max(azi) - np.min(azi) > 5) or (np.min(zen) < 10) or (max(abs(self.min_lat), abs(self.max_lat)) > 80)
        )
...
def expand_arrays(
    arrays, scans, c_align, c_exp, scan_size=16, tpz_size=16, nties=200, track_offset=0.5, scan_offset=0.5
):
    """Expand *data* according to alignment and expansion."""
    nties = nties.item()
    tpz_size = tpz_size.item()
    s_scan, s_track = da.meshgrid(da.arange(nties * tpz_size), da.arange(scans * scan_size))
