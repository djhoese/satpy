...
class VIRR_L1B(HDF5FileHandler):
    ...
    def get_dataset(self, dataset_id, ds_info):
        ...
        data.attrs.update(
            {"platform_name": self["/attr/Satellite Name"], "sensor": self["/attr/Sensor Identification Code"].lower()}
        )
    ...
    def _calibrate_emissive(self, data, band_index):
        slope = self._correct_slope(
            self[self.l1b_prefix + "Emissive_Radiance_Scales"].data[:, band_index][:, np.newaxis]
        )
