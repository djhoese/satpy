...
def _verify_reader_info_assign_config_files(config, config_files):
    try:
        ...
        raise KeyError("Malformed config file {}: missing reader 'reader'".format(config_files))
    else:
...
class AbstractYAMLReader(metaclass=ABCMeta):
    ...
    def __init__(self, config_dict):
        ...
        if isinstance(config_dict, str):
            raise ValueError(
                "Passing config files to create a Reader is deprecated. Use ReaderClass.from_config_files instead."
            )
        ...
        for file_type, filetype_info in self.config["file_types"].items():
            ...
            file_patterns = [os.path.join(*pattern.split("/")) for pattern in filetype_info["file_patterns"]]
    ...
    def available_dataset_ids(self):
        ...
        logger.warning("Available datasets are unknown, returning all datasets...")
    ...
    def supports_sensor(self, sensor):
        ...
        """
        if sensor and not (set(self.info.get("sensors")) & set(listify_string(sensor))):
            return False
        return True

    def select_files_from_directory(self, directory=None, fs=None):
        ...
        all_globs = {os.path.join(directory, globify(pattern)) for pattern in self.file_patterns}
    ...
    def load_ds_ids_from_config(self):
        ...
        for dataset in self.datasets.values():
            ...
            if "coordinates" in dataset and isinstance(dataset["coordinates"], list):
...
class FileYAMLReader(AbstractYAMLReader, DataDownloadMixin):
    ...
    def __init__(self, config_dict, filter_parameters=None, filter_filenames=True, **kwargs):
    ...
    def sensor_names(self):
        ...
        file_handlers = (handlers[0] for handlers in self.file_handlers.values())
        sensor_names = set()
    ...
    def check_file_covers_area(file_handler, check_area):
        ...
        try:
            ...
            logger.debug("Bounding box computation not implemented: %s", str(err))
        else:
    ...
    def sorted_filetype_items(self):
        ...
        while len(file_type_items):
            ...
            if requirements is not None:
                ...
                missing = [req for req in requirements if req not in processed_types]
                if missing:
    ...
    def filename_items_for_filetype(filenames, filetype_info):
        ...
        for pattern in filetype_info["file_patterns"]:
            ...
            for filename in matches:
                try:
                    filename_info = parse(pattern, _get_filebase(filename, pattern))
                except ValueError:
    ...
    def _new_filehandler_instances(self, filetype_info, filename_items, fh_kwargs=None):
        ...
        for filename, filename_info in filename_items:
            try:
                ...
                msg = "No handler for reading requirement {} for {}".format(req, filename)
    ...
    def metadata_matches(self, sample_dict, file_handler=None):
        ...
        if not self.time_matches(sample_dict.get("start_time"), sample_dict.get("end_time")):
            return False
        for key, val in self.filter_parameters.items():
            ...
            if key in ["start_time", "end_time"]:
                ...
                if not self.check_file_covers_area(file_handler, val):
                    logger.info("Filtering out %s based on area", file_handler.filename)
                    break
    ...
    def filter_filenames_by_info(self, filename_items):
        ...
        for filename, filename_info in filename_items:
            ...
            if fend and fend < fstart:
                ...
                fend = fend.replace(year=fstart.year, month=fstart.month, day=fstart.day)
    ...
    def filter_selected_filenames(self, filenames):
        ...
        for _, filetype_info in self.sorted_filetype_items():
            filename_iter = self.filename_items_for_filetype(filenames, filetype_info)
    ...
    def _new_filehandlers_for_filetype(self, filetype_info, filenames, fh_kwargs=None):
        ...
        if self.filter_filenames:
            ...
            filename_iter = self.filter_filenames_by_info(filename_iter)
        filehandler_iter = self._new_filehandler_instances(filetype_info, filename_iter, fh_kwargs=fh_kwargs)
    ...
    def create_filehandlers(self, filenames, fh_kwargs=None):
        ...
        for filetype, filetype_info in self.sorted_filetype_items():
            filehandlers = self._new_filehandlers_for_filetype(filetype_info, filename_set, fh_kwargs=fh_kwargs)

            if filehandlers:
                created_fhs[filetype] = filehandlers
                self.file_handlers[filetype] = sorted(
                    self.file_handlers.get(filetype, []) + filehandlers, key=lambda fhd: (fhd.start_time, fhd.filename)
                )
    ...
    def _load_dataset(dsid, ds_info, file_handlers, dim="y", **kwargs):
        ...
        combined_info = file_handlers[0].combine_info([p.attrs for p in slice_list])
    ...
    def _get_file_handlers(self, dsid):
        ...
        if filetype is None:
            logger.warning("Required file type '%s' not found or loaded for '%s'", ds_info["file_type"], dsid["name"])
        else:
    ...
    def _make_swath_definition_from_lons_lats(self, lons, lats):
        ...
        if sdef is None:
            ...
            sdef.name = "{}_{}_{}_{}".format(
                sensor_str, shape_str, lons.attrs.get("name", lons.name), lats.attrs.get("name", lats.name)
            )
            if key is not None:
...
def _set_orientation(dataset, upper_right_corner):
    ...
    if upper_right_corner == "native":
        logger.debug(
            "Requested orientation for Dataset {} is 'native' (default). No flipping is applied.".format(
                dataset.attrs.get("name")
            )
        )
        return dataset
    ...
    if upper_right_corner not in ["NW", "NE", "SE", "SW", "native"]:
        raise ValueError(
            "Target orientation for Dataset {} not recognized. "
    ...
    if "area" not in dataset.attrs:
        logger.info(
            "Dataset {} is missing the area attribute and will not be flipped.".format(
                dataset.attrs.get("name", "unknown_name")
            )
        )
        return dataset
    ...
    if isinstance(dataset.attrs["area"], SwathDefinition):
        logger.info(
            "Dataset {} is in a SwathDefinition and will not be flipped.".format(
                dataset.attrs.get("name", "unknown_name")
            )
        )
        return dataset
    ...
    if projection_type not in accepted_geos_proj_types:
        logger.info(
            "Dataset {} is not in one of the known geostationary projections {} and cannot be flipped.".format(
                dataset.attrs.get("name", "unknown_name"), accepted_geos_proj_types
            )
        )
        return dataset
    ...
    if target_northup == current_northup and target_eastright == current_eastright:
        logger.info(
            "Dataset {} is already in the target orientation and will not be flipped.".format(
                dataset.attrs.get("name", "unknown_name")
            )
        )
        return dataset
    ...
    if target_northup != current_northup:
        dataset, area_extents_to_update = _flip_dataset_data_and_area_extents(
            dataset, area_extents_to_update, "upsidedown"
        )
    if target_eastright != current_eastright:
        dataset, area_extents_to_update = _flip_dataset_data_and_area_extents(
            dataset, area_extents_to_update, "leftright"
        )

    dataset.attrs["area"] = _get_new_flipped_area_definition(
        dataset.attrs["area"], area_extents_to_update, flip_areadef_stacking=target_northup != current_northup
    )
...
class GEOSegmentYAMLReader(GEOFlippableFileYAMLReader):
    ...
    def create_filehandlers(self, filenames, fh_kwargs=None):
        ...
        created_fhs = super(GEOSegmentYAMLReader, self).create_filehandlers(filenames, fh_kwargs=fh_kwargs)
    ...
    def _load_dataset(self, dsid, ds_info, file_handlers, dim="y", pad_data=True):
        """Load only a piece of the dataset."""
        if not pad_data:
            return FileYAMLReader._load_dataset(dsid, ds_info, file_handlers)

        counter, expected_segments, slice_list, failure, projectable = _find_missing_segments(
            file_handlers, ds_info, dsid
        )
    ...
    def _pad_later_segments_area(self, file_handlers, dsid):
        ...
        filetype = file_handlers[0].filetype_info["file_type"]
        available_segments = [int(fh.filename_info.get("segment", 1)) for fh in file_handlers]

        area_defs = self._get_segments_areadef_with_later_padded(
            file_handlers, filetype, dsid, available_segments, expected_segments
        )

        return area_defs

    def _get_segments_areadef_with_later_padded(
        self, file_handlers, filetype, dsid, available_segments, expected_segments
    ):
        seg_size = None
        area_defs = {}
    ...
    def _get_new_areadef_for_padded_segment(self, area, filetype, seg_size, segment, padding_type):
        ...
        new_height_px, new_ll_y, new_ur_y = self._get_y_area_extents_for_padded_segment(
            area, filetype, padding_type, seg_size, segment
        )

        fill_extent = (area.area_extent[0], new_ll_y, area.area_extent[2], new_ur_y)
        area = AreaDefinition("fill", "fill", "fill", area.crs, seg_size[1], new_height_px, fill_extent)
        return area

    def _get_y_area_extents_for_padded_segment(self, area, filetype, padding_type, seg_size, segment):
        new_height_proj_coord, new_height_px = self._get_new_areadef_heights(
            area, seg_size, segment_n=segment, filetype=filetype
        )
...
def _stack_area_defs(area_def_dict):
    ...
    area_defs = [area_def_dict[area_def] for area_def in sorted(area_def_dict.keys()) if area_def is not None]
...
def _find_missing_segments(file_handlers, ds_info, dsid):
    ...
    for fh in handlers:
        ...
        try:
            ...
            logger.warning("Failed to load %s from %s", str(dsid), str(fh), exc_info=True)
...
class GEOVariableSegmentYAMLReader(GEOSegmentYAMLReader):
    ...
    def _initialise_segment_infos(self, filetype):
        ...
        self.segment_infos.update(
            {
                filetype: {
                    "available_segment_infos": [],
...
def _compute_positioning_data_for_missing_group(segment_start_rows, segment_end_rows, segment_heights, group):
    ...
    proposed_sizes_missing_segments = _compute_proposed_sizes_of_missing_segments_in_group(
        group, segment_end_rows, segment_start_rows
    )
    _populate_start_end_rows_of_missing_segments_with_proposed_sizes(
        group, proposed_sizes_missing_segments, segment_start_rows, segment_end_rows, segment_heights
    )


def _populate_start_end_rows_of_missing_segments_with_proposed_sizes(
    group, proposed_sizes_missing_segments, segment_start_rows, segment_end_rows, segment_heights
):
...
def _init_positioning_arrays_for_variable_padding(chk_infos, grid_type, exp_segment_nr):
    ...
    _populate_positioning_arrays_with_available_segment_info(
        chk_infos, grid_type, segment_start_rows, segment_end_rows, segment_heights
    )
    return segment_start_rows, segment_end_rows, segment_heights


def _populate_positioning_arrays_with_available_segment_info(
    chk_infos, grid_type, segment_start_rows, segment_end_rows, segment_heights
):
