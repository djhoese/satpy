...
class BaseResampler(object):
    ...
    def _create_cache_filename(self, cache_dir, prefix="", fmt=".zarr", **kwargs):
...
class KDTreeResampler(BaseResampler):
    ...
    def precompute(self, mask=None, radius_of_influence=None, epsilon=0, cache_dir=None, **kwargs):
    ...
    def _adjust_radius_of_influence(self, radius_of_influence):
        ...
        warnings.warn("Upgrade 'pyresample' for a more accurate default 'radius_of_influence'.", stacklevel=3)
        try:
            radius_of_influence = self.source_geo_def.lons.resolution * 3
        except AttributeError:
            try:
                radius_of_influence = (
                    max(abs(self.source_geo_def.pixel_size_x), abs(self.source_geo_def.pixel_size_y)) * 3
                )
            except AttributeError:
    ...
    def _check_numpy_cache(self, cache_dir, mask=None, **kwargs):
        """Check if there's Numpy cache file and convert it to zarr."""
        if cache_dir is None:
            return
        fname_np = self._create_cache_filename(cache_dir, prefix="resample_lut-", mask=mask, fmt=".npz", **kwargs)
        fname_zarr = self._create_cache_filename(cache_dir, prefix="nn_lut-", mask=mask, fmt=".zarr", **kwargs)
        LOG.debug("Check if %s exists", fname_np)
        if os.path.exists(fname_np) and not os.path.exists(fname_zarr):
            import warnings

            warnings.warn("Using Numpy files as resampling cache is deprecated.", stacklevel=3)
    ...
    def load_neighbour_info(self, cache_dir, mask=None, **kwargs):
        ...
        for idx_name in NN_COORDINATES:
            if mask_name in self._index_caches:
                cached[idx_name] = self._apply_cached_index(self._index_caches[mask_name][idx_name], idx_name)
            elif cache_dir:
                try:
                    filename = self._create_cache_filename(cache_dir, prefix="nn_lut-", mask=mask_name, **kwargs)
    ...
    def save_neighbour_info(self, cache_dir, mask=None, **kwargs):
        ...
        if cache_dir:
            ...
            for idx_name, coord in NN_COORDINATES.items():
                ...
                cache[idx_name] = self._apply_cached_index(cache[idx_name], idx_name, persist=True)
    ...
    def _read_resampler_attrs(self):
        ...
        return {attr_name: getattr(self.resampler, attr_name) for attr_name in NN_COORDINATES}

    def compute(self, data, weight_funcs=None, fill_value=np.nan, with_uncert=False, **kwargs):
        """Resample data."""
        del kwargs
...
class _LegacySatpyEWAResampler(BaseResampler):
    ...
    def precompute(self, cache_dir=None, swath_usage=0, **kwargs):
        ...
        if kwargs.get("mask") is not None:
            LOG.warning("'mask' parameter has no affect during EWA resampling")
        ...
        res = da.map_blocks(
            self._call_ll2cr, lons, lats, target_geo_def, swath_usage, dtype=lons.dtype, chunks=chunks, new_axis=[0]
        )
        cols = res[0]
        rows = res[1]
    ...
    def _call_fornav(self, cols, rows, target_geo_def, data, grid_coverage=0, **kwargs):
        """Wrap fornav() to run as a dask delayed."""
        num_valid_points, res = fornav(cols, rows, target_geo_def, data, **kwargs)
        ...
        if not grid_covered:
            msg = "EWA resampling only found %f%% of the grid covered (need %f%%)" % (
    ...
    def compute(
        ...
        **kwargs,
        ...
        # "scanline"
        rows_per_scan = kwargs.get("rows_per_scan", data.attrs.get("rows_per_scan", data.shape[0]))

        if data.ndim == 3 and "bands" in data.dims:
            data_in = tuple(data.sel(bands=band).data for band in data["bands"])
        elif data.ndim == 2:
            data_in = data.data
        else:
...
class BilinearResampler(BaseResampler):
    ...
    def precompute(self, mask=None, radius_of_influence=50000, epsilon=0, reduce_data=True, cache_dir=False, **kwargs):
    ...
    def load_bil_info(self, cache_dir, **kwargs):
        ...
        if cache_dir:
            filename = self._create_cache_filename(cache_dir, prefix="bil_lut-", **kwargs)
            try:
                self.resampler.load_resampling_info(filename)
            except AttributeError:
                warnings.warn(
                    "Bilinear resampler can't handle caching, please upgrade Pyresample to 0.17.0 or newer.",
                    stacklevel=2,
                )
                raise IOError
        else:
    ...
    def compute(self, data, fill_value=None, **kwargs):
        ...
        res = self.resampler.get_sample_from_bil_info(data, fill_value=fill_value, output_shape=target_shape)
...
def _move_existing_caches(cache_dir, filename):
    ...
    try:
        ...
        os.remove(os.path.join(old_cache_dir, os.path.basename(filename)))
        shutil.move(filename, old_cache_dir)
...
def _mean(data, y_size, x_size):
    ...
    new_shape = (int(rows / y_size), int(y_size), int(cols / x_size), int(x_size))
...
class NativeResampler(BaseResampler):
    ...
    def resample(self, data, cache_dir=None, mask_area=False, **kwargs):
        ...
        return super(NativeResampler, self).resample(data, cache_dir=cache_dir, mask_area=mask_area, **kwargs)
    ...
    def _expand_reduce(cls, d_arr, repeats):
        ...
        if all(x <= 1 for x in repeats.values()):
            # reduce
            y_size = 1.0 / repeats[0]
            x_size = 1.0 / repeats[1]
            return _aggregate(d_arr, y_size, x_size)
        raise ValueError("Must either expand or reduce in both directions")
    ...
    def compute(self, data, expand=True, **kwargs):
        ...
        if isinstance(self.target_geo_def, (list, tuple)):
            ...
            target_geo_def = test_func(self.target_geo_def, key=lambda x: x.shape)
        else:
            target_geo_def = self.target_geo_def
        ...
        repeats = {axis_idx: 1.0 for axis_idx in range(data.ndim) if axis_idx not in [y_axis, x_axis]}
...
def _aggregate(d, y_size, x_size):
    ...
    return da.core.map_blocks(
        _mean, d, y_size, x_size, meta=np.array((), dtype=d.dtype), dtype=d.dtype, chunks=new_chunks
    )
...
def _rechunk_if_nonfactor_chunks(dask_arr, y_size, x_size):
    ...
    if need_rechunk:
        warnings.warn(
            "Array chunk size is not divisible by aggregation factor. Re-chunking to continue native resampling.",
            PerformanceWarning,
            stacklevel=5,
        )
...
def _replicate(d_arr, repeats):
    ...
    d_arr = d_arr.map_blocks(
        _repeat_by_factor, meta=np.array((), dtype=d_arr.dtype), dtype=d_arr.dtype, chunks=repeated_chunks
    )
    return d_arr
...
class BucketResamplerBase(BaseResampler):
    ...
    def precompute(self, **kwargs):
        ...
        self.resampler = bucket.BucketResampler(self.target_geo_def, source_lons, source_lats)
    ...
    def resample(self, data, **kwargs):
        ...
        if not PR_USE_SKIPNA and "skipna" in kwargs:
            raise ValueError(
                "You are trying to set the skipna argument but you are using an old version of"
        ...
        result = xr.DataArray(result, dims=dims, coords=coords, attrs=attrs)
    ...
    def compute(self, data, fill_value=np.nan, skipna=True, **kwargs):
        ...
        if data.ndim == 3:
            for i in range(data.shape[0]):
                res = self.resampler.get_average(data[i, :, :], fill_value=fill_value, **kwargs)
                results.append(res)
        else:
            res = self.resampler.get_average(data, fill_value=fill_value, **kwargs)
            results.append(res)
...
class BucketSum(BucketResamplerBase):
    ...
    def compute(self, data, skipna=True, **kwargs):
        ...
        if data.ndim == 3:
            for i in range(data.shape[0]):
                res = self.resampler.get_sum(data[i, :, :], **kwargs)
                results.append(res)
        else:
...
class BucketFraction(BucketResamplerBase):
    ...
    def compute(self, data, fill_value=np.nan, categories=None, **kwargs):
        ...
        result = self.resampler.get_fractions(data, categories=categories, fill_value=fill_value)
...
def prepare_resampler(source_area, destination_area, resampler=None, **resample_kwargs):
    ...
    if isinstance(resampler, (BaseResampler, PRBaseResampler)):
        raise ValueError("Trying to create a resampler when one already exists.")
    if isinstance(resampler, str):
        ...
        if resampler_class is None:
            if resampler == "gradient_search":
                warnings.warn("Gradient search resampler not available. Maybe missing `shapely`?", stacklevel=2)
            raise KeyError("Resampler '%s' not available" % resampler)
    else:
        resampler_class = resampler

    key = (resampler_class, source_area, destination_area, hash_dict(resample_kwargs).hexdigest())
    try:
...
def resample(source_area, data, destination_area, resampler=None, **kwargs):
    ...
    if not isinstance(resampler, (BaseResampler, PRBaseResampler)):
        ...
        _, resampler_instance = prepare_resampler(source_area, destination_area, resampler)
    else:
...
def resample_dataset(dataset, destination_area, **kwargs):
    ...
    try:
        ...
        LOG.info("Cannot reproject dataset %s, missing area info", dataset.attrs["name"])
