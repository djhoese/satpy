...
class TestABIComposites(unittest.TestCase):
    ...
    def test_simulated_green(self):
        ...
        comp = SimulatedGreen(
            "green", prerequisites=("C01", "C02", "C03"), standard_name="toa_bidirectional_reflectance"
        )
        c01 = xr.DataArray(
            da.zeros((rows, cols), chunks=25) + 0.25, dims=("y", "x"), attrs={"name": "C01", "area": area}
        )
        c02 = xr.DataArray(
            da.zeros((rows, cols), chunks=25) + 0.30, dims=("y", "x"), attrs={"name": "C02", "area": area}
        )
        c03 = xr.DataArray(
            da.zeros((rows, cols), chunks=25) + 0.35, dims=("y", "x"), attrs={"name": "C03", "area": area}
        )
