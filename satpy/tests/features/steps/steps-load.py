...
def step_impl_data_available(context):
    ...
    if not os.path.exists("/tmp/SVM02_npp_d20150311_t1122204_e1123446_b17451_c20150311113206961730_cspp_dev.h5"):
        response = urlopen(
            "https://zenodo.org/record/16355/files/"
            "SVM02_npp_d20150311_t1122204_e1123446_b17451_c20150311113206961730_cspp_dev.h5"
        )
        with open(
            "/tmp/SVM02_npp_d20150311_t1122204_e1123446_b17451_c20150311113206961730_cspp_dev.h5", mode="w"
        ) as fp:
    ...
    if not os.path.exists("/tmp/GMTCO_npp_d20150311_t1122204_e1123446_b17451_c20150311113205873710_cspp_dev.h5"):
        ...
        with open(
            "/tmp/GMTCO_npp_d20150311_t1122204_e1123446_b17451_c20150311113205873710_cspp_dev.h5", mode="w"
        ) as fp:
            fp.write(response.read())
...
def step_impl_user_loads_no_config(context):
    ...
    readers_files = find_files_and_readers(
        sensor="viirs", start_time=datetime(2015, 3, 11, 11, 20), end_time=datetime(2015, 3, 11, 11, 26)
    )
...
def step_impl_user_checks_availability(context):
    ...
    os.chdir("/tmp/")
    reader_files = find_files_and_readers(
        sensor="viirs", start_time=datetime(2015, 3, 11, 11, 20), end_time=datetime(2015, 3, 11, 11, 26)
    )
...
@then("available datasets are returned")
...
def step_impl_datasets_with_same_name(context):
    ...
    scn[make_dataid(name="ds1", resolution=500, calibration="radiance", modifiers=("mod1",))] = DataArray(
        [[5, 6], [7, 8]]
    )
