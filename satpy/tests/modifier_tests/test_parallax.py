...
def _get_fake_areas(center, sizes, resolution, code=4326):
    ...
    return [
        create_area_def(
            "fribullus_xax", code, units="degrees", resolution=resolution, center=center, shape=(size, size)
        )
        for size in sizes
    ]
...
class TestForwardParallax:
    ...
    def test_get_parallax_corrected_lonlats_ssp(self):
        ...
        sat_lat = sat_lon = lon = lat = 0.0
        height = 5000.0  # m
        sat_alt = 30_000_000.0  # m
    ...
    def test_get_parallax_corrected_lonlats_clearsky(self):
        ...
        sat_alt = 35_000_000.0  # m above surface
        (corr_lon, corr_lat) = get_parallax_corrected_lonlats(sat_lon, sat_lat, sat_alt, lon, lat, height)
    ...
    def test_get_parallax_corrected_lonlats_cloudy_ssp(self, lat, lon, resolution):
        ...
        sat_alt = 35_000_000.0  # satellite at 35 Mm
        (corr_lon, corr_lat) = get_parallax_corrected_lonlats(lon, lat, sat_alt, lons, lats, height)
    ...
    def test_get_parallax_corrected_lonlats_cloudy_slant(self):
        ...
        np.testing.assert_allclose(corr_lat[4, 4], 19.955, rtol=5e-4)
        np.testing.assert_allclose(corr_lon[4, 4], 19.960, rtol=5e-4)
    ...
    def test_get_parallax_corrected_lonlats_mixed(self):
        ...
        alt = da.array(
            [
                [np.nan, np.nan, 5000.0, 6000.0, np.nan],
                [np.nan, 6000.0, 7000.0, 7000.0, 7000.0],
                [np.nan, 7000.0, 8000.0, 9000.0, np.nan],
                [np.nan, 7000.0, 7000.0, 7000.0, np.nan],
                [np.nan, 4000.0, 3000.0, np.nan, np.nan],
            ]
        )
        (corrected_lon, corrected_lat) = get_parallax_corrected_lonlats(sat_lon, sat_lat, sat_alt, lon, lat, alt)
    ...
    def test_get_surface_parallax_displacement(self):
        ...
        val = get_surface_parallax_displacement(0, 0, 36_000_000, 0, 10, 10_000)
...
class TestParallaxCorrectionClass:
    ...
    @pytest.mark.parametrize("sat_pos,ar_pos", [((0, 0), (0, 0)), ((0, 0), (40, 0))])
    @pytest.mark.parametrize("resolution", [0.01, 0.5, 10])
    def test_correct_area_clearsky(self, sat_pos, ar_pos, resolution, caplog):
        ...
        small = 5
        large = 9
        (fake_area_small, fake_area_large) = _get_fake_areas((ar_lon, ar_lat), [small, large], resolution)
        ...
        np.testing.assert_allclose(new_area.get_lonlats(), fake_area_small.get_lonlats())

    @pytest.mark.parametrize("lat,lon", [(0, 0), (0, 40), (0, 180), (90, 0)])  # relevant for Арктика satellites
    @pytest.mark.parametrize("resolution", [0.01, 0.5, 10])
    def test_correct_area_ssp(self, lat, lon, resolution):
        ...
        codes = {(0, 0): 4326, (0, 40): 4326, (0, 180): 3575, (90, 0): 3575}
        small = 5
        large = 9
        (fake_area_small, fake_area_large) = _get_fake_areas(
            (lon, lat), [small, large], resolution, code=codes[(lat, lon)]
        )
        corrector = ParallaxCorrection(fake_area_small)
        ...
        if lat != 90:  # don't check SSP longitude if lat=90
            np.testing.assert_allclose(old_lonlats[0][2, 2], new_lonlats[0][2, 2], atol=1e-9)
            np.testing.assert_allclose(old_lonlats[0][2, 2], lon, atol=1e-9)
        np.testing.assert_allclose(old_lonlats[1][2, 2], new_lonlats[1][2, 2], atol=1e-9)
        np.testing.assert_allclose(old_lonlats[1][2, 2], lat, atol=1e-9)
    ...
    def test_correct_area_partlycloudy(self, daskify):
        ...
        small = 5
        large = 9
        (fake_area_small, fake_area_large) = _get_fake_areas((0, 50), [small, large], 0.1)
        ...
        sc = make_fake_scene(
            {
                "CTH": np.array(
                    [
                        [np.nan, np.nan, 5000.0, 6000.0, 7000.0, 6000.0, 5000.0, np.nan, np.nan],
                        [np.nan, 6000.0, 7000.0, 7000.0, 7000.0, np.nan, np.nan, np.nan, np.nan],
                        [np.nan, 7000.0, 8000.0, 9000.0, np.nan, np.nan, np.nan, np.nan, np.nan],
                        [np.nan, 7000.0, 7000.0, 7000.0, np.nan, np.nan, np.nan, np.nan, np.nan],
                        [np.nan, 4000.0, 3000.0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan],
                        [np.nan, np.nan, 5000.0, 8000.0, 8000.0, 8000.0, 6000.0, np.nan, np.nan],
                        [np.nan, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, np.nan],
                        [np.nan, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, np.nan],
                        [np.nan, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, 9000.0, np.nan],
                    ]
                )
        ...
        np.testing.assert_allclose(
            ...
            np.array(
                [
                    ...
                    [-0.20048537, -0.10038778, 0.0, 0.10038778, 0.20058219],
                ]
            ),
            rtol=1e-5,
        )
    ...
    def test_correct_area_clearsky_different_resolutions(self, res1, res2):
        ...
        area1 = create_area_def("fribullus_xax", 4326, units="degrees", resolution=res1, area_extent=[-1, -1, 1, 1])

        area2 = create_area_def("fribullus_xax", 4326, units="degrees", resolution=res2, area_extent=[-1, -1, 1, 1])
        ...
        new_area = corrector(sc["CTH_clear"])
        np.testing.assert_allclose(new_area.get_lonlats(), area2.get_lonlats())
    ...
    def test_correct_area_cloudy_same_area(
        ...
        sc = make_fake_scene(
            {"CTH_constant": np.full((9, 9), 10000)}, daskify=False, area=area, common_attrs=_get_attrs(0, 0, 35_000)
        )
    ...
    def test_correct_area_no_orbital_parameters(self, caplog, fake_tle):
        ...
        small = 5
        large = 9
        (fake_area_small, fake_area_large) = _get_fake_areas((0, 0), [small, large], 0.05)
        corrector = ParallaxCorrection(fake_area_small)
...
class TestParallaxCorrectionModifier:
    ...
    def test_parallax_modifier_interface(self):
        ...
        cth_clear = xr.DataArray(
            np.full((9, 9), np.nan), dims=("y", "x"), attrs={"area": area_large, **_get_attrs(0, 0, 35_000)}
        )
    ...
    def test_parallax_modifier_interface_with_cloud(self):
        ...
        fake_cth_data = np.where(np.isfinite(lons_cth) & np.isfinite(lats_cth), 15000, np.nan)
        ...
        fake_bt = xr.DataArray(fake_bt_data, dims=("y", "x"), attrs={**attrs, "area": fake_area_bt})
        fake_cth = xr.DataArray(fake_cth_data, dims=("y", "x"), attrs={**attrs, "area": fake_area_cth})
    ...
    def test_area(self, request):
        ...
        extents = {
            ...
            "ouagadougou": [-232482.90622750926, 1328206.360136668, -114074.70310250926, 1422810.852324168],
        }
        where = request.param
    ...
    def _get_fake_cloud_datasets(self, test_area, cth, use_dask):
        ...
        fake_bt_data = np.linspace(270, 330, math.prod(test_area.shape), dtype="f8").reshape(test_area.shape).round(2)
        fake_cth_data = np.full(test_area.shape, np.nan, dtype="f8")
        fake_bt_data[lat_min_i:lat_max_i, lon_min_i:lon_max_i] = (
            np.linspace(180, 220, w_cloud * h_cloud).reshape(h_cloud, w_cloud).round(2)
        )
        ...
        fake_bt = xr.DataArray(fake_bt_data, dims=("y", "x"), attrs={**attrs, "area": test_area})

        fake_cth = xr.DataArray(fake_cth_data, dims=("y", "x"), attrs={**attrs, "area": test_area})
    ...
    def test_modifier_interface_cloud_moves_to_observer(self, cth, use_dask, test_area):
        ...
        cloud_location = {
            "foroyar": {7500: (197, 202, 152, 172), 15000: (239, 244, 165, 184)},
            "ouagadougou": {7500: (159, 164, 140, 160), 15000: (163, 168, 141, 161)},
        }
        ...
        # cloud may shrink.
        assert (res.attrs["area"].get_lonlats()[1][dest_mask]).mean() < fake_bt.attrs["area"].get_lonlats()[1][
            cma
        ].mean()
...
class TestParallaxCorrectionSceneLoad:
    ...
    def fake_scene(self, yaml_code):
        ...
        sc["ctth_alti"] = xr.DataArray(
            ...
            dims=("y", "x"),
            attrs={
                "_satpy_id": make_dataid(name="ctth_alti", resolution=3000, modifiers=()),
                "modifiers": (),
                "sensor": {"seviri"},
    ...
    def test_double_load(self, fake_scene, conf_file, fake_tle):
        ...
        """
        with (
            unittest.mock.patch("satpy.composites.config_loader.config_search_paths") as sccc,
