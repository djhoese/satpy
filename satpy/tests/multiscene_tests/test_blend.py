...
def cloud_type_data_array1(test_area, data_type, image_mode):
    ...
    dsid1 = make_dataid(name="geo-ct", resolution=3000, modifiers=())
...
def cloud_type_data_array2(test_area, data_type, image_mode):
    ...
    dsid1 = make_dataid(name="polar-ct", resolution=1000, modifiers=())
    shape = DEFAULT_SHAPE if len(image_mode) == 0 else (len(image_mode),) + DEFAULT_SHAPE
    dims = ("y", "x") if len(image_mode) == 0 else ("bands", "y", "x")
    if data_type is np.int8:
...
def scene1_with_weights(cloud_type_data_array1, test_area):
    ...
    dsid2 = make_dataid(name="geo-cma", resolution=3000, modifiers=())
...
def scene2_with_weights(cloud_type_data_array2, test_area):
    ...
    dsid2 = make_dataid(name="polar-cma", resolution=1000, modifiers=())
...
def groups():
    ...
    return {DataQuery(name="CloudType"): ["geo-ct", "polar-ct"], DataQuery(name="CloudMask"): ["geo-cma", "polar-cma"]}


class TestBlendFuncs:
    """Test individual functions used for blending."""

    def test_blend_two_scenes_using_stack(
        self, multi_scene_and_weights, groups, scene1_with_weights, scene2_with_weights
    ):
    ...
    def test_blend_two_scenes_using_stack_weighted(
        self,
        multi_scene_and_weights,
        groups,
        scene1_with_weights,
        scene2_with_weights,
        combine_times,
