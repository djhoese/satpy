...
class TestMultiSceneSave(unittest.TestCase):
    ...
    def test_save_mp4_distributed(self):
        ...
        fn = os.path.join(self.base_dir, "test_save_mp4_{name}_{start_time:%Y%m%d_%H}_{end_time:%Y%m%d_%H}.mp4")
        ...
        client_mock.gather.side_effect = lambda x: x
        with (
            mock.patch("satpy.multiscene._multiscene.imageio.get_writer") as get_writer,
    ...
    def test_save_datasets_simple(self):
        ...
        with mock.patch("satpy.multiscene._multiscene.Scene.save_datasets") as save_datasets:
            ...
            # force order of datasets by specifying them
            mscn.save_datasets(
                base_dir=self.base_dir, client=False, datasets=["ds1", "ds2", "ds3"], writer="simple_image"
            )
    ...
    def test_save_datasets_distributed_delayed(self):
        ...
        with mock.patch("satpy.multiscene._multiscene.Scene.save_datasets") as save_datasets:
            ...
            # force order of datasets by specifying them
            mscn.save_datasets(
                base_dir=self.base_dir, client=client_mock, datasets=["ds1", "ds2", "ds3"], writer="simple_image"
            )
    ...
    def test_save_datasets_distributed_source_target(self):
        ...
        with mock.patch("satpy.multiscene._multiscene.Scene.save_datasets") as save_datasets:
            ...
            with self.assertRaises(NotImplementedError):
                mscn.save_datasets(
                    base_dir=self.base_dir, client=client_mock, datasets=["ds1", "ds2", "ds3"], writer="geotiff"
                )
    ...
    def test_crop(self):
        ...
        scene1["3"] = DataArray(np.zeros((y_size, x_size)), dims=("y", "x"), attrs={"area": area_def})
        scene1["4"] = DataArray(np.zeros((y_size // 2, x_size // 2)), dims=("y", "x"), attrs={"area": area_def2})
        mscn = MultiScene([scene1])

        # by lon/lat bbox
        new_mscn = mscn.crop(ll_bbox=(-20.0, -5.0, 0, 0))
...
def test_save_mp4(smg, tmp_path):
    ...
    with mock.patch("satpy.multiscene._multiscene.imageio.get_writer") as get_writer:
        ...
        mscn.save_animation(
            ...
            enh_args={
                "decorate": {
                    "decorate": [{"text": {"txt": "Test {start_time:%Y-%m-%d %H:%M} - {end_time:%Y-%m-%d %H:%M}"}}]
                }
            },
        )
    assert writer_mock.append_data.call_count == 2 + 2
