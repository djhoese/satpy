...
class TestModisL2:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [lazy_fixture("modis_l2_nasa_mod35_file"), True, False, False, 1000],
        ],
    )
    def test_load_longitude_latitude(self, input_files, has_5km, has_500, has_250, default_res):
        ...
        with dask.config.set(scheduler=CustomScheduler(max_computes=1 + has_5km + has_500 + has_250)):
            _load_and_check_geolocation(
                scene, "*", default_res, default_shape, True, check_callback=_check_shared_metadata
            )
            _load_and_check_geolocation(scene, 5000, 5000, shape_5km, has_5km, check_callback=_check_shared_metadata)
            _load_and_check_geolocation(scene, 500, 500, shape_500m, has_500, check_callback=_check_shared_metadata)
            _load_and_check_geolocation(scene, 250, 250, shape_250m, has_250, check_callback=_check_shared_metadata)
    ...
    @pytest.mark.parametrize(
        ...
        [
            [lazy_fixture("modis_l2_nasa_mod35_mod03_files"), ["cloud_mask"], 1000, 1000, True],
            [
