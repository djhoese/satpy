...
def _create_fake_rad_dataarray(rad=None):
    ...
    if rad is None:
        rad_data = (np.arange(10.0).reshape((2, 5)) + 1.0) * 50.0
...
def _create_fake_rad_dataset(rad=None):
    ...
    x__ = xr.DataArray(range(5), attrs={"scale_factor": 2.0, "add_offset": -1.0}, dims=("x",))
    y__ = xr.DataArray(range(2), attrs={"scale_factor": -2.0, "add_offset": 1.0}, dims=("y",))
    proj = xr.DataArray(
        [],
        attrs={
            "semi_major_axis": 1.0,
...
class TestABIYAML:
    ...
    @pytest.mark.parametrize(
        ["channel", "suffix"],
        [("C{:02d}".format(num), suffix) for num in range(1, 17) for suffix in ("", "_test_suffix")],
    )
    def test_file_patterns_match(self, channel, suffix):
        ...
        fn1 = ("OR_ABI-L1b-RadM1-M3{}_G16_s20182541300210_e20182541300267_c20182541300308{}.nc").format(channel, suffix)
        loadables = reader.select_files_from_pathnames([fn1])
        assert len(loadables) == 1
        if not suffix and channel in ["C01", "C02", "C03", "C05"]:
            fn2 = ("OR_ABI-L1b-RadM1-M3{}_G16_s20182541300210_e20182541300267_c20182541300308-000000_0.nc").format(
                channel
            )
...
class Test_NC_ABI_L1B(Test_NC_ABI_L1B_Base):
    ...
    def test_basic_attributes(self):
        ...
        self.assertEqual(self.reader.start_time, datetime(2017, 9, 20, 17, 30, 40, 800000))
        self.assertEqual(self.reader.end_time, datetime(2017, 9, 20, 17, 41, 17, 500000))
    ...
    def test_get_area_def(self, adef):
        ...
        self.assertDictEqual(
            call_args[3], {"a": 1.0, "b": 1.0, "h": 1.0, "lon_0": -90.0, "proj": "geos", "sweep": "x", "units": "m"}
        )
...
class Test_NC_ABI_L1B_ir_cal(Test_NC_ABI_L1B_Base):
    ...
    def test_ir_calibration_attrs(self):
        ...
        res = self.reader.get_dataset(make_dataid(name="C05", calibration="brightness_temperature"), {})
    ...
    def test_ir_calibrate(self):
        ...
        expected = np.array(
            [
                [267.55572248, 305.15576503, 332.37383249, 354.73895301, 374.19710115],
...
class Test_NC_ABI_L1B_clipped_ir_cal(Test_NC_ABI_L1B_Base):
    ...
    def test_ir_calibrate(self):
        """Test IR calibration."""
        res = self.reader.get_dataset(make_dataid(name="C07", calibration="brightness_temperature"), {})

        clipped_ir = 267.07775531
        expected = np.array(
            [
                [clipped_ir, 305.15576503, 332.37383249, 354.73895301, 374.19710115],
                [391.68679226, 407.74064808, 422.69329105, 436.77021913, np.nan],
            ]
        )
        assert np.allclose(res.data, expected, equal_nan=True)
...
class Test_NC_ABI_L1B_vis_cal(Test_NC_ABI_L1B_Base):
    ...
    def test_vis_calibrate(self):
        ...
        res = self.reader.get_dataset(make_dataid(name="C05", calibration="reflectance"), {})
        ...
        self.assertEqual(res.attrs["long_name"], "Bidirectional Reflectance")
...
class Test_NC_ABI_L1B_raw_cal(Test_NC_ABI_L1B_Base):
    ...
    def test_raw_calibrate(self):
        ...
        res = self.reader.get_dataset(make_dataid(name="C05", calibration="counts"), {})
        ...
        self.assertEqual(res.attrs["standard_name"], "counts")
        self.assertEqual(res.attrs["long_name"], "Raw Counts")
