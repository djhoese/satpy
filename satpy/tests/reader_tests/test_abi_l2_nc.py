...
def _create_cmip_dataset(data_variable: str = "HT"):
    ...
    ht_da = xr.DataArray(
        np.array([2, -1, -32768, 32767]).astype(np.int16).reshape((2, 2)),
        dims=("y", "x"),
        attrs={
            "scale_factor": 0.3052037,
            "add_offset": 0.0,
...
class Test_NC_ABI_L2_get_dataset:
    ...
    def test_get_dataset(self):
        ...
        exp_data = np.array([[2 * 0.3052037, np.nan], [32768 * 0.3052037, 32767 * 0.3052037]])
...
class Test_NC_ABI_L2_area_fixedgrid:
    ...
    def test_get_area_def_fixedgrid(self, adef):
        ...
        np.testing.assert_allclose(call_args[6], (-2.0, -2.0, 2.0, 2.0))
