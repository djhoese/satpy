...
class TestHRITJMAFileHandler(unittest.TestCase):
    ...
    def _get_acq_time(self, nlines):
        ...
        acq_time_s = ["LINE:={}\rTIME:={:.6f}\r".format(line, time) for line, time in zip(lines_sparse, times_sparse)]
        acq_time_b = "".join(acq_time_s).encode()
        return acq_time_b

    def _get_mda(
        self, loff=5500.0, coff=5500.0, nlines=11000, ncols=11000, segno=0, numseg=1, vis=True, platform="Himawari-8"
    ):
        """Create metadata dict like HRITFileHandler would do it."""
        if vis:
            idf = b"$HALFTONE:=16\r_NAME:=VISIBLE\r_UNIT:=ALBEDO(%)\r0:=-0.10\r1023:=100.00\r65535:=100.00\r"
        else:
            idf = b"$HALFTONE:=16\r_NAME:=INFRARED\r_UNIT:=KELVIN\r0:=329.98\r1023:=130.02\r65535:=130.02\r"
        proj_h8 = b"GEOS(140.70)                    "
        proj_mtsat2 = b"GEOS(145.00)                    "
        proj_name = proj_h8 if platform == "Himawari-8" else proj_mtsat2
        return {
            "image_segm_seq_no": segno,
    ...
    def test_init(self):
        ...
        cal_expected = np.array([[0, -0.1], [1023, 100], [65535, 100]])
        ...
        # Check area IDs
        expected = [({"area": 1}, 1), ({"area": 1234}, UNKNOWN_AREA), ({}, UNKNOWN_AREA)]
        mda = self._get_mda()
    ...
    def test_get_area_def(self):
        ...
        cases = [
            # Non-segmented, full disk
            {
                ...
                "numseg": 1,
                "area": FULL_DISK,
                "extent": (-5498000.088960204, -5498000.088960204, 5502000.089024927, 5502000.089024927),
            },
            # Non-segmented, northern hemisphere
            {
                ...
                "numseg": 1,
                "area": NORTH_HEMIS,
                "extent": (-5498000.088960204, -198000.00320373234, 5502000.089024927, 5302000.085788833),
            },
            # Non-segmented, southern hemisphere
            {
                ...
                "numseg": 1,
                "area": SOUTH_HEMIS,
                "extent": (-5498000.088960204, -5298000.085724112, 5502000.089024927, 202000.0032684542),
            },
            # Segmented, segment #1
            {
                "loff": 1375.0,
                ...
                "numseg": 10,
                "area": FULL_DISK,
                "extent": (-5498000.088960204, 4402000.071226413, 5502000.089024927, 5502000.089024927),
            },
            # Segmented, segment #7
            {
                "loff": 1375.0,
                ...
                "numseg": 10,
                "area": FULL_DISK,
                "extent": (-5498000.088960204, -2198000.035564665, 5502000.089024927, -1098000.0177661523),
            },
        ]
        for case in cases:
            mda = self._get_mda(
            ...
            reader = self._get_reader(mda=mda, filename_info={"area": case["area"]})
    ...
    def test_calibrate(self):
        ...
        refl = np.array(
            [
                ...
                [97.74946237, 100.0, 100.0, 100.0, np.nan],
            ]
        )
        bt = np.array(
            [
        ...
        # of space pixels
        mda = self._get_mda(nlines=5, ncols=5, loff=1375.0, coff=1375.0, segno=0)
        reader = self._get_reader(mda=mda)
        ...
        mda_bt = self._get_mda(nlines=5, ncols=5, loff=1375.0, coff=1375.0, segno=0, vis=False)
        reader_bt = self._get_reader(mda=mda_bt)
        res = reader_bt.calibrate(data=counts, calibration="brightness_temperature")
        np.testing.assert_allclose(bt, res.values)  # also compares NaN

    def test_mask_space(self):
        """Test masking of space pixels."""
        mda = self._get_mda(loff=1375.0, coff=1375.0, nlines=275, ncols=1375, segno=1, numseg=10)
        reader = self._get_reader(mda=mda)
    ...
    def test_get_dataset(self, base_get_dataset):
        ...
        base_get_dataset.return_value = DataArray(da.ones((275, 1375), chunks=1024), dims=("y", "x"))
        ...
        self.assertDictEqual(
            ...
            {"projection_longitude": 140.7, "projection_latitude": 0.0, "projection_altitude": 35785831.0},
        )
    ...
    def test_mjd2datetime64(self):
        ...
        self.assertEqual(mjd2datetime64(np.array([0])), np.datetime64("1858-11-17", "us"))
        self.assertEqual(mjd2datetime64(np.array([40587.5])), np.datetime64("1970-01-01 12:00", "us"))
    ...
    def test_get_acq_time(self):
        ...
        for platform in ["Himawari-8", "MTSAT-2"]:
            ...
            reader = self._get_reader(mda=mda)
            np.testing.assert_allclose(reader.acq_time.astype(np.int64), acq_time_exp.astype(np.int64), atol=45000)
    ...
    def test_start_time_from_filename(self):
        ...
        for platform in ["Himawari-8", "MTSAT-2"]:
            mda = self._get_mda(platform=platform)
            reader = self._get_reader(mda=mda, filename_info={"start_time": start_time})
