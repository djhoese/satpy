...
class TestAHIHSDNavigation(unittest.TestCase):
    ...
    def test_region(self, fromfile, np2str):
        ...
        with mock.patch("satpy.readers.ahi_hsd.open", m, create=True):
            ...
            np.testing.assert_allclose(
                area_def.area_extent, (592000.0038256242, 4132000.0267018233, 1592000.0102878273, 5132000.033164027)
            )
    ...
    def test_segment(self, fromfile, np2str):
        ...
        with mock.patch("satpy.readers.ahi_hsd.open", m, create=True):
            fh = AHIHSDFileHandler(
                "somefile", {"segment": 8, "total_segments": 10}, filetype_info={"file_type": "hsd_b01"}
            )
            fh.proj_info = {
                "CFAC": 40932549,
                "COFF": 5500.5,
            ...
            self.assertEqual(proj_dict["units"], "m")
            np.testing.assert_allclose(
                area_def.area_extent, (-5500000.035542117, -3300000.021325271, 5500000.035542117, -2200000.0142168473)
            )
...
def hsd_file_jp01(tmp_path):
    ...
    dat_type = np.dtype(
        [
            ...
            ("navigation_corrections", _NAVIGATION_CORRECTION_SUBINFO_TYPE, (number_nav_corrections,)),
            ("block9", _OBSERVATION_TIME_INFO_TYPE),
            ("observation_time_information", _OBSERVATION_LINE_TIME_INFO_TYPE, (number_observation_times,)),
    ...
    dat["block8"]["blocklength"] = (
        _NAVIGATION_CORRECTION_INFO_TYPE.itemsize
        + number_nav_corrections * _NAVIGATION_CORRECTION_SUBINFO_TYPE.itemsize
    )
    dat["block9"]["blocklength"] = (
        _OBSERVATION_TIME_INFO_TYPE.itemsize + number_observation_times * _OBSERVATION_LINE_TIME_INFO_TYPE.itemsize
    )
...
class TestAHIHSDFileHandler:
    ...
    @pytest.mark.parametrize(
        ...
        [(False, (140.65699999999998, 0.0042985719753897015, 35786903.00011936)), (True, (140.657, 0.0, 35786850.0))],
    )
...
class TestAHICalibration(unittest.TestCase):
    ...
    @mock.patch("satpy.readers.ahi_hsd.AHIHSDFileHandler.__init__", return_value=None)
    def setUp(self, *mocks):
        ...
        self.counts = da.array(np.array([[0, 1000], [2000, 5000]], dtype=np.uint16))
        self.fh = fh
    ...
    def test_default_calibrate(self, *mocks):
        ...
        self.setUp()
        # Counts
        self.assertEqual(self.fh.calibrate(data=123, calibration="counts"), 123)

        # Radiance
        rad_exp = np.array([[15.2, 11.5], [7.8, -3.3]])
        ...
        bt = self.fh.calibrate(data=self.counts, calibration="brightness_temperature")
        np.testing.assert_allclose(bt, bt_exp)

        # Reflectance
        refl_exp = np.array([[2.92676, 2.214325], [1.50189, 0.0]])
    ...
    def test_updated_calibrate(self):
        ...
        rad_exp = np.array([[30.4, 23.0], [15.6, -6.6]])
        rad = self.fh.calibrate(data=self.counts, calibration="radiance")
        self.assertTrue(np.allclose(rad, rad_exp))
    ...
    def test_user_calibration(self):
        ...
        self.fh.user_calibration = {"B13": {"slope": 0.95, "offset": -0.1}}
        self.fh.band_name = "B13"
        rad = self.fh.calibrate(data=self.counts, calibration="radiance")
        rad_np = rad.compute()
        assert rad.dtype == rad_np.dtype
        assert rad.dtype == np.float32
        rad_exp = np.array([[16.10526316, 12.21052632], [8.31578947, -3.36842105]])
        self.assertTrue(np.allclose(rad, rad_exp))

        # This is for DN calibration
        self.fh.user_calibration = {"B13": {"slope": -0.0032, "offset": 15.20}, "type": "DN"}
        self.fh.band_name = "B13"
        rad = self.fh.calibrate(data=self.counts, calibration="radiance")
        rad_np = rad.compute()
        assert rad.dtype == rad_np.dtype
        assert rad.dtype == np.float32
        rad_exp = np.array([[15.2, 12.0], [8.8, -0.8]])
        self.assertTrue(np.allclose(rad, rad_exp))
...
def _fake_hsd_handler(fh_kwargs=None):
    ...
    m = mock.mock_open()
    with (
        mock.patch("satpy.readers.ahi_hsd.np.fromfile", _custom_fromfile),
