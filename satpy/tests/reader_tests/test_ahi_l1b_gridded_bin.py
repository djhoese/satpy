...
class TestAHIGriddedArea(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.FULLDISK_SIZES = {
            0.005: {"x_size": 24000, "y_size": 24000},
            0.01: {"x_size": 12000, "y_size": 12000},
            0.02: {"x_size": 6000, "y_size": 6000},
        }

        self.AHI_FULLDISK_EXTENT = [85.0, -60.0, 205.0, 60.0]
    ...
    def make_fh(filetype, area="fld"):
        ...
        with mock.patch("satpy.readers.ahi_l1b_gridded_bin.open", m, create=True):
            fh = AHIGriddedFileHandler("somefile", {"area": area}, filetype_info={"file_type": filetype})
            return fh
...
class TestAHIGriddedFileCalibration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with mock.patch("satpy.readers.ahi_l1b_gridded_bin.open", m, create=True):
            in_fname = "test_file"
            fh = AHIGriddedFileHandler(in_fname, {"area": "fld"}, filetype_info={"file_type": "tir.01"})
            self.fh = fh
    ...
    def test_calibrate(self, np_loadtxt, os_exist, get_luts):
        ...
        load_return = np.squeeze(np.dstack([np.arange(0, 2048, 1), np.arange(0, 120, 0.05859375)]))

        np_loadtxt.return_value = load_return
        get_luts.return_value = True

        in_data = np.array([[100.0, 300.0, 500.0], [800.0, 1500.0, 2040.0]])

        refl_out = np.array([[5.859375, 17.578125, 29.296875], [46.875, 87.890625, 119.53125]])
...
class TestAHIGriddedFileHandler(unittest.TestCase):
    ...
    @mock.patch("satpy.readers.ahi_l1b_gridded_bin.unzip_file", mock.MagicMock(side_effect=new_unzip))
    def setUp(self):
        """Create a test file handler."""
        m = mock.mock_open()
        with mock.patch("satpy.readers.ahi_l1b_gridded_bin.open", m, create=True):
        ...
        key = {"calibration": "counts", "name": "vis.01"}
        info = {"units": "unitless", "standard_name": "vis.01", "wavelength": 10.8, "resolution": 0.05}
        self.key = key
        self.info = info
    ...
    def test_get_dataset(self, mocked_read):
        ...
        out_data = np.array([[100.0, 300.0, 500.0], [800.0, 1500.0, 2040.0]])
...
class TestAHIGriddedLUTs(unittest.TestCase):
    ...
    @mock.patch(
        "satpy.readers.ahi_l1b_gridded_bin.AHIGriddedFileHandler._download_luts",
