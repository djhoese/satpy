...
DEFAULT_FILE_FLAGS = np.arange(DEFAULT_FILE_SHAPE[0] * DEFAULT_FILE_SHAPE[1], dtype=np.byte).reshape(DEFAULT_FILE_SHAPE)
...
def fake_test_content(filename, **kwargs):
    ...
    # category
    variable3 = xr.DataArray(
        DEFAULT_FILE_FLAGS,
        dims=("scan_lines_along_track_direction", "pixel_elements_along_scan_direction"),
        attrs={"SCALED": 0, "_FillValue": -127, "units": "1", "flag_values": [0, 1, 2, 3]},
    )
...
class TestCLAVRXReaderGeo:
    ...
    @pytest.mark.parametrize(("filenames", "expected_loadables"), [([AHI_FILE], 1)])
