...
class Testrecarray2dict(unittest.TestCase):
    ...
    def test_fun(self):
        ...
        expected = {
            "test_sec": {"test_str": np.array([b"Testing"], dtype="<S20"), "test_int": np.array([10], dtype=np.int32)},
...
class TestHRITGOMSProFileHandler(unittest.TestCase):
    ...
    def test_init(self, new_fh_init, fromfile):
        ...
        tss = np.array([(2, 292, 19002, "ELECTRO", 1.3264, 1, 0.0)], dtype=satellite_status)
        tia = np.tile(np.array([(3, 24, 2, 9119019, 0.0)], dtype=image_acquisition), (1, 10))
        ...
        with mock.patch("satpy.readers.electrol_hrit.open", m, create=True) as newopen:
            ...
            self.reader = HRITGOMSPrologueFileHandler(
                "filename",
                {"platform_shortname": "GOMS2", "start_time": test_t, "service": "test_service"},
                {"filetype": "info"},
            )

        # assertDictEqual doesn't seem to work for dicts containing dicts,
        # so we must compare some items individually
        self.assertDictEqual(self.test_pro["SatelliteStatus"], self.reader.prologue["SatelliteStatus"])
        prop = "ImageAcquisition"
        for key in self.reader.prologue[prop]:
            np.testing.assert_array_equal(self.test_pro[prop][key], self.reader.prologue[prop][key])
        np.testing.assert_array_equal(self.test_pro["ImageCalibration"], self.reader.prologue["ImageCalibration"])
...
class TestHRITGOMSEpiFileHandler(unittest.TestCase):
    ...
    def test_init(self, new_fh_init, fromfile):
        ...
        with mock.patch("satpy.readers.electrol_hrit.open", m, create=True) as newopen:
            ...
            np.testing.assert_array_equal(epi["RadiometricProcessing"]["RPSummary"]["IsOptic"], np.ones(10))
            np.testing.assert_array_equal(epi["GeometricProcessing"]["TimeProcessing"], np.ones(10))
...
class TestHRITGOMSFileHandler(unittest.TestCase):
    ...
    def test_calibrate(self, *mocks):
        ...
        self.assertRaises(NotImplementedError, fh.calibrate, counts, "nonsense")
        ...
        self.assertTrue(np.allclose(out.values, lut[0, counts] / 1000.0))
    ...
    def test_get_area_def(self, *mocks):
        ...
        self.assertTrue(np.allclose(np.array(area.area_extent), np.array(example_area_ext)))
