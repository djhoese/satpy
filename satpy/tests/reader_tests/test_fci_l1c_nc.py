...
def _get_test_calib_for_channel_vis(data, meas):
    data["state/celestial/earth_sun_distance"] = FakeH5Variable(da.repeat(da.array([149597870.7]), 6000), dims="x")
...
def _get_test_image_data_for_channel(data, ch_str, n_rows_cols):
    ...
    if "38" in ch_path:
        ...
        d = FakeH5Variable(
            ...
            dims=("y", "x"),
            attrs={"valid_range": [0, 8191], "warm_scale_factor": 2, "warm_add_offset": -300, **common_attrs},
        )
    else:
        d = FakeH5Variable(
            da.ones(n_rows_cols, dtype="uint16", chunks=1024),
            dims=("y", "x"),
            attrs={"valid_range": [0, 4095], "warm_scale_factor": 1, "warm_add_offset": 0, **common_attrs},
        )
...
class FakeFCIFileHandlerFDHSI(FakeFCIFileHandlerBase):
    ...
    chan_patterns = {
        "vis_{:>02d}": {"channels": [4, 5, 6, 8, 9], "grid_type": "1km"},
        "nir_{:>02d}": {"channels": [13, 16, 22], "grid_type": "1km"},
        "ir_{:>02d}": {"channels": [38, 87, 97, 105, 123, 133], "grid_type": "2km"},
        "wv_{:>02d}": {"channels": [63, 73], "grid_type": "2km"},
    }
...
class FakeFCIFileHandlerWithBadData(FakeFCIFileHandlerFDHSI):
    ...
    def _get_test_content_all_channels(self):
        ...
        data.update(
            {
                "data/ir_105/measured/radiance_to_bt_conversion_coefficient_wavenumber": v,
...
class FakeFCIFileHandlerWithBadIDPFData(FakeFCIFileHandlerFDHSI):
    ...
    def _get_test_content_all_channels(self):
        ...
        data["data/vis_06/measured/x"].attrs["scale_factor"] = np.float32(
            data["data/vis_06/measured/x"].attrs["scale_factor"]
        )
        data["data/vis_06/measured/x"].attrs["add_offset"] = np.float32(
            data["data/vis_06/measured/x"].attrs["add_offset"]
        )
        data["data/vis_06/measured/y"].attrs["scale_factor"] = np.float32(
            data["data/vis_06/measured/y"].attrs["scale_factor"]
        )
        data["data/vis_06/measured/y"].attrs["add_offset"] = np.float32(
...
class FakeFCIFileHandlerHRFI(FakeFCIFileHandlerBase):
    ...
    chan_patterns = {
        "vis_{:>02d}_hr": {"channels": [6], "grid_type": "500m"},
        "nir_{:>02d}_hr": {"channels": [22], "grid_type": "500m"},
        "ir_{:>02d}_hr": {"channels": [38, 105], "grid_type": "1km"},
    }
...
def reader_configs():
    ...
    return config_search_paths(os.path.join("readers", "fci_l1c_nc.yaml"))
...
_chans_fdhsi = {
    "solar": ["vis_04", "vis_05", "vis_06", "vis_08", "vis_09", "nir_13", "nir_16", "nir_22"],
    "solar_grid_type": ["1km"] * 8,
    "terran": ["ir_38", "wv_63", "wv_73", "ir_87", "ir_97", "ir_105", "ir_123", "ir_133"],
...
def FakeFCIFileHandlerFDHSI_fixture():
    ...
    with mocked_basefilehandler(FakeFCIFileHandlerFDHSI):
        param_dict = {"filetype": "fci_l1c_fdhsi", "channels": _chans_fdhsi, "filenames": _test_filenames["fdhsi"]}
        yield param_dict
...
def FakeFCIFileHandlerHRFI_fixture():
    ...
    with mocked_basefilehandler(FakeFCIFileHandlerHRFI):
        param_dict = {"filetype": "fci_l1c_hrfi", "channels": _chans_hrfi, "filenames": _test_filenames["hrfi"]}
        yield param_dict
...
class TestFCIL1cNCReader:
    ...
    fh_param_for_filetype = {
        "hrfi": {"channels": _chans_hrfi, "filenames": _test_filenames["hrfi"]},
        "fdhsi": {"channels": _chans_fdhsi, "filenames": _test_filenames["fdhsi"]},
    }
    ...
    @pytest.mark.parametrize(
        "filenames",
        [_test_filenames["fdhsi"][0].replace("BODY", "TRAIL"), _test_filenames["hrfi"][0].replace("BODY", "TRAIL")],
    )
    ...
    @pytest.mark.parametrize(
        "fh_param,expected_res_n",
        [(lazy_fixture("FakeFCIFileHandlerFDHSI_fixture"), 16), (lazy_fixture("FakeFCIFileHandlerHRFI_fixture"), 4)],
    )
    def test_load_counts(self, reader_configs, fh_param, expected_res_n):
        ...
        res = reader.load(
            [
                ...
                for name in fh_param["channels"]["solar"] + fh_param["channels"]["terran"]
            ],
            pad_data=False,
        )
        assert expected_res_n == len(res)
        for ch, grid_type in zip(
            fh_param["channels"]["solar"] + fh_param["channels"]["terran"],
            fh_param["channels"]["solar_grid_type"] + fh_param["channels"]["terran_grid_type"],
        ):
            assert res[ch].shape == (
                GRID_TYPE_INFO_FOR_TEST_CONTENT[grid_type]["nrows"],
    ...
    def test_load_radiance(self, reader_configs, fh_param, expected_res_n):
    ...
    @pytest.mark.parametrize(
        "fh_param,expected_res_n",
        [(lazy_fixture("FakeFCIFileHandlerFDHSI_fixture"), 8), (lazy_fixture("FakeFCIFileHandlerHRFI_fixture"), 2)],
    )
    def test_load_reflectance(self, reader_configs, fh_param, expected_res_n):
        """Test loading with reflectance."""
        reader = _get_reader_with_filehandlers(fh_param["filenames"], reader_configs)
        res = reader.load(
            [make_dataid(name=name, calibration="reflectance") for name in fh_param["channels"]["solar"]],
            pad_data=False,
        )
        assert expected_res_n == len(res)
    ...
    def test_load_bt(self, reader_configs, caplog, fh_param, expected_res_n):
        ...
        with caplog.at_level(logging.WARNING):
            res = reader.load(
                [
                    ...
                    for name in fh_param["channels"]["terran"]
                ],
                pad_data=False,
            )
            assert caplog.text == ""
    ...
    @pytest.mark.parametrize(
        "fh_param",
        [(lazy_fixture("FakeFCIFileHandlerFDHSI_fixture")), (lazy_fixture("FakeFCIFileHandlerHRFI_fixture"))],
    )
    def test_orbital_parameters_attr(self, reader_configs, fh_param):
        """Test the orbital parameter attribute."""
        reader = _get_reader_with_filehandlers(fh_param["filenames"], reader_configs)
        res = reader.load(
            [make_dataid(name=name) for name in fh_param["channels"]["solar"] + fh_param["channels"]["terran"]],
            pad_data=False,
        )
    ...
    expected_pos_info_for_filetype = {
        "fdhsi": {
            "1km": {"start_position_row": 1, "end_position_row": 200, "segment_height": 200, "grid_width": 11136},
            "2km": {"start_position_row": 1, "end_position_row": 100, "segment_height": 100, "grid_width": 5568},
        },
        "hrfi": {
            "500m": {"start_position_row": 1, "end_position_row": 400, "segment_height": 400, "grid_width": 22272},
            "1km": {"start_position_row": 1, "end_position_row": 200, "grid_width": 11136, "segment_height": 200},
        },
    }

    @pytest.mark.parametrize(
        "fh_param, expected_pos_info",
        [
    ...
    def test_load_index_map(self, reader_configs, fh_param, expected_res_n):
        ...
        reader = _get_reader_with_filehandlers(fh_param["filenames"], reader_configs)
        res = reader.load(
            [name + "_index_map" for name in fh_param["channels"]["solar"] + fh_param["channels"]["terran"]],
            pad_data=False,
        )
        assert expected_res_n == len(res)
        for ch, grid_type in zip(
            ...
            assert res[ch + "_index_map"].shape == (
                GRID_TYPE_INFO_FOR_TEST_CONTENT[grid_type]["nrows"],
                GRID_TYPE_INFO_FOR_TEST_CONTENT[grid_type]["ncols"],
            )
    ...
    def test_load_quality_only(self, reader_configs, fh_param, expected_res_n):
        ...
        reader = _get_reader_with_filehandlers(fh_param["filenames"], reader_configs)
        res = reader.load(
            [name + "_pixel_quality" for name in fh_param["channels"]["solar"] + fh_param["channels"]["terran"]],
            pad_data=False,
        )
        assert expected_res_n == len(res)
        for ch, grid_type in zip(
            ...
            assert res[ch + "_pixel_quality"].shape == (
                GRID_TYPE_INFO_FOR_TEST_CONTENT[grid_type]["nrows"],
                GRID_TYPE_INFO_FOR_TEST_CONTENT[grid_type]["ncols"],
            )
    ...
    def test_area_definition_computation(self, reader_configs, fh_param, expected_area):
        ...
        np.testing.assert_array_almost_equal(
            ...
            np.array([-5567999.994203, -5367999.994411, 5567999.994203, -5567999.994203]),
            decimal=2,
        )
    ...
    def test_excs(self, reader_configs, fh_param):
        ...
        with pytest.raises(ValueError):
            reader.file_handlers[fh_param["filetype"]][0].get_dataset(
                make_dataid(name="ir_123", calibration="unknown"), {"units": "unknown"}
            )
...
class TestFCIL1cNCReaderBadData:
    ...
    def test_handling_bad_data_ir(self, reader_configs, caplog):
        ...
        with mocked_basefilehandler(FakeFCIFileHandlerWithBadData):
            ...
            with caplog.at_level(logging.ERROR):
                reader.load([make_dataid(name="ir_105", calibration="brightness_temperature")], pad_data=False)
    ...
    def test_handling_bad_data_vis(self, reader_configs, caplog):
        ...
        with mocked_basefilehandler(FakeFCIFileHandlerWithBadData):
            reader = _get_reader_with_filehandlers(_test_filenames["fdhsi"], reader_configs)
            with caplog.at_level(logging.ERROR):
                reader.load([make_dataid(name="vis_06", calibration="reflectance")], pad_data=False)
...
class TestFCIL1cNCReaderBadDataFromIDPF:
    ...
    def test_bad_xy_coords(self, reader_configs):
        ...
        with mocked_basefilehandler(FakeFCIFileHandlerWithBadIDPFData):
            ...
            np.testing.assert_array_almost_equal(
                np.array(area_def.area_extent),
                np.array([-5568000.227139, -5368000.221262, 5568000.100073, -5568000.227139]),
                decimal=2,
            )
