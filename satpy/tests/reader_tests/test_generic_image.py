...
class TestGenericImage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.area_def = AreaDefinition(
            "geotiff_area", pcs_id, pcs_id, proj4_dict, self.x_size, self.y_size, area_extent
        )

        # Create datasets for L, LA, RGB and RGBA mode images
        r__ = da.random.randint(0, 256, size=(self.y_size, self.x_size), chunks=(50, 50)).astype(np.uint8)
        g__ = da.random.randint(0, 256, size=(self.y_size, self.x_size), chunks=(50, 50)).astype(np.uint8)
        b__ = da.random.randint(0, 256, size=(self.y_size, self.x_size), chunks=(50, 50)).astype(np.uint8)
        a__ = 255 * np.ones((self.y_size, self.x_size), dtype=np.uint8)
        a__[:10, :10] = 0
        a__ = da.from_array(a__, chunks=(50, 50))

        r_nan__ = np.random.uniform(0.0, 1.0, size=(self.y_size, self.x_size))
        r_nan__[:10, :10] = np.nan
        r_nan__ = da.from_array(r_nan__, chunks=(50, 50))

        ds_l = xr.DataArray(
            da.stack([r__]), dims=("bands", "y", "x"), attrs={"name": "test_l", "start_time": self.date}
        )
        ds_l["bands"] = ["L"]
        ds_la = xr.DataArray(
            da.stack([r__, a__]), dims=("bands", "y", "x"), attrs={"name": "test_la", "start_time": self.date}
        )
        ds_la["bands"] = ["L", "A"]
        ds_rgb = xr.DataArray(
            da.stack([r__, g__, b__]), dims=("bands", "y", "x"), attrs={"name": "test_rgb", "start_time": self.date}
        )
        ds_rgb["bands"] = ["R", "G", "B"]
        ds_rgba = xr.DataArray(
            da.stack([r__, g__, b__, a__]),
            dims=("bands", "y", "x"),
            attrs={"name": "test_rgba", "start_time": self.date},
        )
        ds_rgba["bands"] = ["R", "G", "B", "A"]

        ds_l_nan = xr.DataArray(
            da.stack([r_nan__]), dims=("bands", "y", "x"), attrs={"name": "test_l_nan", "start_time": self.date}
        )
        ds_l_nan["bands"] = ["L"]
        ...
        scn.save_dataset(
            "l_nan", os.path.join(self.base_dir, "test_l_nan_fillvalue.tif"), writer="geotiff", fill_value=0
        )
        scn.save_dataset("l_nan", os.path.join(self.base_dir, "test_l_nan_nofillvalue.tif"), writer="geotiff")
