...
def gerb_l2_hr_h5_dummy_file(tmp_path_factory):
    ...
    with h5py.File(filename, "w") as fid:
        ...
        write_h5_null_string_att(
            fid["/Geolocation"].id, "Geolocation File Name", "G4_SEV4_L20_HR_GEO_20180111_181500_V010.hdf"
        )
        ...
        write_h5_null_string_att(
            fid["/Scene Identification"].id, "Solar Angular Dependency Models Set Version", "CERES_TRMM.1"
        )
        write_h5_null_string_att(
            fid["/Scene Identification"].id, "Thermal Angular Dependency Models Set Version", "RMIB.3"
        )
        ...
        fid["/Scene Identification/Cloud Optical Depth (logarithm)"] = np.ones(
            shape=(1237, 1237), dtype=np.dtype(">i2")
        )
        fid["/Scene Identification/Cloud Optical Depth (logarithm)"].attrs["Quantisation Factor"] = np.array(
            0.00025, dtype="float64"
        )
        ...
        write_h5_null_string_att(
            fid["/Scene Identification/Cloud Phase"].id, "Unit", "Percent (Water=0%,Mixed,Ice=100%)"
        )
