...
class GOESNCEUMFileHandlerRadianceTest(unittest.TestCase):
    ...
    def setUp(self, xr_):
        ...
        self.reader = GOESEUMNCFileHandler(filename="dummy", filename_info={}, filetype_info={}, geo_data=geo_data)

    def test_get_dataset_radiance(self):
        """Test getting the radiances."""
        for ch in self.channels:
            if not is_vis_channel(ch):
                radiance = self.reader.get_dataset(key=make_dataid(name=ch, calibration="radiance"), info={})
                # ... this only compares the valid (unmasked) elements
                self.assertTrue(
                    np.all(self.radiance == radiance.to_masked_array()),
                    msg="get_dataset() returns invalid radiance for channel {}".format(ch),
                )
            ...
            if not is_vis_channel(ch):
                ...
                for calib, method in calibs.items():
                    with mock.patch.object(self.reader, method) as target_func:
                        self.reader.calibrate(data=self.reader.nc["data"], calibration=calib, channel=ch)
    ...
    def test_get_dataset_reflectance(self):
        ...
        for ch in self.channels:
            if is_vis_channel(ch):
                refl = self.reader.get_dataset(key=make_dataid(name=ch, calibration="reflectance"), info={})
                # ... this only compares the valid (unmasked) elements
                self.assertTrue(
                    np.all(self.reflectance == refl.to_masked_array()),
                    msg="get_dataset() returns invalid reflectance for channel {}".format(ch),
                )
