...
class TestHDF5FileHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Add datasets
        ds1_f = g1.create_dataset(
            "ds1_f", shape=(10, 100), dtype=np.float32, data=np.arange(10.0 * 100).reshape((10, 100))
        )
        ds1_i = g1.create_dataset("ds1_i", shape=(10, 100), dtype=np.int32, data=np.arange(10 * 100).reshape((10, 100)))
        ds2_f = h.create_dataset(
            "ds2_f", shape=(10, 100), dtype=np.float32, data=np.arange(10.0 * 100).reshape((10, 100))
        )
        ds2_i = h.create_dataset("ds2_i", shape=(10, 100), dtype=np.int32, data=np.arange(10 * 100).reshape((10, 100)))
