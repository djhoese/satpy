...
class FakeHDF5FileHandler2(FakeHDF5FileHandler):
    ...
    def _get_geo_data(self, num_rows, num_cols):
        geo = {
            "wvc_lon": xr.DataArray(
                da.ones((num_rows, num_cols), chunks=1024, dtype=np.float32),
                attrs={
                    "fill_value": 1.7e38,
                    "scale_factor": 1.0,
                    "add_offset": 0.0,
    ...
    def _get_selection_data(self, num_rows, num_cols):
        selection = {
            "wvc_selection": xr.DataArray(
                da.ones((num_rows, num_cols), chunks=1024, dtype=np.int8),
                attrs={
                    "fill_value": 0,
                    "scale_factor": 1.0,
                    "add_offset": 0.0,
            ...
            "wind_speed_selection": xr.DataArray(
                da.ones((num_rows, num_cols), chunks=1024, dtype=np.int16),
                attrs={
                    "fill_value": -32767,
                    "scale_factor": 0.1,
                    "add_offset": 0.0,
            ...
            "wind_dir_selection": xr.DataArray(
                da.ones((num_rows, num_cols), chunks=1024, dtype=np.int16),
                attrs={
                    "fill_value": -32767,
                    "scale_factor": 0.01,
            ...
            "wvc_quality_flag": xr.DataArray(
                da.ones((num_rows, num_cols), chunks=1024, dtype=np.uint16),
                attrs={
                    "fill_value": 2.14748e09,
                    "scale_factor": 1.0,
                    "add_offset": 0.0,
                    "units": "na",
                    "valid range": [1, 2.14748e09],
                },
                dims=("y", "x"),
            ),
        }
    ...
    def _get_all_ambiguities_data(self, num_rows, num_cols, num_amb):
        all_amb = {
            "max_likelihood_est": xr.DataArray(
                da.ones((num_rows, num_cols, num_amb), chunks=1024, dtype=np.int16),
                attrs={
                    "fill_value": -32767,
                    "scale_factor": 1.0,
                    "add_offset": 0.0,
    ...
    def _get_global_attrs(self, num_rows, num_cols):
        return {
            ...
            "/attr/L2B_Algorithm_Descriptor": (
                "Wind retrieval processing uses the multiple solution scheme (MSS) for "
...
class TestHY2SCATL2BH5Reader(unittest.TestCase):
    ...
    def test_load_data_selection(self):
        ...
        # Make sure we have some files
        self.assertTrue(reader.file_handlers)
        res = reader.load(["wind_speed_selection", "wind_dir_selection", "wvc_selection"])
