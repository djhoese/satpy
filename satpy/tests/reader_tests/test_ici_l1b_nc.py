...
def reader(fake_file):
    ...
    return IciL1bNCFileHandler(
        filename=fake_file,
        filename_info={
            "sensing_start_time": datetime.fromisoformat("2000-01-01T01:00:00"),
            "sensing_end_time": datetime.fromisoformat("2000-01-01T02:00:00"),
            "creation_time": datetime.fromisoformat("2000-01-01T03:00:00"),
        },
        filetype_info={
...
class IciL1bFakeFileWriter:
    ...
    def _write_quality_group(dataset):
        ...
        duration_of_product = group.createVariable("duration_of_product", "f4")
        duration_of_product[:] = 1000.0
    ...
    def _write_navigation_data_group(dataset):
        ...
        delta_longitude[:] = 1000.0 * np.ones(shape)
        delta_latitude = group.createVariable(
            "delta_latitude",
            np.float32,
            dimensions=dimensions,
        )
        delta_latitude[:] = 1000.0 * np.ones(shape)
    ...
    def _write_measurement_data_group(dataset):
        ...
        samples = group.createVariable("n_samples", "i4", dimensions=("n_samples",))
        samples[:] = np.arange(N_SAMPLES)
        bt_a = group.createVariable("bt_conversion_a", np.float32, dimensions=("n_channels",))
        bt_a[:] = np.ones(N_CHANNELS)
        bt_b = group.createVariable("bt_conversion_b", np.float32, dimensions=("n_channels",))
        bt_b[:] = np.zeros(N_CHANNELS)
        cw = group.createVariable("centre_wavenumber", np.float32, dimensions=("n_channels",))
        cw[:] = np.array([6.0] * 3 + [8.0] * 2 + [11.0] * 3 + [15.0] * 3 + [22.0] * 2)
...
class TestIciL1bNCFileHandler:
    ...
    def test_interpolate_returns_none_if_dataset_not_exist(self, reader):
        ...
        azimuth, zenith = reader._interpolate(InterpolationType.OBSERVATION_ANGLES)
    ...
    def test_interpolate_viewing_angle(self, reader):
        ...
        zenith = xr.DataArray(100.0 * np.ones(shape), dims=dims)
    ...
    def test_orthorectify(self, reader):
        """Test orthorectify."""
        variable = xr.DataArray(
            np.ones((N_SCAN, N_SAMPLES, N_HORNS)), dims=("y", "x", "n_horns"), coords={"n_horns": np.arange(N_HORNS)}
        )
    ...
    def test_get_global_attributes(self, reader):
        ...
        assert attributes == {
            ...
            "platform_name": "SGB",
            "quality_group": {
                "duration_of_product": np.array(1000.0, dtype=np.float32),
