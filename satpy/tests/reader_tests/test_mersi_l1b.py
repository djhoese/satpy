...
def _get_calibration(num_scans):
    calibration = {
        "Calibration/VIS_Cal_Coeff": xr.DataArray(
            da.ones((19, 3), chunks=1024),
            attrs={"Slope": np.array([1.0] * 19), "Intercept": np.array([0.0] * 19)},
            dims=("_bands", "_coeffs"),
        ),
        "Calibration/IR_Cal_Coeff": xr.DataArray(
            da.ones((6, 4, num_scans), chunks=1024),
            attrs={"Slope": np.array([1.0] * 6), "Intercept": np.array([0.0] * 6)},
...
def _get_250m_data(num_scans, rows_per_scan, num_cols):
    ...
    def_attrs = {
        ...
        "Intercept": np.array([0.0] * 1),
    }
    nounits_attrs = {**def_attrs, **{"units": "NO"}}
    radunits_attrs = {**def_attrs, **{"units": "mW/ (m2 cm-1 sr)"}}

    data = {
        "Data/EV_250_RefSB_b1": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=nounits_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_RefSB_b2": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=nounits_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_RefSB_b3": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=nounits_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_RefSB_b4": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=nounits_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_Emissive_b24": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=radunits_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_Emissive_b25": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=radunits_attrs,
            dims=("_rows", "_cols"),
        ),
...
def _get_1km_data(num_scans, rows_per_scan, num_cols):
    data = {
        ...
        "Data/EV_1KM_RefSB": xr.DataArray(
            da.ones((15, num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs={
                "Slope": np.array([1.0] * 15),
                "Intercept": np.array([0.0] * 15),
                "FillValue": 65535,
                "units": "NO",
                "valid_range": [0, 4095],
                "long_name": b"1km Earth View Science Data",
        ...
        "Data/EV_1KM_Emissive": xr.DataArray(
            da.ones((4, num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs={
                "Slope": np.array([1.0] * 4),
                "Intercept": np.array([0.0] * 4),
                "FillValue": 65535,
                "units": "mW/ (m2 cm-1 sr)",
                "valid_range": [0, 25000],
        ...
        "Data/EV_250_Aggr.1KM_RefSB": xr.DataArray(
            da.ones((4, num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs={
                "Slope": np.array([1.0] * 4),
                "Intercept": np.array([0.0] * 4),
        ...
        "Data/EV_250_Aggr.1KM_Emissive": xr.DataArray(
            da.ones((2, num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs={
                "Slope": np.array([1.0] * 2),
                "Intercept": np.array([0.0] * 2),
                "FillValue": 65535,
                "units": "mW/ (m2 cm-1 sr)",
                "valid_range": [0, 4095],
...
def _get_250m_ll_data(num_scans, rows_per_scan, num_cols):
    ...
    data = {
        "Data/EV_250_Emissive_b6": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=def_attrs,
            dims=("_rows", "_cols"),
        ),
        "Data/EV_250_Emissive_b7": xr.DataArray(
            da.ones((num_scans * rows_per_scan, num_cols), chunks=1024, dtype=np.uint16),
            attrs=def_attrs,
            dims=("_rows", "_cols"),
        ),
