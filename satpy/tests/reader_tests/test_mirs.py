...
DEFAULT_LAT = np.linspace(23.09356, 36.42844, N_SCANLINE * N_FOV, dtype=DEFAULT_FILE_DTYPE)
DEFAULT_LON = np.linspace(127.6879, 144.5284, N_SCANLINE * N_FOV, dtype=DEFAULT_FILE_DTYPE)
FREQ = xr.DataArray(
    [
        23.8,
        31.4,
...
POLO = xr.DataArray(
    [2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3][:N_CHANNEL],
    dims="Channel",
    attrs={"description": "Polarizations"},
)

DS_IDS = ["RR", "longitude", "latitude"]
TEST_VARS = ["btemp_88v", "btemp_165h", "btemp_23v", "RR", "Sfc_type"]
DEFAULT_UNITS = {"btemp_88v": "K", "btemp_165h": "K", "btemp_23v": "K", "RR": "mm/hr", "Sfc_type": "1"}
...
def fake_coeff_from_fn(fn):
    ...
    for idx in range(1, N_CHANNEL + 1):
        ...
        for fov in range(1, N_FOV + 1):
            ...
            coeffs_line = " {:>2} {:>2}  {} {}   {}\n".format(idx, fov, str_coeff, str_means, error_val)
...
def _get_datasets_with_attributes(**kwargs):
    ...
    bt = xr.DataArray(
        np.linspace(1830, 3930, N_SCANLINE * N_FOV * N_CHANNEL).reshape(N_SCANLINE, N_FOV, N_CHANNEL),
        attrs={
    ...
    rr = xr.DataArray(
        np.random.randint(100, 500, size=(N_SCANLINE, N_FOV)),
        attrs={
    ...
    sfc_type = xr.DataArray(
        np.random.randint(0, 4, size=(N_SCANLINE, N_FOV)),
        attrs={
    ...
    latitude = xr.DataArray(
        DEFAULT_LAT.reshape(DEFAULT_2D_SHAPE),
        attrs={"long_name": "Latitude of the view (-90,90)"},
        dims=("Scanline", "Field_of_view"),
    )
    longitude = xr.DataArray(
        DEFAULT_LON.reshape(DEFAULT_2D_SHAPE),
        attrs={"long_name": "Longitude of the view (-180,180)"},
        dims=("Scanline", "Field_of_view"),
    )
...
def _get_datasets_with_less_attributes():
    ...
    bt = xr.DataArray(
        np.linspace(1830, 3930, N_SCANLINE * N_FOV * N_CHANNEL).reshape(N_SCANLINE, N_FOV, N_CHANNEL),
        attrs={"long_name": "Channel Temperature (K)", "scale_factor": 0.01},
        dims=("Scanline", "Field_of_view", "Channel"),
    )
    rr = xr.DataArray(
        np.random.randint(100, 500, size=(N_SCANLINE, N_FOV)),
        attrs={"long_name": "Rain Rate (mm/hr)", "scale_factor": 0.1},
        dims=("Scanline", "Field_of_view"),
    )

    sfc_type = xr.DataArray(
        np.random.randint(0, 4, size=(N_SCANLINE, N_FOV)),
        attrs={"description": "type of surface:0-ocean," + "1-sea ice,2-land,3-snow"},
        dims=("Scanline", "Field_of_view"),
    )
    latitude = xr.DataArray(
        DEFAULT_LAT.reshape(DEFAULT_2D_SHAPE),
...
class TestMirsL2_NcReader:
    ...
    def test_basic_load(self, filenames, loadable_ids, platform_name, reader_kw):
        ...
        with mock.patch("satpy.readers.mirs.xr.open_dataset") as od:
            ...
            with (
                mock.patch("satpy.readers.mirs.read_atms_coeff_to_string") as fd,
                mock.patch("satpy.readers.mirs.retrieve"),
            ):
