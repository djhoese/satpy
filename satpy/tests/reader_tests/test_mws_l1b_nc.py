...
def reader(fake_file):
    ...
    return MWSL1BFile(
        filename=fake_file,
        filename_info={
            "start_time": datetime.fromisoformat("2000-01-01T01:00:00"),
            "end_time": datetime.fromisoformat("2000-01-01T02:00:00"),
...
class MWSL1BFakeFileWriter:
    ...
    def _write_status_group(dataset):
        ...
        subsat_latitude_start = group.createVariable("subsat_latitude_start", "f4")
        subsat_latitude_start[:] = 52.19

        subsat_longitude_start = group.createVariable("subsat_longitude_start", "f4")
        subsat_longitude_start[:] = 23.26

        subsat_latitude_end = group.createVariable("subsat_latitude_end", "f4")
        subsat_latitude_end[:] = 60.00

        subsat_longitude_end = group.createVariable("subsat_longitude_end", "f4")
    ...
    def _write_calibration_data_group(dataset):
        ...
        toa_bt = group.createVariable(
            "mws_toa_brightness_temperature",
            np.float32,
            dimensions=(
                "n_scans",
                "n_fovs",
...
class TestMwsL1bNCFileHandler:
    ...
    def test_get_dataset_get_channeldata_bts(self, reader):
        """Test getting channel data."""
        dataset_id = {"name": "1", "units": "K", "calibration": "brightness_temperature"}
    ...
    def test_get_dataset_aux_data_expected_data_missing(self, caplog, reader):
        ...
        log_output = "Could not find key data/navigation/mws_surface_type in NetCDF file," + " no valid Dataset created"
        assert log_output in caplog.text
    ...
    def test_get_global_attributes(self, reader):
        ...
        assert attributes == {
            ...
            "quality_group": {
                "duration_of_product": np.array(5944.0, dtype=np.float32),
