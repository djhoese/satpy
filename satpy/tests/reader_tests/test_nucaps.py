...
class FakeNetCDF4FileHandler2(FakeNetCDF4FileHandler):
    ...
    def get_test_content(self, filename, filename_info, filetype_info):
        ...
        for k, units, standard_name in [
            ...
            file_content[k + "/attr/valid_range"] = (0.0, 120.0)
            file_content[k + "/attr/_FillValue"] = -9999.0
            if standard_name:
        ...
        file_content[k + "/attr/valid_range"] = (0.0, 2000.0)
        file_content[k + "/attr/_FillValue"] = -9999.0
        ...
        file_content[k + "/attr/valid_range"] = (-180.0, 180.0)
        ...
        file_content[k + "/attr/valid_range"] = (-90.0, 90.0)
...
class TestNUCAPSReader(unittest.TestCase):
    ...
    def test_load_individual_pressure_levels_min_max(self):
        ...
        r.create_filehandlers(loadables)
        datasets = r.load(r.pressure_dataset_names["Temperature"], pressure_levels=(100.0, 150.0))
        self.assertEqual(len(datasets), 6)
        for v in datasets.values():
            self.assertEqual(v.ndim, 1)
    ...
    def test_load_pressure_levels_min_max(self):
        ...
        r.create_filehandlers(loadables)
        datasets = r.load(["Temperature"], pressure_levels=(100.0, 150.0))
        self.assertEqual(len(datasets), 1)
        for v in datasets.values():
            self.assertEqual(v.ndim, 2)
            self.assertTupleEqual(v.shape, (DEFAULT_PRES_FILE_SHAPE[0], 6))
    ...
    def test_load_pressure_levels_single(self):
        ...
        for v in datasets.values():
            self.assertEqual(v.ndim, 2)
            self.assertTupleEqual(v.shape, (DEFAULT_PRES_FILE_SHAPE[0], 1))
    ...
    def test_load_pressure_levels_single_and_pressure_levels(self):
        ...
        self.assertTupleEqual(t_ds.shape, (DEFAULT_PRES_FILE_SHAPE[0], 1))
