...
dimensions = {"nx": 1530, "ny": 928, "pal_colors_250": 250, "pal_rgb": 3}
...
COT_PALETTE_MEANINGS = (
    "0 2 5 8 10 13 16 19 23 26 29 33 36 40 43 47 51 55 59 63 68 72 77 81 86 91 96"
...
class TestNcNWCSAFGeo:
    ...
    @pytest.mark.parametrize("platform, instrument", [("GOES16", "abi"), ("MSG4", "seviri")])
    ...
    @pytest.mark.parametrize(
        "attrs, expected",
        [
            ({"scale_factor": np.array(1.5), "add_offset": np.array(2.5), "_FillValue": 1}, [np.nan, 5.5, 7]),
            ({"scale_factor": np.array(1.5), "add_offset": np.array(2.5), "valid_min": 1.1}, [np.nan, 5.5, 7]),
            ({"scale_factor": np.array(1.5), "add_offset": np.array(2.5), "valid_max": 2.1}, [4, 5.5, np.nan]),
            (
                {"scale_factor": np.array(1.5), "add_offset": np.array(2.5), "valid_range": (1.1, 2.1)},
                [np.nan, 5.5, np.nan],
            ),
        ],
    )
    ...
    def test_scale_dataset_floating_nwcsaf_geo_ctth(self, nwcsaf_geo_ct_filehandler):
        ...
        attrs = {"scale_factor": np.array(1.0), "add_offset": np.array(-2000.0), "valid_range": (0.0, 27000.0)}
        var = xr.DataArray([1, 2, 3], attrs=attrs)
        var = nwcsaf_geo_ct_filehandler.scale_dataset(var, "dummy")
        np.testing.assert_allclose(var, [-1999.0, -1998.0, -1997.0])
        assert "scale_factor" not in var.attrs
        assert "add_offset" not in var.attrs
        np.testing.assert_equal(var.attrs["valid_range"], (-2000.0, 25000.0))
...
class TestNcNWCSAFPPS:
    ...
    def test_drop_xycoords(self, nwcsaf_pps_cmic_filehandler):
        ...
        lat = xr.DataArray(
            ...
            attrs={"name": "lat", "standard_name": "latitude"},
        )
        lon = xr.DataArray(
            np.ones((5, 10)),
            dims=("y", "x"),
            coords={"y": y_line, "x": x_pixel},
            attrs={"name": "lon", "standard_name": "longitude"},
        )
        data_array_in = xr.DataArray(
            np.ones((5, 10)),
            attrs={"scale_factor": np.array(0, dtype=float), "add_offset": np.array(1, dtype=float)},
            dims=("y", "x"),
    ...
    def test_get_dataset_scales_and_offsets(self, nwcsaf_pps_cpp_filehandler):
        ...
        info = dict(name="cpp_cot", file_type="nc_nwcsaf_cpp")
    ...
    def test_get_dataset_scales_and_offsets_palette_meanings_using_other_dataset(self, nwcsaf_pps_cpp_filehandler):
        ...
        info = dict(name="cpp_cot_pal", file_type="nc_nwcsaf_cpp", scale_offset_dataset="cot")
    ...
    def test_get_palette_fill_value_color_added(self, nwcsaf_pps_ctth_filehandler):
        ...
        info = dict(name="ctth_alti_pal", file_type="nc_nwcsaf_ctth", scale_offset_dataset="ctth_alti")
    ...
    def test_get_dataset_raises_when_dataset_missing(self, nwcsaf_pps_cpp_filehandler):
        ...
        dsid = {"name": "cpp_phase"}
        info = dict(name="cpp_phase", file_type="nc_nwcsaf_cpp")
    ...
    def test_get_dataset_can_handle_file_key_list(self, nwcsaf_pps_cmic_filehandler, nwcsaf_pps_cpp_filehandler):
        ...
        info_cpp = dict(name="cmic_reff", file_key=["reff", "cre"], file_type="nc_nwcsaf_cpp")

        res_cpp = nwcsaf_pps_cpp_filehandler.get_dataset(dsid_cpp, info_cpp)

        info_cmic = dict(name="cmic_reff", file_key=["reff", "cre"], file_type="nc_nwcsaf_cpp")
