...
class TestOLCIReader(unittest.TestCase):
    ...
    def test_open_file_objects(self, mocked_open_dataset):
        ...
        assert open_file.open.return_value in mocked_open_dataset.call_args[
            0
        ] or open_file.open.return_value == mocked_open_dataset.call_args[1].get("filename_or_obj")
    ...
    def test_get_mask(self, mocked_dataset):
        ...
        mocked_dataset.return_value = xr.Dataset(
            {"mask": (["rows", "columns"], np.array([1 << x for x in range(30)]).reshape(5, 6))},
            coords={"rows": np.arange(5), "columns": np.arange(6)},
        )
    ...
    def test_olci_angles(self, mocked_dataset):
        ...
        mocked_dataset.return_value = xr.Dataset(
            {
                "SAA": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
                "SZA": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
                "OAA": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
                "OZA": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
            },
            coords={"rows": np.arange(5), "columns": np.arange(6)},
            attrs=attr_dict,
        )
    ...
    def test_olci_meteo(self, mocked_dataset):
        ...
        data = {
            "humidity": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
            "total_ozone": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
            "sea_level_pressure": (["tie_rows", "tie_columns"], np.array([1 << x for x in range(30)]).reshape(5, 6)),
            "total_columnar_water_vapour": (
                ["tie_rows", "tie_columns"],
                np.array([1 << x for x in range(30)]).reshape(5, 6),
            ),
        }
        mocked_dataset.return_value = xr.Dataset(
            data, coords={"rows": np.arange(5), "columns": np.arange(6)}, attrs=attr_dict
        )
    ...
    def test_chl_nn(self, mocked_dataset):
        ...
        data = {"CHL_NN": (["rows", "columns"], np.arange(30).reshape(5, 6).astype(float), {"units": "lg(re mg.m-3)"})}
        mocked_dataset.return_value = xr.Dataset(
            data, coords={"rows": np.arange(5), "columns": np.arange(6)}, attrs=attr_dict
        )
        ds_info = {
        ...
        filename_info = {
            ...
            "creation_time": datetime.datetime(2019, 9, 24, 11, 40, 26),
            "duration": 179,
            "cycle": 49,
            "relative_orbit": 307,
            "frame": 1800,
