...
class TestSAFENC(unittest.TestCase):
    ...
    def setUp(self, xr_):
        ...
        self.nc = xr.Dataset(
            {
                "owiWindSpeed": xr.DataArray(
                    self.dummy2d, dims=("owiAzSize", "owiRaSize"), attrs={"_FillValue": np.nan}
                ),
            ...
            attrs={"_FillValue": np.nan, "missionName": "S1A"},
        )
        ...
        self.reader = SAFENC(
            filename="dummy",
            filename_info={"start_time": 0, "end_time": 0, "fstart_time": 0, "fend_time": 0, "polarization": "vv"},
            filetype_info={},
        )
    ...
    def test_get_dataset(self):
        ...
        for ch in self.channels:
            dt = self.reader.get_dataset(key=make_dataid(name=ch), info={})
            # ... this only compares the valid (unmasked) elements
            self.assertTrue(
                np.all(self.nc[ch] == dt.to_masked_array()),
                msg="get_dataset() returns invalid data for dataset {}".format(ch),
            )
