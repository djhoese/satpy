...
def _create_test_netcdf(filename, resolution=742):
    ...
    lat = xr.DataArray(
        lat, dims=("y", "x"), attrs={"name": "lat", "standard_name": "latitude", "modifiers": np.array([])}
    )
    lon = xr.DataArray(
        lon, dims=("y", "x"), attrs={"name": "lon", "standard_name": "longitude", "modifiers": np.array([])}
    )

    solar_zenith_angle_i = xr.DataArray(
        data_visir,
        dims=("y", "x"),
        attrs={"name": "solar_zenith_angle", "coordinates": "lat lon", "resolution": resolution},
    )

    scene = Scene()
    scene.attrs["sensor"] = ["viirs"]
    scene_dict = {"lat": lat, "lon": lon, "solar_zenith_angle": solar_zenith_angle_i}

    tstart = datetime(2019, 4, 1, 12, 0)
    tend = datetime(2019, 4, 1, 12, 15)
    common_attrs = {"start_time": tstart, "end_time": tend, "platform_name": "NOAA 20", "orbit_number": 99999}

    for key in scene_dict:
        scene[key] = scene_dict[key]
        if key != "swath_data":
            scene[key].attrs.update(common_attrs)
    scene.save_datasets(writer="cf", filename=filename, engine="h5netcdf", flatten_attrs=True, pretty=True)
    return filename
...
def _cf_scene():
    ...
    proj_dict = {"a": 6378169.0, "b": 6356583.8, "h": 35785831.0, "lon_0": 0.0, "proj": "geos", "units": "m"}
    ...
    qual_f = xr.DataArray(
        ...
        attrs={"name": "qual_flags", "id_tag": "qual_flags"},
    )
    lat = xr.DataArray(
        lat,
        dims=("y", "x"),
        coords={"y": y_visir, "x": x_visir},
        attrs={"name": "lat", "standard_name": "latitude", "modifiers": np.array([])},
    )
    lon = xr.DataArray(
        lon,
        dims=("y", "x"),
        coords={"y": y_visir, "x": x_visir},
        attrs={"name": "lon", "standard_name": "longitude", "modifiers": np.array([])},
    )

    # for prefix testing
    prefix_data = xr.DataArray(
        data_visir,
        dims=("y", "x"),
        coords={"y": y_visir, "x": x_visir},
        attrs={
...
class TestCFReader:
    ...
    def test_write_and_read_with_area_definition(self, _cf_scene, _nc_filename):
        ...
        _cf_scene.save_datasets(writer="cf", filename=_nc_filename, engine="h5netcdf", flatten_attrs=True, pretty=True)
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename])
    ...
    def test_fix_modifier_attr(self):
        ...
        reader = SatpyCFFileHandler("filename", {}, {"filetype": "info"})
        ds_info = {"modifiers": []}
    ...
    def test_read_prefixed_channels(self, _cf_scene, _nc_filename):
        ...
        _cf_scene.save_datasets(writer="cf", filename=_nc_filename, engine="netcdf4", flatten_attrs=True, pretty=True)
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename])
        scn_.load(["1"])
        ...
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename], reader_kwargs={})
        scn_.load(["1"])
        np.testing.assert_array_equal(scn_["1"].data, _cf_scene["1"].data)
        np.testing.assert_array_equal(scn_["1"].coords["lon"], _cf_scene["lon"].data)  # lon loaded as coord
    ...
    def test_read_prefixed_channels_by_user(self, _cf_scene, _nc_filename):
        ...
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename], reader_kwargs={"numeric_name_prefix": "USER"})
        scn_.load(["1"])
        np.testing.assert_array_equal(scn_["1"].data, _cf_scene["1"].data)
    ...
    def test_orbital_parameters(self, _cf_scene, _nc_filename):
        ...
        _cf_scene.save_datasets(writer="cf", filename=_nc_filename)
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename])
        scn_.load(["image0"])
    ...
    def test_write_and_read_from_two_files(self, _nc_filename, _nc_filename_i):
        ...
        scn_ = Scene(reader="satpy_cf_nc", filenames=[_nc_filename, _nc_filename_i])
    ...
    def test_dataid_attrs_equal_not_matching_dataset(self, _cf_scene, _nc_filename):
        ...
        ds_id = DataID(
            default_id_keys_config, name="solar_zenith_angle", resolution=not_existing_resolution, modifiers=()
        )
    ...
    def test_dataid_attrs_equal_contains_not_matching_key(self, _cf_scene, _nc_filename):
        ...
        reader = SatpyCFFileHandler(_nc_filename, {}, {"filetype": "info"})
        ds_id = DataID(
            default_id_keys_config, name="solar_zenith_angle", resolution=742, modifiers=(), calibration="counts"
        )
        res = reader.get_dataset(ds_id, {})
        assert res.attrs["resolution"] == 742
