...
class TestSCMIFileHandler(unittest.TestCase):
    ...
    def setUp(self, xr_):
        ...
        self.expected_rad = rad_data.astype(np.float64) * 0.5 + -1.0
        ...
        self.reader = SCMIFileHandler("filename", {"platform_shortname": "G16"}, {"filetype": "info"})
    ...
    def test_basic_attributes(self):
        ...
        self.assertEqual(self.reader.start_time, datetime(2017, 7, 29, 12, 0, 0, 0))
        self.assertEqual(self.reader.end_time, datetime(2017, 7, 29, 12, 0, 0, 0))
        self.assertEqual(self.reader.get_shape(make_dataid(name="C05"), {}), (2, 5))
...
class TestSCMIFileHandlerArea(unittest.TestCase):
    ...
    def create_reader(self, proj_name, proj_attrs, xr_):
        ...
        x__ = xr.DataArray(
            [0, 1],
            attrs={"scale_factor": 2.0, "add_offset": -1.0, "units": "meters"},
        )
        y__ = xr.DataArray(
            [0, 1],
            attrs={"scale_factor": -2.0, "add_offset": 1.0, "units": "meters"},
        ...
        xr_.open_dataset.return_value = FakeDataset(
            {"goes_imager_projection": proj, "x": x__, "y": y__, "Sectorized_CMI": np.ones((2, 2))},
            {
                "satellite_id": "GOES-16",
        ...
        return SCMIFileHandler("filename", {"platform_shortname": "G16"}, {"filetype": "info"})
    ...
    def test_get_area_def_geos(self, adef):
        ...
        self.assertDictEqual(
            call_args[3],
            {"a": 1.0, "b": 1.0, "h": 1.0, "lon_0": -90.0, "lat_0": 0.0, "proj": "geos", "sweep": "x", "units": "m"},
        )
        self.assertEqual(call_args[4], reader.ncols)
        self.assertEqual(call_args[5], reader.nlines)
        np.testing.assert_allclose(call_args[6], (-2.0, -2.0, 2, 2.0))
    ...
    def test_get_area_def_lcc(self, adef):
        ...
        self.assertDictEqual(
            call_args[3],
            {"a": 1.0, "b": 1.0, "lon_0": -90.0, "lat_0": 25.0, "lat_1": 25.0, "proj": "lcc", "units": "m"},
        )
        self.assertEqual(call_args[4], reader.ncols)
        self.assertEqual(call_args[5], reader.nlines)
        np.testing.assert_allclose(call_args[6], (-2.0, -2.0, 2, 2.0))
    ...
    def test_get_area_def_stere(self, adef):
        ...
        self.assertDictEqual(
            call_args[3],
            {"a": 1.0, "b": 1.0, "lon_0": -90.0, "lat_0": 90.0, "lat_ts": 60.0, "proj": "stere", "units": "m"},
        )
        self.assertEqual(call_args[4], reader.ncols)
        self.assertEqual(call_args[5], reader.nlines)
        np.testing.assert_allclose(call_args[6], (-2.0, -2.0, 2, 2.0))
    ...
    def test_get_area_def_merc(self, adef):
        ...
        self.assertDictEqual(
            call_args[3],
            {"a": 1.0, "b": 1.0, "lon_0": -90.0, "lat_0": 0.0, "lat_ts": 0.0, "proj": "merc", "units": "m"},
        )
        self.assertEqual(call_args[4], reader.ncols)
        self.assertEqual(call_args[5], reader.nlines)
        np.testing.assert_allclose(call_args[6], (-2.0, -2.0, 2, 2.0))
