...
COUNTS_INPUT = xr.DataArray(
    np.array(
        [
            [377.0, 377.0, 377.0, 376.0, 375.0],
            [376.0, 375.0, 376.0, 374.0, 374.0],
            [374.0, 373.0, 373.0, 374.0, 374.0],
            [347.0, 345.0, 345.0, 348.0, 347.0],
            [306.0, 306.0, 307.0, 307.0, 308.0],
        ],
        dtype=np.float32,
    )
)
...
RADIANCES_OUTPUT = xr.DataArray(
    np.array(
        [
            [66.84162903, 66.84162903, 66.84162903, 66.63659668, 66.4315567],
            [66.63659668, 66.4315567, 66.63659668, 66.22652435, 66.22652435],
            [66.22652435, 66.02148438, 66.02148438, 66.22652435, 66.22652435],
            [60.69055939, 60.28048706, 60.28048706, 60.89559937, 60.69055939],
            [52.28409576, 52.28409576, 52.48912811, 52.48912811, 52.69416809],
        ],
        dtype=np.float32,
    )
)
...
TBS_OUTPUT1 = xr.DataArray(
    np.array(
        [
            [269.29684448, 269.29684448, 269.29684448, 269.13296509, 268.96871948],
            [269.13296509, 268.96871948, 269.13296509, 268.80422974, 268.80422974],
            [268.80422974, 268.63937378, 268.63937378, 268.80422974, 268.80422974],
            [264.23751831, 263.88912964, 263.88912964, 264.41116333, 264.23751831],
            [256.77682495, 256.77682495, 256.96743774, 256.96743774, 257.15756226],
        ],
        dtype=np.float32,
    )
)
...
TBS_OUTPUT2 = xr.DataArray(
    np.array(
        [
            [268.94519043, 268.94519043, 268.94519043, 268.77984619, 268.61422729],
            [268.77984619, 268.61422729, 268.77984619, 268.44830322, 268.44830322],
            [268.44830322, 268.28204346, 268.28204346, 268.44830322, 268.44830322],
            [263.84396362, 263.49285889, 263.49285889, 264.01898193, 263.84396362],
            [256.32858276, 256.32858276, 256.52044678, 256.52044678, 256.71188354],
        ],
        dtype=np.float32,
    )
)
...
VIS008_RADIANCE = xr.DataArray(
    np.array(
        [
            [0.62234485, 0.59405649, 0.59405649, 0.59405649, 0.59405649],
...
VIS008_REFLECTANCE = xr.DataArray(
    np.array(
        [
            [2.739768, 2.615233, 2.615233, 2.615233, 2.615233],
...
class TestSeviriCalibrationHandler:
    ...
    def test_init(self):
        ...
        with pytest.raises(ValueError):
            SEVIRICalibrationHandler(
                platform_id=None, channel_name=None, coefs=None, calib_mode="invalid", scan_time=None
            )
    ...
    def _get_calibration_handler(self, calib_mode="NOMINAL", ext_coefs=None):
        ...
        return SEVIRICalibrationHandler(
            ...
            coefs={
                "coefs": {
                    "NOMINAL": {"gain": 10, "offset": -1},
                    "GSICS": {"gain": 20, "offset": -2},
    ...
    def test_get_gain_offset(self, calib_mode, ext_coefs, expected):
        ...
        calib = self._get_calibration_handler(calib_mode=calib_mode, ext_coefs=ext_coefs)
...
class TestFileHandlerCalibrationBase:
    ...
    expected = {
        "VIS006": {
            "counts": {"NOMINAL": xr.DataArray([[0, 10], [100, 255]], dims=("y", "x"))},
            "radiance": {
                "NOMINAL": xr.DataArray([[np.nan, 9], [99, 254]], dims=("y", "x")),
                "GSICS": xr.DataArray([[np.nan, 9], [99, 254]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 90], [990, 2540]], dims=("y", "x")),
            },
            "reflectance": {
                "NOMINAL": xr.DataArray([[np.nan, 41.88985], [460.7884, 1182.2247]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 418.89853], [4607.8843, 11822.249]], dims=("y", "x")),
            },
        },
        "IR_108": {
            "counts": {"NOMINAL": xr.DataArray([[0, 10], [100, 255]], dims=("y", "x"))},
            "radiance": {
                "NOMINAL": xr.DataArray([[np.nan, 81], [891, 2286]], dims=("y", "x")),
                "GSICS": xr.DataArray([[np.nan, 8.19], [89.19, 228.69]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 180], [1980, 5080]], dims=("y", "x")),
            },
            "brightness_temperature": {
                "NOMINAL": xr.DataArray([[np.nan, 279.82318], [543.2585, 812.77167]], dims=("y", "x")),
                "GSICS": xr.DataArray([[np.nan, 189.20985], [285.53293, 356.06668]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 335.14236], [758.6249, 1262.7567]], dims=("y", "x")),
            },
        },
        "HRV": {
            "counts": {"NOMINAL": xr.DataArray([[0, 10], [100, 255]], dims=("y", "x"))},
            "radiance": {
                "NOMINAL": xr.DataArray([[np.nan, 108], [1188, 3048]], dims=("y", "x")),
                "GSICS": xr.DataArray([[np.nan, 108], [1188, 3048]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 45], [495, 1270]], dims=("y", "x")),
            },
            "reflectance": {
                "NOMINAL": xr.DataArray([[np.nan, 415.26767], [4567.944, 11719.775]], dims=("y", "x")),
                "EXTERNAL": xr.DataArray([[np.nan, 173.02817], [1903.31, 4883.2397]], dims=("y", "x")),
            },
        },
    }
    ...
    def counts(self):
        ...
        return xr.DataArray([[0, 10], [100, 255]], dims=("y", "x"))

    def _get_expected(self, channel, calibration, calib_mode, use_ext_coefs):
        if use_ext_coefs:
