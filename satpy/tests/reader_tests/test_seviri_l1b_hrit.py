...
class TestHRITMSGFileHandlerHRV(TestHRITMSGBase):
    ...
    def setUp(self):
        ...
        self.reader.mda.update({"segment_sequence_number": 18, "planned_start_segment_number": 1})
    ...
    def test_read_hrv_band(self, memmap):
        ...
        memmap.return_value = np.random.randint(0, 256, size=int((464 * 5568 * nbits) / 8), dtype=np.uint8)
    ...
    def test_get_dataset(self, calibrate, parent_get_dataset):
        ...
        np.testing.assert_equal(res["acq_time"], setup.get_acq_time_exp(self.observation_start_time, self.nlines))
    ...
    def test_get_area_def(self):
        ...
        self.assertEqual(
            area.area_extent, (-45561979844414.07, -3720765401003.719, 45602912357076.38, 77771774058.38356)
        )
        ...
        area = self.reader.get_area_def(make_dataid(name="HRV", resolution=1000))
        npt.assert_allclose(
            area.defs[0].area_extent, (-22017598561055.01, -2926674655354.9604, 23564847539690.22, 77771774058.38356)
        )
        npt.assert_allclose(
            area.defs[1].area_extent, (-30793529275853.656, -3720765401003.719, 14788916824891.568, -2926674655354.9604)
        )
...
class TestHRITMSGFileHandler(TestHRITMSGBase):
    ...
    def _get_fake_data(self):
        return xr.DataArray(data=np.zeros((self.nlines, self.ncols)), dims=("y", "x"))
    ...
    def test_get_area_def(self):
        ...
        self.assertEqual(proj_dict["units"], "m")
        self.assertEqual(
            area.area_extent, (-77771774058.38356, -3720765401003.719, 30310525626438.438, 77771774058.38356)
        )
        ...
        area = self.reader.get_area_def(make_dataid(name="VIS006", resolution=3000))
        self.assertEqual(
            area.area_extent, (-77771772558.38356, -3720765402503.719, 30310525627938.438, 77771772558.38356)
        )
    ...
    def test_read_band(self, memmap):
        """Test reading a band."""
        nbits = self.reader.mda["number_of_bits_per_pixel"]
        memmap.return_value = np.random.randint(0, 256, size=int((464 * 3712 * nbits) / 8), dtype=np.uint8)
    ...
    def test_get_dataset(self, calibrate, parent_get_dataset):
        ...
        expected["acq_time"] = ("y", setup.get_acq_time_exp(self.observation_start_time, self.nlines))
        xr.testing.assert_equal(res, expected)
        self.assert_attrs_equal(res.attrs, setup.get_attrs_exp(self.projection_longitude))
    ...
    def test_satpos_no_valid_orbit_polynomial(self):
        ...
        self.assertNotIn("satellite_actual_longitude", reader.mda["orbital_parameters"])
...
class TestHRITMSGEpilogueFileHandler(unittest.TestCase):
    ...
    def setUp(self, init, *mocks):
        ...
        self.reader = HRITMSGEpilogueFileHandler(
            filename="dummy_epilogue_filename", filename_info={"service": ""}, filetype_info=None, calib_mode="nominal"
        )
    ...
    def test_extra_kwargs(self, init, *mocks):
        ...
        HRITMSGEpilogueFileHandler(
            filename="dummy_epilogue_filename",
            filename_info={"service": ""},
            filetype_info=None,
            ext_calib_coefs={},
            mda_max_array_size=123,
...
class TestHRITMSGCalibration(TestFileHandlerCalibrationBase):
    ...
    def file_handler(self):
        ...
        epilog = {"ImageProductionStats": {"ActualScanningSummary": {"ForwardScanStart": self.scan_time}}}
        mda = {
        ...
        with mock.patch("satpy.readers.seviri_l1b_hrit.HRITMSGFileHandler.__init__", return_value=None):
    ...
    def test_mask_bad_quality(self, file_handler):
        ...
        channel = "VIS006"
        expected = self._get_expected(
            channel=channel, calibration="radiance", calib_mode="NOMINAL", use_ext_coefs=False
        )
