...
def new_get_hd(instance, hdr_info):
    ...
    instance.mda["projection_parameters"] = {"a": 6378169.00, "b": 6356583.80, "h": 35785831.00, "SSP_longitude": 0.0}
...
def get_fake_file_handler(
    observation_start_time, nlines, ncols, projection_longitude=0, orbit_polynomials=ORBIT_POLYNOMIALS
):
    ...
    m = mock.mock_open()
    with (
        mock.patch("satpy.readers.seviri_l1b_hrit.np.fromfile") as fromfile,
        ...
        mock.patch.object(HRITMSGPrologueFileHandler, "read_prologue", new=get_new_read_prologue(prologue)),
    ):
        fromfile.return_value = np.array([(1, 2)], dtype=[("total_header_length", int), ("hdr_id", int)])
        ...
        reader = HRITMSGFileHandler("filename", filename_info, {"filetype": "info"}, prologue, epilogue)
...
def get_fake_prologue(projection_longitude, orbit_polynomials):
    ...
    return {
        "SatelliteStatus": {
            "SatelliteDefinition": {"SatelliteId": 324, "NominalLongitude": -3.5},
            "Orbit": {
        ...
        "ImageDescription": {
            "ProjectionDescription": {"LongitudeOfSSP": projection_longitude},
...
def get_fake_filename_info(start_time):
    ...
    return {"platform_shortname": "MSG3", "start_time": start_time, "service": "MSG"}


def get_fake_dataset_info():
    """Create fake dataset info."""
    return {"units": "units", "wavelength": "wavelength", "standard_name": "standard_name"}
