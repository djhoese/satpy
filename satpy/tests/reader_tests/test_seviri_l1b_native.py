...
TEST_IS_ROI_FULLDISK = {"is_full_disk": True, "is_rapid_scan": 0, "is_roi": False}

TEST_IS_ROI_RAPIDSCAN = {"is_full_disk": False, "is_rapid_scan": 1, "is_roi": False}

TEST_IS_ROI_ROI = {"is_full_disk": False, "is_rapid_scan": 0, "is_roi": True}
...
TEST_PADDER_RSS_ROI = {
    ...
    "final_shape": (5, 5),
    "expected_padded_data": xr.DataArray(
        np.array(
            [
...
def create_test_header(earth_model, dataset_id, is_full_disk, is_rapid_scan, good_qual="OK"):
    ...
    if is_full_disk:
        ...
        ssp_lon = 0
    header = {
        "15_MAIN_PRODUCT_HEADER": {"QQOV": {"Name": "QQOV", "Value": good_qual}},
        "15_DATA_HEADER": {
            "ImageDescription": {
                reference_grid: {
                ...
                "ProjectionDescription": {"LongitudeOfSSP": ssp_lon},
            },
            "GeometricProcessing": {
                "EarthModel": {
    ...
    trailer = {
        "15TRAILER": {
            "ImageProductionStats": {
                ...
                "ActualScanningSummary": {"ReducedScan": is_rapid_scan},
            }
        }
    }
...
def prepare_area_definitions(test_dict):
    ...
    with (
        ...
        mock.patch("satpy.readers.seviri_l1b_native.has_archive_header") as has_archive_header,
    ):
...
def test_area_definitions(actual, expected):
    ...
    np.testing.assert_allclose(np.array(actual.area_extent), np.array(expected["Area extent"]))
...
def test_stacked_area_definitions(actual, expected):
    ...
    np.testing.assert_allclose(np.array(actual.defs[0].area_extent), np.array(expected["Area extent 0"]))
    np.testing.assert_allclose(np.array(actual.defs[1].area_extent), np.array(expected["Area extent 1"]))
    assert actual.width == expected["Number of columns"]
    assert actual.height == expected["Number of rows"]
...
class TestNativeMSGCalibration(TestFileHandlerCalibrationBase):
    ...
    def file_handler(self):
        ...
        header = {
            "15_DATA_HEADER": {"ImageAcquisition": {"PlannedAcquisitionTime": {"TrueRepeatCycleStart": self.scan_time}}}
        }
        trailer = {
            "15TRAILER": {"ImageProductionStats": {"ActualScanningSummary": {"ForwardScanStart": self.scan_time}}}
        }
        header["15_DATA_HEADER"].update(TEST_HEADER_CALIB)
        with mock.patch("satpy.readers.seviri_l1b_native.NativeMSGFileHandler.__init__", return_value=None):
            fh = NativeMSGFileHandler(filename=None, filename_info={}, filetype_info=None)
            fh.header = header
            fh.trailer = trailer
    ...
    def test_calibrate(self, file_handler, counts, channel, calibration, calib_mode, use_ext_coefs):
        """Test the calibration."""
        external_coefs = self.external_coefs if use_ext_coefs else {}
        expected = self._get_expected(
            channel=channel, calibration=calibration, calib_mode=calib_mode, use_ext_coefs=use_ext_coefs
        )
...
class TestNativeMSGDataset:
    ...
    def file_handler(self):
        ...
        mda = {
            ...
            "projection_parameters": {"ssp_longitude": 0.0, "h": 35785831.0, "a": 6378169.0, "b": 6356583.8},
        }
    ...
    def _fake_header():
        header = {
            "15_DATA_HEADER": {
                "SatelliteStatus": {
                    "SatelliteDefinition": {"NominalLongitude": 0.0},
                    "Orbit": {"OrbitPolynomial": ORBIT_POLYNOMIALS},
                },
                "ImageAcquisition": {
    ...
    def _fake_data():
        ...
        visir_rec = [("line_data", np.uint8, (num_visir_cols,)), ("acq_time", time_cds_short)]
        vis006_line1 = ([1, 2, 3, 4, 5], (1, 1000))  # line_data  # acq_time (days, milliseconds)
    ...
    def test_get_dataset(self, file_handler):
        ...
        dataset_id = make_dataid(name="VIS006", resolution=3000, calibration="counts")
        dataset_info = {"units": "1", "wavelength": (1, 2, 3), "standard_name": "counts"}
    ...
    def _exp_data_array():
        expected = xr.DataArray(
            np.array(
                [
                    [4.0, 32.0, 193.0, 5.0],
                    [24.0, 112.0, 514.0, 266.0],
                    [44.0, 192.0, 835.0, 527.0],
                    [64.0, 273.0, 132.0, 788.0],
                ],
                dtype=np.float32,
            ),
            dims=["y", "x"],
        ...
        expected["acq_time"] = (
            "y",
            [
                np.datetime64("1958-01-02 00:00:01"),
    ...
    def test_satpos_no_valid_orbit_polynomial(self, file_handler):
        ...
        file_handler.header["15_DATA_HEADER"]["SatelliteStatus"]["Orbit"]["OrbitPolynomial"] = ORBIT_POLYNOMIALS_INVALID
        dataset_id = make_dataid(name="VIS006", resolution=3000, calibration="counts")
        dataset_info = {"units": "1", "wavelength": (1, 2, 3), "standard_name": "counts"}
        with pytest.warns(UserWarning, match="No orbit polynomial"):
            xarr = file_handler.get_dataset(dataset_id, dataset_info)
        assert "satellite_actual_longitude" not in xarr.attrs["orbital_parameters"]
...
class TestNativeMSGFilenames:
    ...
    def reader(self):
        ...
        reader_configs = config_search_paths(os.path.join("readers", "seviri_l1b_native.yaml"))
...
def test_header_type(file_content, exp_header_size):
    ...
    header = create_test_header(
        dataset_id=make_dataid(name="VIS006", resolution=3000), earth_model=1, is_full_disk=True, is_rapid_scan=0
    )
...
@pytest.mark.parametrize("starts_with, expected", [(ASCII_STARTSWITH, True), (b"this_shall_fail", False)])
