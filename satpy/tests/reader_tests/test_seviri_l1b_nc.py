...
class TestNCSEVIRIFileHandler(TestFileHandlerCalibrationBase):
    ...
    def _get_fake_dataset(self, counts, h5netcdf):
        ...
        orbit_poly_start_day, orbit_poly_start_msec = to_cds_time(
            np.array([datetime(2019, 12, 31, 18), datetime(2019, 12, 31, 22)], dtype="datetime64")
        )
        orbit_poly_end_day, orbit_poly_end_msec = to_cds_time(
            np.array([datetime(2019, 12, 31, 22), datetime(2020, 1, 1, 2)], dtype="datetime64")
        )
        counts = counts.rename({"y": "num_rows_vis_ir", "x": "num_columns_vis_ir"})
        scan_time_days, scan_time_msecs = to_cds_time(self.scan_time)
        ds = xr.Dataset(
            {
                "ch1": counts.copy(),
                "ch9": counts.copy(),
                "HRV": (("num_rows_hrv", "num_columns_hrv"), [[1, 2, 3], [4, 5, 6], [7, 8, 9]]),
                ...
                "channel_data_visir_data_line_validity": (("num_rows_vis_ir", "channels_vis_ir_dim"), line_validity),
                ...
                "orbit_polynomial_x": (
                    ("orbit_polynomial_dim_row", "orbit_polynomial_dim_col"),
                ...
                "orbit_polynomial_start_time_day": ("orbit_polynomial_dim_row", orbit_poly_start_day),
                "orbit_polynomial_start_time_msec": ("orbit_polynomial_dim_row", orbit_poly_start_msec),
                "orbit_polynomial_end_time_day": ("orbit_polynomial_dim_row", orbit_poly_end_day),
                "orbit_polynomial_end_time_msec": ("orbit_polynomial_dim_row", orbit_poly_end_msec),
            },
            attrs={
        ...
        if h5netcdf:
            nattrs = {
                "equatorial_radius": np.array([6378.169]),
        ...
        ds["ch1"].attrs.update({"scale_factor": self.gains_nominal[0], "add_offset": self.offsets_nominal[0]})
        ...
        strip_attrs = {"comment": None, "long_name": None, "valid_min": None, "valid_max": None}
        for name in ["ch1", "ch9"]:
    ...
    def file_handler(self, counts, h5netcdf):
        ...
        with mock.patch(
            ...
            return NCSEVIRIFileHandler(
                "filename",
                {"platform_shortname": "MSG3", "start_time": self.scan_time, "service": "MSG"},
                {"filetype": "info"},
            )
    ...
    def test_calibrate(self, file_handler, channel, calibration, use_ext_coefs):
        """Test the calibration."""
        external_coefs = self.external_coefs if use_ext_coefs else {}
        expected = self._get_expected(
            channel=channel, calibration=calibration, calib_mode="NOMINAL", use_ext_coefs=use_ext_coefs
        )
        fh = file_handler
    ...
    def test_mask_bad_quality(self, file_handler):
        ...
        channel = "VIS006"
        key = channel_keys_dict[channel]
        dataset_info = {"nc_key": key, "units": "units", "wavelength": "wavelength", "standard_name": "standard_name"}
        expected = self._get_expected(
    ...
    @pytest.mark.parametrize(
        ...
        [("VIS006", "reflectance", True), ("VIS006", "reflectance", False), ("IR_108", "brightness_temperature", True)],
    )
    def test_get_dataset(self, file_handler, channel, calibration, mask_bad_quality_scan_lines):
        ...
        expected = self._get_expected(
            channel=channel, calibration=calibration, calib_mode="NOMINAL", use_ext_coefs=False
        )
        expected.attrs = {
            "orbital_parameters": {
        ...
        expected["acq_time"] = ("y", [np.datetime64("1958-01-02 00:00:01"), np.datetime64("1958-01-02 00:00:02")])
        ...
        for key in ["sun_earth_distance_correction_applied", "sun_earth_distance_correction_factor"]:
    ...
    def test_satpos_no_valid_orbit_polynomial(self, file_handler):
        ...
        res = file_handler.get_dataset(dataset_id, dataset_info)
        assert "satellite_actual_longitude" not in res.attrs["orbital_parameters"]
