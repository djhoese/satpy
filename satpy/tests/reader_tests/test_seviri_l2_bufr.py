...
FILENAME_INFO = {"start_time": "20191112000000", "spacecraft": "MSG2"}
FILENAME_INFO2 = {"start_time": "20191112000000", "spacecraft": "MSG2", "server": "TESTSERVER"}
MPEF_PRODUCT_HEADER = {
...
DATASET_INFO_LAT = {"name": "latitude", "key": "#1#latitude", "fill_value": -1.0e100}

DATASET_INFO_LON = {"name": "longitude", "key": "#1#longitude", "fill_value": -1.0e100}


DATASET_ATTRS = {"platform_name": "MET09", "ssp_lon": 45.5, "seg_size": 16}

AREA_DEF = geometry.AreaDefinition(
    "msg_seviri_iodc_48km",
    "MSG SEVIRI Indian Ocean Data Coverage service area definition with 48 km resolution",
    "",
    {"a": 6378169.0, "b": 6356583.8, "lon_0": DATASET_ATTRS["ssp_lon"], "h": 35785831.0, "proj": "geos", "units": "m"},
    232,
    232,
...
AREA_DEF_FES = geometry.AreaDefinition(
    ...
    "",
    {"a": 6378169.0, "b": 6356583.8, "lon_0": 0.0, "h": 35785831.0, "proj": "geos", "units": "m"},
    232,
    232,
    (-5570248.6867, -5567248.2834, 5567248.2834, 5570248.6867),
)
...
class SeviriL2BufrData:
    ...
    def __init__(self, filename, with_adef=False, rect_lon="default"):
        ...
        if "BUFRProd" in filename:
            ...
            with mock.patch("satpy.readers.seviri_l2_bufr.open", self.m, create=True):
                with mock.patch(
                    "eccodes.codes_bufr_new_from_file", side_effect=[self.buf1, None, self.buf1, None, self.buf1, None]
                ) as ec1:
    ...
    def get_data(self, dataset_info):
        ...
        with mock.patch("satpy.readers.seviri_l2_bufr.open", self.m, create=True):
            with mock.patch("eccodes.codes_bufr_new_from_file", side_effect=[self.buf1, self.buf1, None]) as ec1:
                ec1.return_value = ec1.side_effect
                with mock.patch("eccodes.codes_set") as ec2:
                    ec2.return_value = 1
                    with mock.patch("eccodes.codes_release") as ec5:
...
class SeviriL2AMVBufrData:
    ...
    def __init__(self, filename):
        ...
        with mock.patch("satpy.readers.seviri_l2_bufr.np.fromfile"):
            self.fh = SeviriL2BufrFileHandler(
                filename, FILENAME_INFO2, filetype_info={"file_type": "seviri_l2_bufr_amv"}, with_area_definition=True
            )
