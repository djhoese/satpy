...
FAKE_MESSAGE = {
    ...
    "Nx": 1000,
    "Ny": 1200,
    "earthMajorAxis": 6400.0,
    "earthMinorAxis": 6300.0,
    "NrInRadiusOfEarth": 6.0,
    "XpInGridLengths": 500,
    "parameterNumber": 30,
    "missingValue": 9999,
}
        ...
        with mock.patch("builtins.open", mock.mock_open()) as mock_file:
            with mock.patch("satpy.readers.seviri_l2_grib.ec", self.ec_):
                self.reader = SeviriL2GribFileHandler(
                    ...
                    filename_info={
                        "spacecraft": "MET11",
                        "start_time": datetime.datetime(year=2020, month=10, day=20, hour=19, minute=45, second=0),
                    },
                    filetype_info={},
                )
                ...
                with mock.patch(
                    "satpy.readers.seviri_l2_grib.calculate_area_extent", mock.Mock(name="calculate_area_extent")
                ) as cae:
                    with mock.patch("satpy.readers.seviri_l2_grib.get_area_definition", mock.Mock()) as gad:
                        dataset_id = make_dataid(name="dummmy", resolution=400.0)
