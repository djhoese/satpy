...
class FakeNetCDF4FileHandlerSMOSL2WIND(FakeNetCDF4FileHandler):
    ...
    def get_test_content(self, filename, filename_info, filetype_info):
        ...
        if filetype_info["file_type"] == "smos_l2_wind":
            ...
            file_content["lat"] = np.arange(-90.0, 90.25, 0.25)
            ...
            file_content["lon"] = np.arange(0.0, 360.0, 0.25)
            ...
            file_content["wind_speed"] = np.ndarray(
                shape=(1, len(file_content["lat"]), len(file_content["lon"]))  # Time dimension
            )
            file_content["wind_speed/shape"] = (1, len(file_content["lat"]), len(file_content["lon"]))
            file_content["wind_speed"] = DataArray(
                file_content["wind_speed"],
                dims=("time", "lat", "lon"),
...
class TestSMOSL2WINDReader(unittest.TestCase):
    ...
    def test_adjust_lon(self):
        ...
        smos_l2_wind_fh = SMOSL2WINDFileHandler(
            "SM_OPER_MIR_SCNFSW_20200420T021649_20200420T035013_110_001_7.nc",
            {},
            filetype_info={"file_type": "smos_l2_wind"},
        )
        data = DataArray(np.arange(0.0, 360.0, 0.25), dims="lon")
        adjusted = smos_l2_wind_fh._adjust_lon_coord(data)
        expected = DataArray(np.concatenate((np.arange(0, 180.0, 0.25), np.arange(-180.0, 0, 0.25))), dims="lon")
    ...
    def test_roll_dataset(self):
        ...
        expected = np.arange(-180.0, 180.0, 0.25)
