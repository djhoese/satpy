...
class TestViiNCBaseFileHandler(unittest.TestCase):
    ...
    def setUp(self, pgi_):
        ...
        with Dataset(self.test_file_name, "w") as nc:
            ...
            tpw[:] = 1.0
            tpw.test_attr = "attr"
            lon = g1_1.createVariable("longitude", np.float32, dimensions=("num_tie_points_act", "num_tie_points_alt"))
            lon[:] = 100.0
            lat = g1_1.createVariable("latitude", np.float32, dimensions=("num_tie_points_act", "num_tie_points_alt"))
            lat[:] = 10.0
        ...
        interp_latitude = xr.DataArray(np.ones((10, 100)) * 2.0)
        pgi_.return_value = (interp_longitude, interp_latitude)

        # Filename info valid for all readers
        filename_info = {
            "creation_time": datetime.datetime(year=2017, month=9, day=22, hour=22, minute=40, second=10),
            "sensing_start_time": datetime.datetime(year=2017, month=9, day=20, hour=12, minute=30, second=30),
            "sensing_end_time": datetime.datetime(year=2017, month=9, day=20, hour=18, minute=30, second=50),
        }
        ...
        self.reader_3 = ViiNCBaseFileHandler(
            filename=self.test_file_name, filename_info=filename_info, filetype_info={}, orthorect=True
        )
    ...
    def test_file_reading(self):
        ...
        expected_start_time = datetime.datetime(
            year=2017, month=9, day=20, hour=17, minute=30, second=40, microsecond=888000
        )
        self.assertEqual(self.reader.start_time, expected_start_time)

        expected_end_time = datetime.datetime(
            year=2017, month=9, day=20, hour=17, minute=41, second=17, microsecond=555000
        )
        ...
        expected_global_attributes = {
            ...
            "filename_start_time": datetime.datetime(year=2017, month=9, day=20, hour=12, minute=30, second=30),
            "filename_end_time": datetime.datetime(year=2017, month=9, day=20, hour=18, minute=30, second=50),
            "platform_name": "test_spacecraft",
            "quality_group": {
                "duration_of_product": 1.0,
                "duration_of_data_present": 2.0,
                "duration_of_data_missing": 3.0,
                "duration_of_data_degraded": 4.0,
                "gap_start_time_utc": (5.0, 6.0),
                "gap_end_time_utc": (7.0, 8.0),
            },
        }
    ...
    def test_functions(self, tpgi_, tpi_):
        ...
        variable = xr.DataArray(
            dims=("y", "x"),
            name="test_name",
            attrs={"key_1": "value_1", "key_2": "value_2"},
            data=np.zeros((10, 100)),
        )
        tpi_.return_value = [xr.DataArray(dims=("num_tie_points_act", "num_tie_points_alt"), data=np.ones((10, 100)))]
        ...
        variable_lon = xr.DataArray(
            dims=("y", "x"),
            name="test_lon",
            attrs={"key_1": "value_lon_1", "key_2": "value_lon_2"},
            data=np.zeros((10, 100)),
        )
        variable_lat = xr.DataArray(
            dims=("y", "x"),
            name="test_lat",
            attrs={"key_1": "value_lat_1", "key_2": "value_lat_2"},
            data=np.ones((10, 100)) * 2.0,
        )

        tpgi_.return_value = (
            xr.DataArray(dims=("num_tie_points_act", "num_tie_points_alt"), data=np.ones((10, 100))),
            xr.DataArray(dims=("num_tie_points_act", "num_tie_points_alt"), data=6 * np.ones((10, 100))),
        )
    ...
    def test_dataset(self, po_, pi_, pc_):
        ...
        variable = self.reader.get_dataset(None, {"file_key": "data/measurement_data/tpw", "calibration": None})
        ...
        self.reader.get_dataset(
            None,
            {"file_key": "data/measurement_data/tpw", "calibration": None, "orthorect_data": "test_orthorect_data"},
        )
        po_.assert_called()
        ...
        longitude = self.reader.get_dataset(
            None, {"file_key": "cached_longitude", "calibration": "reflectance", "interpolate": True}
        )
        pc_.assert_not_called()
        pi_.assert_not_called()
        self.assertEqual(longitude[0, 0], 1.0)

        # Checks the correct execution of the get_dataset function with a 'cached_latitude' file_key
        latitude = self.reader.get_dataset(None, {"file_key": "cached_latitude", "calibration": None})
        self.assertEqual(latitude[0, 0], 2.0)
        ...
        # Checks the correct execution of the get_dataset function with a 'cached_longitude' file_key
        longitude = self.reader_2.get_dataset(None, {"file_key": "cached_longitude", "calibration": None})
        self.assertEqual(longitude[0, 0], 100.0)

        # Checks the correct execution of the get_dataset function with a 'cached_longitude' file_key
        # in a reader without defined longitude
        longitude = self.reader_3.get_dataset(
            None, {"file_key": "cached_longitude", "calibration": "reflectance", "interpolate": True}
        )
        # Checks that the function returns None
