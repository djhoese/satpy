...
class TestViiL1bNCFileHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with Dataset(self.test_file_name, "w") as nc:
            ...
            sza = g1_2.createVariable(
                "solar_zenith", np.float32, dimensions=("num_tie_points_alt", "num_tie_points_act")
            )
            sza[:] = 25.0
    ...
    def test_calibration_functions(self):
        ...
        expected_bt = np.array(
            [[675.04993213, 753.10301462, 894.93149648], [963.20401882, 1048.95086402, 1270.95546218]]
        )
        ...
        expected_refl = np.array([[62.8318531, 125.6637061, 314.1592654], [439.8229715, 628.3185307, 1256.637061]])
    ...
    def test_functions(self):
        ...
        with self.assertRaises(ValueError):
            self.reader._perform_calibration(variable, {"calibration": "invalid", "name": "test"})

        # brightness_temperature calibration: checks that the return value is correct
        calibrated_variable = self.reader._perform_calibration(
            variable, {"calibration": "brightness_temperature", "chan_thermal_index": 3}
        )
        ...
        calibrated_variable = self.reader._perform_calibration(
            variable, {"calibration": "reflectance", "wavelength": [0.658, 0.668, 0.678], "chan_solar_index": 2}
        )
