...
DEFAULT_FILE_DATA = np.arange(DEFAULT_FILE_SHAPE[0] * DEFAULT_FILE_SHAPE[1], dtype=DEFAULT_FILE_DTYPE).reshape(
    DEFAULT_FILE_SHAPE
)
...
class FakeHDF5FileHandler2(FakeHDF5FileHandler):
    ...
    def _add_basic_metadata_to_file_content(file_content, filename_info, num_grans):
        ...
        end_time = filename_info["end_time"].replace(year=start_time.year, month=start_time.month, day=start_time.day)
    ...
    def _add_granule_specific_info_to_file_content(
        self, file_content, dataset_group, num_granules, num_scans_per_granule, gran_group_prefix
    ):
        ...
        for granule_idx in range(num_granules):
            prefix_gran = "{prefix}/{dataset_group}_Gran_{idx}".format(
                prefix=gran_group_prefix, dataset_group=dataset_group, idx=granule_idx
            )
    ...
    def _get_per_granule_lons():
        return [
            np.array(
                [50.51393, 49.566296, 48.865967, 18.96082, -4.0238385, -7.05221, -10.405702, 14.638646],
                dtype=np.float32,
            ),
            np.array(
                [53.52594, 51.685738, 50.439102, 14.629087, -10.247547, -13.951393, -18.256989, 8.36572],
                dtype=np.float32,
            ),
            np.array(
                [59.386833, 55.770416, 53.38952, 8.353765, -18.062435, -22.608992, -27.867302, -1.3537619],
                dtype=np.float32,
            ),
            np.array(
                [72.50243, 64.17125, 59.15234, -1.3654504, -27.620953, -33.091743, -39.28113, -17.749891],
                dtype=np.float32,
            ),
        ]
    ...
    def _get_per_granule_lats():
        return [
            np.array(
                [67.969505, 65.545685, 63.103046, 61.853905, 55.169273, 57.062447, 58.86063, 66.495514],
                dtype=np.float32,
            ),
            np.array(
                [72.74879, 70.2493, 67.84738, 66.49691, 58.77254, 60.465942, 62.11525, 71.08249], dtype=np.float32
            ),
            np.array(
                [77.393425, 74.977875, 72.62976, 71.083435, 62.036346, 63.465122, 64.78075, 75.36842], dtype=np.float32
            ),
            np.array(
                [81.67615, 79.49934, 77.278656, 75.369415, 64.72178, 65.78417, 66.66166, 79.00025], dtype=np.float32
            ),
        ]
    ...
    def _add_data_info_to_file_content(self, file_content, filename, data_var_prefix, num_grans):
        ...
        for k in keys:
            ...
            if self.include_factors:
                file_content[k + "Factors"] = np.repeat(DEFAULT_FILE_FACTORS.copy()[None, :], num_grans, axis=0).ravel()
    ...
    def _add_geolocation_info_to_file_content(file_content, filename, data_var_prefix, num_grans):
        ...
        angles = ["SolarZenithAngle", "SolarAzimuthAngle", "SatelliteZenithAngle", "SatelliteAzimuthAngle"]
        if is_dnb:
    ...
    def get_test_content(self, filename, filename_info, filetype_info):
        ...
        for dataset in self.datasets:
            ...
            self._add_granule_specific_info_to_file_content(
                file_content, dataset_group, self._num_test_granules, self._num_scans_per_gran, prefix1
            )
            ...
            if filename[:3] in ["SVM", "SVI", "SVD"]:
                self._add_data_info_to_file_content(file_content, filename, prefix3, self._num_test_granules)
            elif filename[0] == "G":
                self._add_geolocation_info_to_file_content(file_content, filename, prefix3, self._num_test_granules)
...
class TestVIIRSSDRReader(unittest.TestCase):
    ...
    def test_init_start_time_beyond(self):
        ...
        r = load_reader(self.reader_configs, filter_parameters={"start_time": datetime(2012, 2, 26)})
    ...
    def test_init_end_time_beyond(self):
        ...
        r = load_reader(self.reader_configs, filter_parameters={"end_time": datetime(2012, 2, 24)})
        fhs = r.create_filehandlers(
            [
                "SVI01_npp_d20120225_t1801245_e1802487_b01708_c20120226002130255476_noaa_ops.h5",
            ]
    ...
    def test_init_start_end_time(self):
        ...
        r = load_reader(
            self.reader_configs,
            filter_parameters={"start_time": datetime(2012, 2, 24), "end_time": datetime(2012, 2, 26)},
        )
        loadables = r.select_files_from_pathnames(
            [
                "SVI01_npp_d20120225_t1801245_e1802487_b01708_c20120226002130255476_noaa_ops.h5",
