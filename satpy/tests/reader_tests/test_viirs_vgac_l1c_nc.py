...
class TestVGACREader:
    ...
    def test_read_vgac(self, _nc_filename):
        ...
        # Read data
        scn_ = Scene(reader="viirs_vgac_l1c_nc", filenames=[_nc_filename])
        scn_.load(["M05", "M15"])
        assert scn_["M05"][0, 0] == 100
        assert scn_["M15"][0, 0] == 400
        assert scn_.start_time == datetime.datetime(year=2023, month=3, day=28, hour=9, minute=8, second=7)
        assert scn_.end_time == datetime.datetime(year=2023, month=3, day=28, hour=10, minute=11, second=12)
