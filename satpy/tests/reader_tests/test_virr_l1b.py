...
class FakeHDF5FileHandler2(FakeHDF5FileHandler):
    ...
    def _make_file(self, platform_id, geolocation_prefix, l1b_prefix, ECWN, Emissive_units):
        ...
        test_file = {
            # Satellite data.
            "/attr/Day Or Night Flag": "D",
            "/attr/Observing Beginning Date": "2018-12-25",
            "/attr/Observing Beginning Time": "21:41:47.090",
            "/attr/Observing Ending Date": "2018-12-25",
            "/attr/Observing Ending Time": "21:47:28.254",
            "/attr/Satellite Name": platform_id,
            ...
            l1b_prefix + "EV_RefSB/attr/valid_range": [0, 32767],
            l1b_prefix + "EV_RefSB/attr/units": "none",
            "/attr/RefSB_Cal_Coefficients": np.ones(14, dtype=np.float32) * 2,
        }
        for attribute in [
            "Latitude",
            "Longitude",
            geolocation_prefix + "SolarZenith",
            geolocation_prefix + "SensorZenith",
            geolocation_prefix + "SolarAzimuth",
            geolocation_prefix + "SensorAzimuth",
        ]:
            test_file[attribute] = self.make_test_data([dim_0, dim_1])
            test_file[attribute + "/attr/Intercept"] = 0.0
            test_file[attribute + "/attr/units"] = "degrees"
            if "Solar" in attribute or "Sensor" in attribute:
                test_file[attribute + "/attr/Slope"] = 0.01
                if "Azimuth" in attribute:
                    test_file[attribute + "/attr/valid_range"] = [0, 18000]
                else:
                    test_file[attribute + "/attr/valid_range"] = [-18000, 18000]
            else:
                test_file[attribute + "/attr/Slope"] = 1.0
                if "Longitude" == attribute:
                    test_file[attribute + "/attr/valid_range"] = [-180.0, 180.0]
                else:
                    test_file[attribute + "/attr/valid_range"] = [-90.0, 90.0]
        return test_file
    ...
    def get_test_content(self, filename, filename_info, filetype_info):
        ...
        return self._make_file(
            filename_info["platform_id"], "Geolocation/", "Data/", "Emissive_Centroid_Wave_Number", "none"
        )
...
class TestVIRRL1BReader(unittest.TestCase):
    ...
    def _fy3_helper(self, platform_name, reader, Emissive_units):
        ...
        band_values = {
            ...
            "satellite_zenith_angle": 0.1,
            "solar_azimuth_angle": 0.1,
            "satellite_azimuth_angle": 0.1,
            "longitude": 10,
        }
        ...
        for dataset in datasets:
            ...
            if dataset["name"] in ["1", "2", "6", "7", "8", "9", "10"]:
                ...
                self.assertTrue(
                    ...
                    in ["solar_zenith_angle", "sensor_zenith_angle", "solar_azimuth_angle", "sensor_azimuth_angle"]
                )
                self.assertEqual(["virr_geoxx", "virr_l1b"], attributes["file_type"])
                self.assertEqual(("longitude", "latitude"), attributes["coordinates"])
            self.assertEqual(
                band_values[dataset["name"]], round(float(np.array(ds[ds.shape[0] // 2][ds.shape[1] // 2])), 6)
            )
    ...
    def test_fy3c_file(self):
        ...
        FY3C_files = FY3C_reader.select_files_from_pathnames(
            ["tf2018359143912.FY3C-L_VIRRX_GEOXX.HDF", "tf2018359143912.FY3C-L_VIRRX_L1B.HDF"]
        )
