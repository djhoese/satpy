...
class TestDataAccessMethods:
    ...
    def test_getitem_modifiers(self):
        ...
        scene[make_dataid(name="1", modifiers=("mod1",))] = xr.DataArray(np.arange(5))
        ...
        scene["1"] = ds1_m0 = xr.DataArray(np.arange(5))
        scene[make_dataid(name="1", modifiers=("mod1",))] = xr.DataArray(np.arange(5))
        scene[make_dataid(name="1", modifiers=("mod1", "mod2"))] = xr.DataArray(np.arange(5))
        assert scene["1"] is ds1_m0
        assert len(list(scene.keys())) == 3

        scene = Scene()
        scene[make_dataid(name="1", modifiers=("mod1", "mod2"))] = ds1_m2 = xr.DataArray(np.arange(5))
        scene[make_dataid(name="1", modifiers=("mod1",))] = ds1_m1 = xr.DataArray(np.arange(5))
        assert scene["1"] is ds1_m1
        assert scene[make_dataid(name="1", modifiers=("mod1", "mod2"))] is ds1_m2
        pytest.raises(KeyError, scene.__getitem__, make_dataid(name="1", modifiers=tuple()))
        assert len(list(scene.keys())) == 2
    ...
    def test_getitem_slices(self):
        ...
        scene1["2"] = scene2["2"] = xr.DataArray(np.zeros((5, 10)), dims=("y", "x"))
        scene1["3"] = xr.DataArray(np.zeros((5, 10)), dims=("y", "x"), attrs={"area": area_def})
        anc_vars = [xr.DataArray(np.ones((5, 10)), attrs={"name": "anc_var", "area": area_def})]
        attrs = {"ancillary_variables": anc_vars, "area": area_def}
        scene1["3a"] = xr.DataArray(np.zeros((5, 10)), dims=("y", "x"), attrs=attrs)
        scene2["4"] = xr.DataArray(np.zeros((5, 10)), dims=("y", "x"), attrs={"area": swath_def})
        anc_vars = [xr.DataArray(np.ones((5, 10)), attrs={"name": "anc_var", "area": swath_def})]
        attrs = {"ancillary_variables": anc_vars, "area": swath_def}
        scene2["4a"] = xr.DataArray(np.zeros((5, 10)), dims=("y", "x"), attrs=attrs)
    ...
    def test_contains(self):
        ...
        scene["1"] = xr.DataArray(
            np.arange(5), attrs={"wavelength": (0.1, 0.2, 0.3), "_satpy_id_keys": default_id_keys_config}
        )
        assert "1" in scene
    ...
    def test_delitem(self):
        ...
        scene["2"] = xr.DataArray(
            np.arange(5), attrs={"wavelength": (0.4, 0.5, 0.6), "_satpy_id_keys": default_id_keys_config}
        )
        scene["3"] = xr.DataArray(
            np.arange(5), attrs={"wavelength": (0.7, 0.8, 0.9), "_satpy_id_keys": default_id_keys_config}
        )
        del scene["1"]
        del scene["3"]
        del scene[0.45]
