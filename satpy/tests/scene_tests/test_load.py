...
class TestSceneAllAvailableDatasets:
    ...
    def test_available_dataset_no_readers(self):
        ...
        scene = Scene()
        pytest.raises(KeyError, scene.available_dataset_ids, reader_name="fake")
    ...
    def test_available_dataset_names_no_readers(self):
        ...
        scene = Scene()
        pytest.raises(KeyError, scene.available_dataset_names, reader_name="fake")
    ...
    def test_all_datasets_multiple_reader(self):
        ...
        scene = Scene(filenames={"fake1_1ds": ["fake1_1ds_1.txt"], "fake2_1ds": ["fake2_1ds_1.txt"]})
        id_list = scene.all_dataset_ids()
    ...
    def test_available_datasets_one_reader(self):
        ...
        scene = Scene(filenames=["fake1_1ds_1.txt"], reader="fake1_1ds")
    ...
    def test_available_composites_known_versus_all(self):
        ...
        scene = Scene(filenames=["fake1_1.txt"], reader="fake1", reader_kwargs={"not_available": ["ds2", "ds3"]})
...
class TestLoadingReaderDatasets:
    ...
    def test_load_no_exist2(self):
        ...
        with mock.patch.object(FileYAMLReader, "load", load_mock):
            ...
            lmock.assert_called_once_with({make_dataid(name="ds9_fail_load", wavelength=(1.0, 1.1, 1.2))})
    ...
    def test_load_ds1_load_twice(self):
        ...
        with mock.patch.object(FileYAMLReader, "load", load_mock):
            ...
            loaded_ids = list(scene._datasets.keys())
            assert len(loaded_ids) == 1
            assert loaded_ids[0] == make_dataid(
                name="ds1", resolution=250, calibration="reflectance", modifiers=tuple()
            )
            assert not lmock.called, "Reader.load was called again when loading something that's already loaded"

    def test_load_ds1_unknown_modifier(self):
        """Test loading one dataset with no loaded compositors."""
        scene = Scene(filenames=["fake1_1.txt"], reader="fake1")
        pytest.raises(KeyError, scene.load, [make_dataid(name="ds1", modifiers=("_fake_bad_mod_",))])
...
class TestLoadingComposites:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "comp12", make_cid(name="comp12"), id="composite with required prereqs as modified wavelengths"
            ),
            pytest.param("comp13", make_cid(name="comp13"), id="composite with modified res-changed prereq"),
            pytest.param(
                "comp14", make_cid(name="comp14", resolution=555), id="composite that changes DataID resolution"
            ),
            ...
            pytest.param(
                "comp21", make_cid(name="comp21"), id="composite with prereq with modifier with unloadable opt prereq"
            ),
            pytest.param(
                "comp22", make_cid(name="comp22"), id="composite with prereq with modifier with only opt prereqs"
            ),
    ...
    def test_load_comp18(self):
        ...
        assert make_dataid(name="ds4", calibration="reflectance", modifiers=("mod1", "mod3")) in scene._datasets
        assert make_dataid(name="ds5", resolution=250, modifiers=("mod1",)) in scene._datasets
    ...
    def test_load_multiple_comps(self):
        ...
        scene = Scene(filenames=["fake1_1.txt"], reader="fake1")
        scene.load(["comp1", "comp2", "comp3", "comp4", "comp5", "comp6", "comp7", "comp9", "comp10"])
        loaded_ids = list(scene._datasets.keys())
    ...
    def test_load_dataset_after_composite(self):
        ...
        with (
            mock.patch.object(FileYAMLReader, "load", load_mock),
    ...
    def test_load_dataset_after_composite2(self):
        ...
        with (
            ...
            with mock.patch.object(
                ...
                wraps=scene._generate_composites_nodes_from_loaded_datasets,
            ) as m:
                scene.load(["ds1"])
                assert lmock.call_count == 2
                loaded_ids = list(scene._datasets.keys())
                ...
                assert (
                    make_dataid(name="ds1", resolution=250, calibration="reflectance", modifiers=tuple()) in loaded_ids
                )
