...
class TestSceneCrop:
    ...
    def test_crop(self):
        ...
        scene1["3"] = xr.DataArray(np.zeros((y_size, x_size)), dims=("y", "x"), attrs={"area": area_def})
        scene1["4"] = xr.DataArray(np.zeros((y_size // 2, x_size // 2)), dims=("y", "x"), attrs={"area": area_def2})
        ...
        crop_area = AreaDefinition(
            ...
            x_size,
            y_size,
            (area_extent[0] + 10000.0, area_extent[1] + 500000.0, area_extent[2] - 10000.0, area_extent[3] - 500000.0),
        )
        ...
        # by lon/lat bbox
        new_scn1 = scene1.crop(ll_bbox=(-20.0, -5.0, 0, 0))
        assert "1" in new_scn1
        assert "2" in new_scn1
        assert "3" in new_scn1
        assert new_scn1["1"].shape == (y_size, x_size)
        ...
        # by x/y bbox
        new_scn1 = scene1.crop(xy_bbox=(-200000.0, -100000.0, 0, 0))
        assert "1" in new_scn1
        assert "2" in new_scn1
        assert "3" in new_scn1
        assert new_scn1["1"].shape == (y_size, x_size)
    ...
    def test_crop_epsg_crs(self):
        ...
        scene1["1"] = xr.DataArray(np.zeros((y_size, x_size)), dims=("y", "x"), attrs={"area": area_def})
        # by x/y bbox
    ...
    def test_crop_rgb(self):
        ...
        scene1["1"] = xr.DataArray(np.zeros((3, y_size, x_size)), dims=("bands", "y", "x"), attrs={"area": area_def})
        scene1["2"] = xr.DataArray(
            np.zeros((y_size // 2, 3, x_size // 2)), dims=("y", "bands", "x"), attrs={"area": area_def2}
        )
...
class TestSceneResampling:
    ...
    def test_resample_reduce_data_toggle(self, rs):
        ...
        with (
            mock.patch("satpy.scene.Scene._slice_data") as slice_data,
    ...
    def test_resample_multi_ancillary(self):
        ...
        anc_vars = [
            xr.DataArray(np.arange(n * n).reshape(n, n) * i, dims=("y", "x"), attrs={"name": f"anc{i:d}", "area": ar})
            for i in range(2)
        ]
        sc["test"] = xr.DataArray(
            np.arange(n * n).reshape(n, n),
            dims=("y", "x"),
            attrs={"area": ar, "name": "test", "ancillary_variables": anc_vars},
        )
...
class TestSceneAggregation:
    ...
    def _create_test_data(x_size, y_size):
        ...
        scene1["1"] = xr.DataArray(np.ones((y_size, x_size)), attrs={"_satpy_id_keys": default_id_keys_config})
        scene1["2"] = xr.DataArray(
            np.ones((y_size, x_size)), dims=("y", "x"), attrs={"_satpy_id_keys": default_id_keys_config}
        )
        scene1["3"] = xr.DataArray(
            np.ones((y_size, x_size)),
            dims=("y", "x"),
            attrs={"area": area_def, "_satpy_id_keys": default_id_keys_config},
        )
        scene1["4"] = xr.DataArray(
            np.ones((y_size, x_size)),
            dims=("y", "x"),
            attrs={"area": area_def, "standard_name": "backscatter", "_satpy_id_keys": default_id_keys_config},
        )
        return scene1
