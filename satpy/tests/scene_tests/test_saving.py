...
class TestSceneSaving:
    ...
    def test_save_datasets_bad_writer(self, tmp_path):
        ...
        ds1 = xr.DataArray(
            da.zeros((100, 200), chunks=50), dims=("y", "x"), attrs={"name": "test", "start_time": datetime.utcnow()}
        )
        scn = Scene()
        scn["test"] = ds1
        pytest.raises(ValueError, scn.save_datasets, writer="_bad_writer_", base_dir=tmp_path)
    ...
    def test_save_datasets_missing_wishlist(self, tmp_path):
        ...
        pytest.raises(RuntimeError, scn.save_datasets, writer="geotiff", base_dir=tmp_path)
        pytest.raises(KeyError, scn.save_datasets, datasets=["no_exist"])
