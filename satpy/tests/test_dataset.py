...
class TestCombineMetadata(unittest.TestCase):
    ...
    def test_combine_numpy_arrays(self):
        ...
        test_metadata = [
            {"valid_range": np.array([0.0, 0.00032], dtype=np.float32)},
            {"valid_range": np.array([0.0, 0.00032], dtype=np.float32)},
            {"valid_range": np.array([0.0, 0.00032], dtype=np.float32)},
        ]
        result = combine_metadata(*test_metadata)
        assert np.allclose(result["valid_range"], np.array([0.0, 0.00032], dtype=np.float32))
    ...
    def test_combine_dask_arrays(self):
        ...
        test_metadata = [
            {"valid_range": da.from_array(np.array([0.0, 0.00032], dtype=np.float32))},
            {"valid_range": da.from_array(np.array([0.0, 0.00032], dtype=np.float32))},
        ]
        result = combine_metadata(*test_metadata)
    ...
    def test_combine_real_world_mda(self):
        ...
        mda_objects = (
            {
                "_FillValue": np.nan,
                "valid_range": np.array([0.0, 0.00032], dtype=np.float32),
                "ancillary_variables": ["cpp_status_flag", "cpp_conditions", "cpp_quality", "cpp_reff_pal", "-"],
        ...
        np.testing.assert_equal(result.pop("raw_metadata"), expected.pop("raw_metadata"))
...
def test_combine_dicts_close():
    ...
    attrs_close = {
        "raw_metadata": {
            ...
            "d": {"e": np.str_("bar"), "f": datetime(2020, 1, 1, 12, 15, 30), "g": np.array([1, 2, 3]) + 1e-12},
            "h": np.array([datetime(2020, 1, 1), datetime(2020, 1, 1)]),
        }
    }
...
def test_dataid_equal_if_enums_different():
    ...
    id_keys_config2 = {
        ...
        "resolution": None,
        "calibration": {"enum": ["c1", "c1.5", "c2", "c2.5", "c3"]},
        "modifiers": {
            "default": ModifierTuple(),
            "type": ModifierTuple,
        },
...
class TestIDQueryInteractions(unittest.TestCase):
    ...
    def test_id_filtering(self):
        ...
        dataid_container = [
            DataID(
                self.default_id_keys_config, name="ds1", resolution=250, calibration="reflectance", modifiers=tuple()
            )
        ]
        dq = DataQuery(wavelength=0.22, modifiers=tuple())
        assert len(dq.filter_dataids(dataid_container)) == 0
        dataid_container = [DataID(minimal_default_keys_config, name="natural_color")]
    ...
    def test_sort_dataids_with_different_set_of_keys(self):
        ...
        dids = [
            DataID(self.default_id_keys_config, name="solar_zenith_angle", resolution=1000, modifiers=()),
    ...
    def test_seviri_hrv_has_priority_over_vis008(self):
        ...
        dids = [
            DataID(
                self.default_id_keys_config,
                name="HRV",
                wavelength=WavelengthRange(min=0.5, central=0.7, max=0.9, unit="µm"),
...
def test_frequency_quadruple_side_band_channel_distances():
    ...
    frq_dict = {"central": 57, "side": 0.322, "sideside": 0.05, "bandwidth": 0.036, "unit": "GHz"}
