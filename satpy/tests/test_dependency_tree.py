...
class TestDependencyTree(unittest.TestCase):
    ...
    def test_copy_preserves_all_nodes(self):
        ...
        assert self._nodes_equal(self.dependency_tree.leaves(), new_dependency_tree.leaves())
        assert self._nodes_equal(self.dependency_tree.trunk(), new_dependency_tree.trunk())

        # make sure that we can get access to sub-nodes
        c13_id = make_cid(name="comp13")
        assert self._nodes_equal(
            self.dependency_tree.trunk(limit_nodes_to=[c13_id]), new_dependency_tree.trunk(limit_nodes_to=[c13_id])
        )
    ...
    def test_copy_preserves_unique_empty_node(self):
        ...
        self.assertIs(self.dependency_tree._root.children[0].children[0].children[1], self.dependency_tree.empty_node)
        self.assertIs(new_dependency_tree._root.children[0].children[0].children[1], self.dependency_tree.empty_node)
...
class TestMultipleResolutionSameChannelDependency(unittest.TestCase):
    ...
    def test_modis_overview_1000m(self):
        ...
        overview = {
            "_satpy_id": make_dataid(name="overview"),
            "name": "overview",
            "optional_prerequisites": [],
            "prerequisites": [
                DataQuery(name="1", modifiers=("sunz_corrected",)),
        ...
        modifiers = {
            "modis": {
                "sunz_corrected": (
                    SunZenithCorrector,
                    {"optional_prerequisites": ["solar_zenith_angle"], "name": "sunz_corrected", "prerequisites": []},
                )
            }
        }
