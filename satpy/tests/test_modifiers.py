...
def _sunz_bigger_area_def():
    ...
    bigger_area = AreaDefinition("test", "test", "test", {"proj": "merc"}, 4, 4, (-2000, -2000, 2000, 2000))
    return bigger_area


def _sunz_stacked_area_def():
    """Get fake stacked area for testing sunz generation."""
    area1 = AreaDefinition("test", "test", "test", {"proj": "merc"}, 2, 1, (-2000, 0, 2000, 2000))
    area2 = AreaDefinition("test", "test", "test", {"proj": "merc"}, 2, 1, (-2000, -2000, 2000, 0))
    return StackedAreaDefinition(area1, area2)


def _shared_sunz_attrs(area_def):
    attrs = {"area": area_def, "start_time": datetime(2018, 1, 1, 18), "modifiers": tuple(), "name": "test_vis"}
    return attrs
...
def sunz_sza():
    ...
    sza = xr.DataArray(
        np.rad2deg(np.arccos(da.from_array([[0.0149581333, 0.0146694376], [0.0150812684, 0.0147925727]], chunks=2))),
...
class TestNIRReflectance(unittest.TestCase):
    ...
    def test_provide_sunz_no_co2(self, calculator, apply_modifier_info, sza):
        ...
        calculator.return_value = mock.MagicMock(reflectance_from_tbs=self.refl_from_tbs)
    ...
    def test_provide_sunz_and_threshold(self, calculator, apply_modifier_info, sza):
        ...
        calculator.assert_called_with(
            "Meteosat-11", "seviri", "IR_039", sunz_threshold=84.0, masking_limit=NIRReflectance.MASKING_LIMIT
        )
    ...
    def test_provide_masking_limit(self, calculator, apply_modifier_info, sza):
        ...
        calculator.assert_called_with(
            "Meteosat-11", "seviri", "IR_039", sunz_threshold=NIRReflectance.TERMINATOR_LIMIT, masking_limit=None
        )
...
class TestNIREmissivePartFromReflectance(unittest.TestCase):
    ...
    def test_compositor(self, calculator, apply_modifier_info, sza):
        ...
        calculator.assert_called_with(
            "NOAA-20", "viirs", "M12", sunz_threshold=86.0, masking_limit=NIRReflectance.MASKING_LIMIT
        )
...
class TestPSPRayleighReflectance:
    ...
    def test_rayleigh_corrector(
        ...
        ray_cor = PSPRayleighReflectance(
            name=name,
            atmosphere="us-standard",
            aerosol_types=aerosol_type,
            reduce_lim_low=reduce_lim_low,
            reduce_lim_high=reduce_lim_high,
