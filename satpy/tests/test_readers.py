...
local_id_keys_config = {
    ...
    "resolution": None,
    "calibration": {"enum": ["reflectance", "brightness_temperature", "radiance", "counts"]},
    "polarization": None,
    "level": None,
    "modifiers": {
        "required": True,
...
class TestDatasetDict(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.regular_dict = regular_dict = {
            make_dataid(name="test", wavelength=(0, 0.5, 1), resolution=1000): "1",
            make_dataid(name="testh", wavelength=(0, 0.5, 1), resolution=500): "1h",
            make_dataid(name="test2", wavelength=(1, 1.5, 2), resolution=1000): "2",
            make_dataid(name="test3", wavelength=(1.2, 1.7, 2.2), resolution=1000): "3",
            make_dataid(name="test4", calibration="radiance", polarization="V"): "4rad",
            make_dataid(name="test4", calibration="reflectance", polarization="H"): "4refl",
            make_dataid(name="test5", modifiers=("mod1", "mod2")): "5_2mod",
            make_dataid(name="test5", modifiers=("mod2",)): "5_1mod",
    ...
    def test_get_key(self):
        ...
        res2 = get_key(make_dataid(name="test4"), d, calibration="radiance", num_results=0)
        res3 = get_key(make_dataid(name="test4"), d, calibration="radiance", num_results=3)
        ...
        self.assertEqual(res1, make_dataid(name="test4", calibration="radiance", polarization="V"))

        res1 = get_key(0.5, d, query=DataQuery(resolution=500))
        self.assertEqual(res1, make_dataid(name="testh", wavelength=(0, 0.5, 1), resolution=500))
    ...
    def test_keys(self):
        ...
        self.assertListEqual(sorted(set(name_keys))[:4], ["test", "test2", "test3", "test4"])
...
class TestReaderLoader(unittest.TestCase):
    ...
    def test_filenames_as_dict_bad_reader(self):
        ...
        self.assertRaisesRegex(
            ValueError, r"(?=.*__fake__)(?!.*viirs)(^No reader.+)", load_readers, filenames=filenames
        )
    ...
    def test_missing_requirements(self, *mocks):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                self.fail("If at least one set of filenames is complete, no exception should be raised")
    ...
    def test_all_filtered(self):
        ...
        self.assertRaises(
            ValueError, load_readers, filenames=filenames, reader_kwargs={"filter_parameters": filter_params}
        )
    ...
    def test_all_filtered_multiple(self):
        ...
        filter_params = {"start_time": datetime.datetime(1970, 1, 1), "end_time": datetime.datetime(1970, 1, 2)}
        self.assertRaises(
            ValueError, load_readers, filenames=filenames, reader_kwargs={"filter_parameters": filter_params}
        )
    ...
    def test_almost_all_filtered(self):
        ...
        filter_params = {"start_time": datetime.datetime(2012, 2, 25), "end_time": datetime.datetime(2012, 2, 26)}
...
class TestFindFilesAndReaders:
    ...
    def test_reader_name_unmatched_start_end_time(self, viirs_file):
        ...
        with pytest.raises(ValueError):
            find_files_and_readers(
                reader="viirs_sdr", start_time=datetime(2012, 2, 26, 18, 0, 0), end_time=datetime(2012, 2, 26, 19, 0, 0)
            )
    ...
    def test_pending_old_reader_name_mapping(self):
        ...
        if not PENDING_OLD_READER_NAMES:
            return unittest.skip("Skipping pending deprecated reader tests because no pending deprecated readers.")
    ...
    def test_old_reader_name_mapping(self):
        ...
        if not OLD_READER_NAMES:
            return pytest.skip("Skipping deprecated reader tests because no deprecated readers.")
...
class TestYAMLFiles:
    ...
    def test_filename_matches_reader_name(self):
        ...
        for reader_config in glob_config("readers/*.yaml"):
            ...
            assert (
                reader_fn_name == reader_info["name"]
...
class TestGroupFiles(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.unknown_files = ["ʌsɔ˙pıʃɐʌuı", "no such"]
    ...
    def test_two_instruments_files_split(self):
        ...
        groups = group_files(
            self.g16_files + self.g17_files, reader="abi_l1b", group_keys=("start_time", "platform_shortname")
        )
    ...
    def test_viirs_override_keys(self):
        ...
        groups = group_files(
            self.noaa20_files + self.npp_files,
            reader="viirs_sdr",
    ...
    def test_multi_readers(self):
        ...
        groups = group_files(self.g16_files + self.noaa20_files, reader=("abi_l1b", "viirs_sdr"))
    ...
    def test_multi_readers_empty_groups_raises_filenotfounderror(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="when grouping files, group at index 1 had no files for readers: abi_l1b"
        ):
            group_files(
