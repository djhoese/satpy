...
def get_test_data(input_shape=(100, 50), output_shape=(200, 100), output_proj=None, input_dims=("y", "x")):
    ...
    ds1 = DataArray(da.zeros(input_shape, chunks=85), dims=input_dims, attrs={"name": "test_data_name", "test": "test"})
    ...
    input_proj_str = "+proj=geos +lon_0=-95.0 +h=35786023.0 +a=6378137.0 +b=6356752.31414 +sweep=x +units=m +no_defs"
    ...
    ds2 = ds1.copy()
    input_area_shape = tuple(ds1.sizes[dim] for dim in ds1.dims if dim in ["y", "x"])
    ...
    swath_def = SwathDefinition(DataArray(lons, dims=geo_dims), DataArray(lats, dims=geo_dims))
    ...
    output_proj_str = "+proj=lcc +datum=WGS84 +ellps=WGS84 +lon_0=-95. +lat_0=25 +lat_1=25 +units=m +no_defs"
...
class TestHLResample(unittest.TestCase):
    ...
    def test_type_preserve(self):
        ...
        dest_area = SwathDefinition(
            xr.DataArray(da.arange(4, chunks=5).reshape((2, 2)) + 0.0001, dims=["y", "x"]),
            xr.DataArray(da.arange(4, chunks=5).reshape((2, 2)) + 0.0001, dims=["y", "x"]),
        )
...
class TestKDTreeResampler(unittest.TestCase):
    ...
    def test_kd_resampling(self, xr_resampler, create_filename, zarr_open, xr_dset, cnc):
        ...
        resampler.precompute(mask=da.arange(5, chunks=5).astype(bool), cache_dir=".")
        ...
        try:
            ...
            zarr_open.return_value = FakeZarr(
                valid_input_index=1, valid_output_index=2, index_array=3, distance_array=4
            )
            resampler.precompute(cache_dir=the_dir)
    ...
    def test_check_numpy_cache(self, xr_Dataset, np_load):
        ...
        try:
            the_dir = tempfile.mkdtemp()
            kwargs = {}
            np_path = resampler._create_cache_filename(the_dir, prefix="resample_lut-", fmt=".npz", mask=None, **kwargs)
...
@unittest.skipIf(
    LegacyDaskEWAResampler is not None, "Deprecated EWA resampler is now in pyresample. No need to test in Satpy."
)
class TestEWAResampler(unittest.TestCase):
    ...
    def test_2d_ewa(self, get_lonlats, ll2cr, fornav):
        ...
        ll2cr.return_value = (100, np.zeros((10, 10), dtype=np.float32), np.zeros((10, 10), dtype=np.float32))
        fornav.return_value = (100 * 200, np.zeros((200, 100), dtype=np.float32))
        ...
        data = xr.DataArray(
            swath_data.data, dims=("y", "x"), attrs={"area": source_swath, "test": "test2", "name": "test2"}
        )
    ...
    def test_3d_ewa(self, get_lonlats, ll2cr, fornav):
        ...
        swath_data.data = swath_data.data.astype(np.float32)
        ll2cr.return_value = (100, np.zeros((10, 10), dtype=np.float32), np.zeros((10, 10), dtype=np.float32))
        fornav.return_value = ([100 * 200] * 3, [np.zeros((200, 100), dtype=np.float32)] * 3)
        get_lonlats.return_value = (source_swath.lons, source_swath.lats)
        num_chunks = len(source_swath.lons.chunks[0]) * len(source_swath.lons.chunks[1])
...
class TestNativeResampler:
    ...
    def test_expand_reduce_replicate(self):
        ...
        new_data = NativeResampler._expand_reduce(self.d_arr, {0: 2.0, 1: 2.0})
        assert new_data.shape == (12, 40)

    def test_expand_reduce_aggregate(self):
        """Test classmethod 'expand_reduce' to aggregate by half."""
        new_data = NativeResampler._expand_reduce(self.d_arr, {0: 0.5, 1: 0.5})
    ...
    def test_expand_reduce_aggregate_identity(self):
        ...
        new_data = NativeResampler._expand_reduce(self.d_arr, {0: 1.0, 1: 1.0})
        assert new_data.shape == (6, 20)
        assert new_data is self.d_arr

    @pytest.mark.parametrize("dim0_factor", [1.0 / 4, 0.333323423, 1.333323423])
    def test_expand_reduce_aggregate_invalid(self, dim0_factor):
        ...
        with pytest.raises(ValueError):
            NativeResampler._expand_reduce(self.d_arr, {0: dim0_factor, 1: 1.0})
    ...
    def test_expand_reduce_numpy(self):
        ...
        n_arr = np.zeros((6, 20))
        new_data = NativeResampler._expand_reduce(n_arr, {0: 2.0, 1: 1.0})
    ...
    def test_expand_dims_3d(self):
        ...
        ds1, source_area, _, _, target_area = get_test_data(input_shape=(3, 100, 50), input_dims=("bands", "y", "x"))
        # source geo def doesn't actually matter
        resampler = NativeResampler(source_area, target_area)
        new_data = resampler.resample(ds1)
    ...
    def test_expand_without_dims_4D(self):
        ...
        ds1, source_area, _, _, target_area = get_test_data(input_shape=(2, 3, 100, 50), input_dims=None)
        # source geo def doesn't actually matter
        resampler = NativeResampler(source_area, target_area)
        with pytest.raises(ValueError):
...
class TestBilinearResampler(unittest.TestCase):
    ...
    def test_bil_resampling(self, xr_resampler, create_filename, move_existing_caches):
        """Test the bilinear resampler."""
        ...
        resampler.precompute(mask=da.arange(5, chunks=5).astype(bool))
        ...
        fill_value = 8
        resampler.resampler.get_sample_from_bil_info.return_value = xr.DataArray(
            da.zeros(target_area.shape), dims=("y", "x")
        )
    ...
    def test_move_existing_caches(self):
        ...
        try:
            ...
            self.assertTrue(os.path.exists(os.path.join(the_dir, "moved_by_satpy", "test.zarr")))
...
class TestCoordinateHelpers(unittest.TestCase):
    ...
    def test_area_def_coordinates(self):
        ...
        area_def = AreaDefinition(
            "test", "test", "test", {"proj": "lcc", "lat_1": 25, "lat_0": 25}, 100, 200, [-100, -100, 100, 100]
        )
        ...
        # lat/lon area
        area_def = AreaDefinition("test", "test", "test", {"proj": "latlong"}, 100, 200, [-100, -100, 100, 100])
        data_arr = xr.DataArray(
            da.zeros((200, 100), chunks=50),
            attrs={"area": area_def},
            dims=("y", "x"),
        ...
        self.assertIn("units", new_data_arr.coords["y"].attrs)
        self.assertEqual(new_data_arr.coords["y"].attrs["units"], "degrees_north")
        self.assertIn("units", new_data_arr.coords["x"].attrs)
        self.assertEqual(new_data_arr.coords["x"].attrs["units"], "degrees_east")
        self.assertIn("crs", new_data_arr.coords)
        self.assertIsInstance(new_data_arr.coords["crs"].item(), CRS)
        self.assertEqual(area_def.crs, new_data_arr.coords["crs"].item())
    ...
    def test_swath_def_coordinates(self):
        ...
        lons = xr.DataArray(lons_data, attrs={"units": "degrees_east"}, dims=("y", "x"))
        lats = xr.DataArray(lats_data, attrs={"units": "degrees_north"}, dims=("y", "x"))
...
class TestBucketAvg(unittest.TestCase):
    ...
    def test_compute_and_use_skipna_handling(self):
        ...
        self.bucket.resampler.get_average.assert_called_once_with(data, fill_value=2, skipna=True)

        self._compute_mocked_bucket_avg(data, fill_value=2, skipna=False)
        self.bucket.resampler.get_average.assert_called_once_with(data, fill_value=2, skipna=False)
    ...
    def test_compute_and_not_use_skipna_handling(self):
        ...
        self._compute_mocked_bucket_avg(data, fill_value=2, mask_all_nan=True)
        self.bucket.resampler.get_average.assert_called_once_with(data, fill_value=2, mask_all_nan=True)

        self._compute_mocked_bucket_avg(data, fill_value=2, mask_all_nan=False)
        self.bucket.resampler.get_average.assert_called_once_with(data, fill_value=2, mask_all_nan=False)
    ...
    def test_resample(self, pyresample_bucket):
        ...
        data = xr.DataArray(da.ones((1, 5, 5)), dims=("bands", "foo", "bar"), coords={"bands": ["L"]})
...
class TestBucketSum(unittest.TestCase):
    ...
    def test_compute_and_use_skipna_handling(self):
        ...
        self.bucket.resampler.get_sum.assert_called_once_with(data, skipna=True)

        self._compute_mocked_bucket_sum(data, skipna=False)
        self.bucket.resampler.get_sum.assert_called_once_with(data, skipna=False)
    ...
    def test_compute_and_not_use_skipna_handling(self):
        ...
        self._compute_mocked_bucket_sum(data, mask_all_nan=True)
        self.bucket.resampler.get_sum.assert_called_once_with(data, mask_all_nan=True)

        self._compute_mocked_bucket_sum(data, mask_all_nan=False)
        self.bucket.resampler.get_sum.assert_called_once_with(data, mask_all_nan=False)

        self._compute_mocked_bucket_sum(data)
        self.bucket.resampler.get_sum.assert_called_once_with(data, mask_all_nan=False)

        self._compute_mocked_bucket_sum(data, fill_value=2, skipna=True)
        self.bucket.resampler.get_sum.assert_called_once_with(data, fill_value=2, mask_all_nan=False)
...
class TestBucketFraction(unittest.TestCase):
    ...
    def test_compute(self):
        ...
        self.bucket.resampler.get_fractions.assert_called_with(data, categories=None, fill_value=np.nan)
        # Custom kwargs
        _ = self.bucket.compute(data, categories=[1, 2], fill_value=0)
        self.bucket.resampler.get_fractions.assert_called_with(data, categories=[1, 2], fill_value=0)
