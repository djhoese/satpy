...
class TestWritersModule(unittest.TestCase):
    ...
    def test_to_image_2d(self, mock_geoimage):
        ...
        p = xr.DataArray(data, attrs=dict(mode="L", fill_value=0, palette=[0, 1, 2, 3, 4, 5]), dims=["y", "x"])
        to_image(p)

        np.testing.assert_array_equal(data, mock_geoimage.call_args[0][0].values)
...
enhancements:
  ...
  exact_multisensor_comp:
    ...
    def test_multisensor_choice(self):
        ...
        ds = DataArray(
            np.arange(1, 11.0).reshape((2, 5)),
            attrs={"name": "test1", "sensor": {"test_sensor2", "test_sensor1"}, "mode": "L"},
            dims=["y", "x"],
        )
        e = Enhancer()
        ...
        assert set(e.sensor_enhancement_configs) == {os.path.abspath(self.ENH_FN), os.path.abspath(self.ENH_FN2)}
    ...
    def test_multisensor_exact(self):
        ...
        ds = DataArray(
            np.arange(1, 11.0).reshape((2, 5)),
            attrs={"name": "my_comp", "sensor": {"test_sensor2", "test_sensor1"}, "mode": "L"},
            dims=["y", "x"],
        )
        e = Enhancer()
        assert e.enhancement_tree is not None
...
enhancements:
  test1_kelvin:
    ...
    def test_enhance_empty_config(self):
        ...
        ds = DataArray(np.arange(1, 11.0).reshape((2, 5)), attrs=dict(sensor="test_empty", mode="L"), dims=["y", "x"])
        e = Enhancer()
        assert e.enhancement_tree is not None
        get_enhanced_image(ds, enhance=e)
        assert set(e.sensor_enhancement_configs) == {os.path.abspath(self.ENH_FN3)}
    ...
    def test_enhance_with_sensor_no_entry(self):
        ...
        ds = DataArray(np.arange(1, 11.0).reshape((2, 5)), attrs=dict(sensor="test_sensor2", mode="L"), dims=["y", "x"])
        e = Enhancer()
        assert e.enhancement_tree is not None
        get_enhanced_image(ds, enhance=e)
        assert set(e.sensor_enhancement_configs) == {os.path.abspath(self.ENH_FN2), os.path.abspath(self.ENH_ENH_FN2)}
    ...
    def test_writer_custom_enhance(self):
        ...
        writer.save_datasets((ds,), compute=False)
        img = writer.img
        np.testing.assert_almost_equal(img.data.isel(bands=0).max().values, 1.0)
    ...
    def test_enhance_with_sensor_entry(self):
        ...
        assert e.enhancement_tree is not None
        img = get_enhanced_image(ds, enhance=e)
        assert set(e.sensor_enhancement_configs) == {os.path.abspath(self.ENH_FN), os.path.abspath(self.ENH_ENH_FN)}
        np.testing.assert_almost_equal(img.data.isel(bands=0).max().values, 1.0)

        ds = DataArray(
            da.arange(1, 11.0, chunks=5).reshape((2, 5)),
            attrs=dict(name="test1", sensor="test_sensor", mode="L"),
            dims=["y", "x"],
        )
        e = Enhancer()
...
enhancements:
  ...
  test1_reader1_specific:
    ...
    def _get_enhanced_image(self, data_arr):
        ...
        assert set(e.sensor_enhancement_configs) == {os.path.abspath(self.ENH_FN)}
        return img
...
class TestYAMLFiles(unittest.TestCase):
    ...
    def test_filename_matches_writer_name(self):
        ...
        for writer_config in glob_config("writers/*.yaml"):
            ...
            writer_info = read_writer_config([writer_config], loader=IgnoreLoader)
            self.assertEqual(
                writer_fn_name,
                writer_info["name"],
                "Writer YAML filename doesn't match writer name in the YAML file.",
            )
...
class TestComputeWriterResults(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ds1 = xr.DataArray(
            ...
            dims=("y", "x"),
            attrs={"name": "test", "start_time": datetime(2018, 1, 1, 0, 0, 0)},
        )
        self.scn = Scene()
        self.scn["test"] = ds1
    ...
    def test_simple_image(self):
        ...
        res = self.scn.save_datasets(filename=fname, datasets=["test"], writer="simple_image", compute=False)
    ...
    def test_geotiff(self):
        ...
        res = self.scn.save_datasets(filename=fname, datasets=["test"], writer="geotiff", compute=False)
        compute_writer_results([res])
        self.assertTrue(os.path.isfile(fname))
    ...
    def test_multiple_geotiff(self):
        ...
        res1 = self.scn.save_datasets(filename=fname1, datasets=["test"], writer="geotiff", compute=False)
        fname2 = os.path.join(self.base_dir, "geotiff2.tif")
        res2 = self.scn.save_datasets(filename=fname2, datasets=["test"], writer="geotiff", compute=False)
    ...
    def test_multiple_simple(self):
        ...
        res1 = self.scn.save_datasets(filename=fname1, datasets=["test"], writer="simple_image", compute=False)
        fname2 = os.path.join(self.base_dir, "simple_image2.png")
        res2 = self.scn.save_datasets(filename=fname2, datasets=["test"], writer="simple_image", compute=False)
        compute_writer_results([res1, res2])
        self.assertTrue(os.path.isfile(fname1))
        self.assertTrue(os.path.isfile(fname2))
...
class TestOverlays(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.orig_rgb_img = XRImage(
            xr.DataArray(
                da.arange(75.0, chunks=10).reshape(3, 5, 5) / 75.0,
                dims=("bands", "y", "x"),
                coords={"bands": ["R", "G", "B"]},
        ...
        self.orig_l_img = XRImage(
            xr.DataArray(
                da.arange(25.0, chunks=10).reshape(5, 5) / 75.0,
                dims=("y", "x"),
                attrs={"name": "test_ds", "area": self.area_def},
            )
        )
        ...
        modules = {"pycoast": import_mock.pycoast, "pydecorate": import_mock.pydecorate}
    ...
    def test_add_overlay_basic_rgb(self):
        ...
        with mock.patch.object(self.orig_rgb_img, "apply_pil") as apply_pil:
            ...
            with mock.patch.object(self.orig_rgb_img, "convert") as convert:
                ...
                new_img = add_overlay(self.orig_rgb_img, self.area_def, coast_dir, overlays=overlays, fill_value=0)
                pil_args = None
                pil_kwargs = {"fill_value": 0}
                fun_args = (self.orig_rgb_img.data.area, ContourWriterAGG.return_value, overlays)
                fun_kwargs = None
                apply_pil.assert_called_with(
                    _burn_overlay, self.orig_rgb_img.mode, pil_args, pil_kwargs, fun_args, fun_kwargs
                )
                ...
                color = "red"
                expected_overlays = {
                    "coasts": {"outline": color, "width": 0.5, "level": 1},
                ...
                with warnings.catch_warnings(record=True) as wns:
                    ...
                    new_img = add_overlay(
                        self.orig_rgb_img, self.area_def, coast_dir, color=color, grid=grid, fill_value=0
                    )
                    assert len(wns) == 1
...
def test_group_results_by_output_file(tmp_path):
    ...
    x = 10
    dat = xr.DataArray(dims=("y", "x"), data=da.arange(x * x).reshape((x, x)))
