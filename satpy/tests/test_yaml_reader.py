...
MHS_YAML_READER_DICT = {
    ...
    "datasets": {
        ...
        "3": {
            "name": "3",
            "frequency_double_sideband": {"unit": "GHz", "central": 183.31, "side": 1.0, "bandwidth": 1.0},
            "polarization": "V",
            "resolution": 16000,
            "calibration": {"brightness_temperature": {"standard_name": "toa_brightness_temperature"}},
            "coordinates": ["longitude", "latitude"],
...
class TestUtils(unittest.TestCase):
    ...
    def test_get_filebase(self):
        ...
        base_dir = os.path.join(os.path.expanduser("~"), "data", "satellite", "Sentinel-3")
        base_data = (
            "S3A_OL_1_EFR____20161020T081224_20161020T081524_20161020T102406_0179_010_078_2340_SVL_O_NR_002.SEN3"
        )
        base_dir = os.path.join(base_dir, base_data)
        pattern = (
            "{mission_id:3s}_OL_{processing_level:1s}_{datatype_id:_<6s"
    ...
    def test_match_filenames(self):
        ...
        pattern = os.path.join(*pattern.split("/"))
        filenames = [os.path.join(base_dir, "Oa05_radiance.nc"), os.path.join(base_dir, "geo_coordinates.nc")]
    ...
    def test_match_filenames_windows_forward_slash(self):
        ...
        pattern = os.path.join(*pattern.split("/"))
        filenames = [
            os.path.join(base_dir, "Oa05_radiance.nc").replace(os.sep, "/"),
...
class DummyReader(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        super(DummyReader, self).__init__(filename, filename_info, filetype_info)
...
class TestFileFileYAMLReaderMultiplePatterns(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patterns = ["a{something:3s}.bla", "a0{something:2s}.bla"]
        res_dict = {
            "reader": {"name": "fake", "sensors": ["canon"]},
            "file_types": {"ftype1": {"name": "ft1", "file_patterns": patterns, "file_reader": DummyReader}},
            "datasets": {
                "ch1": {
                    "name": "ch01",
                ...
                "lons": {"name": "lons", "file_type": "ftype2"},
                "lats": {"name": "lats", "file_type": "ftype2"},
            },
        }

        self.config = res_dict
        self.reader = yr.FileYAMLReader(
            self.config, filter_parameters={"start_time": datetime(2000, 1, 1), "end_time": datetime(2000, 1, 2)}
        )
    ...
    def test_create_filehandlers(self):
        ...
        filelist = ["a001.bla", "a002.bla", "a001.bla", "a002.bla", "abcd.bla", "k001.bla", "a003.bli"]
...
class TestFileYAMLReaderWithCustomIDKey(unittest.TestCase):
    ...
    def test_custom_type_with_dict_contents_gets_parsed_correctly(self):
        ...
        assert ds_ids[0]["frequency_range"] == FrequencyRange(89.0, 2.8, "GHz")

        assert ds_ids[2]["frequency_double_sideband"] == FrequencyDoubleSideBand(183.31, 1.0, 1.0, "GHz")
...
class TestFileFileYAMLReader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        res_dict = {
            "reader": {"name": "fake", "sensors": ["canon"]},
            "file_types": {"ftype1": {"name": "ft1", "file_reader": BaseFileHandler, "file_patterns": patterns}},
            "datasets": {
                "ch1": {
                    "name": "ch01",
                    "wavelength": [0.5, 0.6, 0.7],
    ...
    def test_all_dataset_names(self):
        ...
        self.assertSetEqual(self.reader.all_dataset_names, set(["ch01", "ch02", "lons", "lats"]))
    ...
    def test_available_dataset_ids(self):
        ...
        self.assertSetEqual(
            set(self.reader.available_dataset_ids),
            {
                make_dataid(name="ch02", wavelength=(0.7, 0.75, 0.8), calibration="counts", modifiers=()),
                make_dataid(name="ch01", wavelength=(0.5, 0.6, 0.7), calibration="reflectance", modifiers=()),
            },
        )
    ...
    def test_available_dataset_names(self):
        ...
        loadables = self.reader.select_files_from_pathnames(["a001.bla"])
        self.reader.create_filehandlers(loadables)
        self.assertSetEqual(set(self.reader.available_dataset_names), set(["ch01", "ch02"]))

    def test_filter_fh_by_time(self):
        """Check filtering filehandlers by time."""
        fh0 = FakeFH(datetime(1999, 12, 30), datetime(1999, 12, 31))
        fh1 = FakeFH(datetime(1999, 12, 31, 10, 0), datetime(2000, 1, 1, 12, 30))
        fh2 = FakeFH(datetime(2000, 1, 1, 10, 0), datetime(2000, 1, 1, 12, 30))
        fh3 = FakeFH(datetime(2000, 1, 1, 12, 30), datetime(2000, 1, 2, 12, 30))
        fh4 = FakeFH(datetime(2000, 1, 2, 12, 30), datetime(2000, 1, 3, 12, 30))
        fh5 = FakeFH(datetime(1999, 12, 31, 10, 0), datetime(2000, 1, 3, 12, 30))
    ...
    def test_file_covers_area(self, bnd, adb, gad):
        ...
        file_handler = FakeFH(datetime(1999, 12, 31, 10, 0), datetime(2000, 1, 3, 12, 30))
    ...
    def test_start_end_time(self):
        ...
        fh0 = FakeFH(datetime(1999, 12, 30, 0, 0), datetime(1999, 12, 31, 0, 0))
        fh1 = FakeFH(datetime(1999, 12, 31, 10, 0), datetime(2000, 1, 1, 12, 30))
        fh2 = FakeFH(datetime(2000, 1, 1, 10, 0), datetime(2000, 1, 1, 12, 30))
        fh3 = FakeFH(datetime(2000, 1, 1, 12, 30), datetime(2000, 1, 2, 12, 30))
        fh4 = FakeFH(datetime(2000, 1, 2, 12, 30), datetime(2000, 1, 3, 12, 30))
    ...
    def test_select_from_directory(self):
        ...
        self.assertEqual(0, len(self.reader.select_files_from_directory(dpath)))
        os.rmdir(dpath)
        ...
        self.assertEqual(res, {"/grocery/apricot.nc", "/grocery/aubergine.nc"})
    ...
    def test_get_coordinates_for_dataset_key(self):
        ...
        ds_q = DataQuery(name="ch01", wavelength=(0.5, 0.6, 0.7, "µm"), calibration="reflectance", modifiers=())
        res = self.reader._get_coordinates_for_dataset_key(ds_q)
        self.assertListEqual(res, [make_dataid(name="lons"), make_dataid(name="lats")])

    def test_get_coordinates_for_dataset_key_without(self):
        """Test getting coordinates for a key without coordinates."""
        ds_id = make_dataid(name="lons", modifiers=())
    ...
    def test_get_coordinates_for_dataset_keys(self):
        ...
        ds_id1 = make_dataid(name="ch01", wavelength=(0.5, 0.6, 0.7), calibration="reflectance", modifiers=())
        ds_id2 = make_dataid(name="ch02", wavelength=(0.7, 0.75, 0.8), calibration="counts", modifiers=())
        lons = make_dataid(name="lons", modifiers=())
        lats = make_dataid(name="lats", modifiers=())

        res = self.reader._get_coordinates_for_dataset_keys([ds_id1, ds_id2, lons])
    ...
    def test_load_entire_dataset(self, xarray):
        ...
        file_handlers = [FakeFH(None, None), FakeFH(None, None), FakeFH(None, None), FakeFH(None, None)]
...
class TestFileYAMLReaderLoading(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.lons = xr.DataArray(
            np.ones((2, 2)) * 2, dims=["y", "x"], attrs={"standard_name": "longitude", "name": "longitude"}
        )
        self.lats = xr.DataArray(
            np.ones((2, 2)) * 2, dims=["y", "x"], attrs={"standard_name": "latitude", "name": "latitude"}
        )
        self.data = None
    ...
    def test_load_dataset_with_builtin_coords(self):
        ...
        self.data = xr.DataArray(
            np.ones((2, 2)), coords={"longitude": self.lons, "latitude": self.lats}, dims=["y", "x"]
        )
    ...
    def test_load_dataset_with_builtin_coords_in_wrong_order(self):
        ...
        self.data = xr.DataArray(
            np.ones((2, 2)), coords={"latitude": self.lats, "longitude": self.lons}, dims=["y", "x"]
        )
...
class TestFileFileYAMLReaderMultipleFileTypes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        res_dict = {
            "reader": {"name": "fake", "sensors": ["canon"]},
            "file_types": {
                "ftype1": {"name": "ft1", "file_patterns": patterns1},
                "ftype2": {"name": "ft2", "file_patterns": patterns2},
                "ftype3": {"name": "ft3", "file_patterns": patterns3},
            },
            "datasets": {
                "ch1": {
                    "name": "ch01",
                ...
                "lons": {"name": "lons", "file_type": ["ftype1", "ftype3"]},
                "lats": {"name": "lats", "file_type": ["ftype1", "ftype3"]},
            },
        }
    ...
    def test_update_ds_ids_from_file_handlers(self):
        ...
        for ftype, resol in zip(("ftype1", "ftype2"), (1, 2)):
            ...
            with (
                patch.dict(self.reader.all_ids, _orig_ids, clear=True),
...
class TestGEOFlippableFileYAMLReader(unittest.TestCase):
    ...
    def test_load_dataset_with_area_for_single_areas(self, ldwa):
        ...
        dummy_ds_xr = xr.DataArray(
            original_array,
            coords={"y": np.arange(2), "x": np.arange(3), "time": ("y", np.arange(2))},
            attrs={"area": area_def},
            dims=("y", "x"),
        )
        ...
        area_def = AreaDefinition(
            ...
            "test",
            "test",
            {"proj": "lcc", "lat_1": 25.0, "type": "crs"},
            3,
            2,
            original_area_extent,
        )

        dummy_ds_xr = xr.DataArray(original_array, dims=("y", "x"), attrs={"area": area_def})
        ldwa.return_value = dummy_ds_xr
        res = reader._load_dataset_with_area(dsid, coords, "NE")
        np.testing.assert_equal(res.values, original_array)
        np.testing.assert_equal(res.attrs["area"].area_extent, original_area_extent)
    ...
    def test_load_dataset_with_area_for_stacked_areas(self, ldwa):
        ...
        dummy_ds_xr = xr.DataArray(
            original_array,
            dims=("y", "x"),
            coords={"y": np.arange(4), "x": np.arange(3), "time": ("y", np.arange(4))},
    ...
    def test_load_dataset_with_area_for_swath_def_data(self, ldwa):
        ...
        dummy_ds_xr = xr.DataArray(original_array, coords={"y": dim}, attrs={"area": swath_def}, dims=("y",))
    ...
    def test_load_dataset_with_area_for_data_without_area(self, ldwa):
        ...
        dummy_ds_xr = xr.DataArray(original_array, coords={"y": dim}, attrs={}, dims=("y",))
...
def _create_mocked_fh_and_areadef(aex, ashape, expected_segments, segment, chk_pos_info):
    ...
    fh = MagicMock()
    filetype_info = {"expected_segments": expected_segments, "file_type": "filetype1"}
...
class TestGEOSegmentYAMLReader(unittest.TestCase):
    ...
    def test_load_dataset(self, mss, xr, parent_load_dataset):
        ...
        mss.return_value = (counter, expected_segments, slice_list, failure, projectable)
        ...
        # Disable padding
        res = reader._load_dataset(dataid, ds_info, file_handlers, pad_data=False)
        parent_load_dataset.assert_called_once_with(dataid, ds_info, file_handlers)
    ...
    def test_pad_later_segments_area(self, AreaDefinition):
        ...
        expected_call = ("fill", "fill", "fill", "some_crs", 500, 200, seg2_extent)
    ...
    def test_pad_earlier_segments_area(self, AreaDefinition):
        ...
        expected_call = ("fill", "fill", "fill", "some_crs", 500, 200, seg1_extent)
        AreaDefinition.assert_called_once_with(*expected_call)
    ...
    def test_find_missing_segments(self):
        ...
        fh_seg2 = MagicMock(filename_info=filename_info, filetype_info=filetype_info)
        projectable = "projectable"
        get_dataset = MagicMock()
        get_dataset.return_value = projectable
...
class TestGEOVariableSegmentYAMLReader:
    ...
    def test_get_empty_segment(self, GVSYReader, fake_mss, fake_xr, fake_geswh):
        ...
        chk_pos_info = {
            "1km": {"start_position_row": 0, "end_position_row": 0, "segment_height": 0, "grid_width": 11136},
            "2km": {"start_position_row": 140, "end_position_row": None, "segment_height": 278, "grid_width": 5568},
        }
        expected_segments = 2
        segment = 2
        aex = [0, 1000, 200, 500]
        ...
        fake_mss.return_value = (counter, expected_segments, slice_list, failure, projectable)
    ...
    def test_pad_earlier_segments_area(self, GVSYReader, fake_adef):
        ...
        expected_call = ("fill", "fill", "fill", "some_crs", 5568, 139, seg1_extent)
    ...
    def test_pad_later_segments_area(self, GVSYReader, fake_adef):
        ...
        chk_pos_info = {
            "1km": {
                "start_position_row": None,
                "end_position_row": 11136 - 278,
                "segment_height": 556,
                "grid_width": 11136,
            },
            "2km": {"start_position_row": 0, "end_position_row": 0, "segment_height": 0, "grid_width": 5568},
        }
        ...
        expected_call = ("fill", "fill", "fill", "some_crs", 11136, 278, seg2_extent)
        fake_adef.assert_called_once_with(*expected_call)
    ...
    def test_pad_later_segments_area_for_multiple_segments_gap(self, GVSYReader, fake_adef):
        ...
        chk_pos_info = {
            "1km": {
                "start_position_row": 11136 - 600 - 100 + 1,
        ...
        chk_pos_info = {
            "1km": {
                "start_position_row": 11136 - 300 - 100 + 1,
        ...
        chk_pos_info = {
            "1km": {
                "start_position_row": 11136 - 100 + 1,
        ...
        expected_call1 = ("fill", "fill", "fill", "some_crs", 11136, 100, (0, 1500.0, 200, 1000))
        expected_call2 = ("fill", "fill", "fill", "some_crs", 11136, 100, (0, 2000.0, 200, 1500))
        expected_call3 = ("fill", "fill", "fill", "some_crs", 11136, 66, (0, 1330.0, 200, 1000))
        expected_call4 = ("fill", "fill", "fill", "some_crs", 11136, 67, (0, 1665.0, 200, 1330.0))
        expected_call5 = ("fill", "fill", "fill", "some_crs", 11136, 67, (0, 2000.0, 200, 1665.0))

        fake_adef.side_effect = None
        fake_adef.assert_has_calls(
            [
                call(*expected_call1),
                call(*expected_call2),
                call(*expected_call3),
