...
def _get_test_area(shape=(200, 100), crs=None, extents=None):
    ...
    if extents is None:
        extents = (-1000.0, -1500.0, 1000.0, 1500.0)
...
def _get_test_data(shape=(200, 100), chunks=50):
    data = np.linspace(0.0, 1.0, shape[0] * shape[1], dtype=np.float32).reshape(shape)
...
class TestAWIPSTiledWriter:
    ...
    @pytest.mark.parametrize("use_save_dataset", [(False,), (True,)])
    ...
    def test_basic_numbered_1_tile(self, extra_attrs, expected_filename, use_save_dataset, caplog, tmp_path):
        ...
        for fn in all_files:
            ...
            np.testing.assert_allclose(input_data_arr.values, output_ds["data"].data, atol=scale_factor / 2)
    ...
    def test_basic_numbered_tiles(self, tile_count, tile_size, tmp_path):
        ...
        if should_error:
            with (
                dask.config.set(scheduler=CustomScheduler(0)),
    ...
    def test_basic_lettered_tiles(self, tmp_path):
        ...
        area_def = _get_test_area(shape=(2000, 1000), extents=(-1000000.0, -1500000.0, 1000000.0, 1500000.0))
    ...
    def test_basic_lettered_tiles_diff_projection(self, tmp_path):
        ...
        data = _get_test_data(shape=(2000, 1000), chunks=500)
        area_def = _get_test_area(shape=(2000, 1000), crs=crs, extents=(-1000000.0, -1500000.0, 1000000.0, 1500000.0))
        ds = _get_test_lcc_data(data, area_def)
        # tile_count should be ignored since we specified lettered_grid
        w.save_datasets([ds], sector_id="LCC", source_name="TESTS", tile_count=(3, 3), lettered_grid=True)
    ...
    def test_lettered_tiles_update_existing(self, tmp_path):
        ...
        area_def2 = _get_test_area(shape=(2000, 1000), extents=(-800000.0, -1500000.0, 1200000.0, 1500000.0))
        data2 = np.linspace(0.0, 1.0, 2000000, dtype=np.float32).reshape((2000, 1000))
    ...
    def test_lettered_tiles_sector_ref(self, tmp_path):
        ...
        ds = _get_test_lcc_data(data, area_def)
        w.save_datasets(
            [ds], sector_id="LCC", source_name="TESTS", lettered_grid=True, use_sector_reference=True, use_end_time=True
        )
        all_files = glob(os.path.join(str(tmp_path), "TESTS_AII*.nc"))
        assert len(all_files) == 16
        for fn in all_files:
    ...
    def test_lettered_tiles_no_fit(self, tmp_path):
        ...
        w = AWIPSTiledWriter(base_dir=str(tmp_path), compress=True)
        data = _get_test_data(shape=(2000, 1000), chunks=500)
        area_def = _get_test_area(shape=(2000, 1000), extents=(4000000.0, 5000000.0, 5000000.0, 6000000.0))
        ds = _get_test_lcc_data(data, area_def)
        w.save_datasets([ds], sector_id="LCC", source_name="TESTS", tile_count=(3, 3), lettered_grid=True)
        # No files created
        all_files = glob(os.path.join(str(tmp_path), "TESTS_AII*.nc"))
    ...
    def test_basic_numbered_tiles_rgb(self, tmp_path):
        ...
        w = AWIPSTiledWriter(base_dir=str(tmp_path), compress=True)
        data = da.from_array(np.linspace(0.0, 1.0, 60000, dtype=np.float32).reshape((3, 200, 100)), chunks=50)
        area_def = _get_test_area()
        ds = _get_test_lcc_data(data, area_def)
