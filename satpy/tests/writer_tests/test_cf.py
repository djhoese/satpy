...
def test_is_projected(caplog):
    ...
    da = xr.DataArray(
        ...
        dims=("y", "x"),
        coords={
            "x": xr.DataArray(np.arange(5), dims=("x",), attrs={"units": "m"}),
    ...
    da = xr.DataArray(
        ...
        dims=("y", "x"),
        coords={
            "x": xr.DataArray(np.arange(5), dims=("x",), attrs={"units": "degrees_east"}),
            "y": xr.DataArray(np.arange(5), dims=("y",), attrs={"units": "degrees_north"}),
        },
    )
    assert not _is_projected(da)

    da = xr.DataArray(np.arange(25).reshape(5, 5), dims=("y", "x"))
...
def test_add_time_cf_attrs():
    ...
    times = np.array(
        ["2018-05-30T10:05:00", "2018-05-30T10:05:01", "2018-05-30T10:05:02", "2018-05-30T10:05:03"],
        dtype=np.datetime64,
    )
    scn["test-array"] = xr.DataArray(
        test_array, dims=["y", "x"], coords={"time": ("y", times)}, attrs=dict(start_time=times[0], end_time=times[-1])
    )
...
class TestCFWriter:
    ...
    def test_save_array(self):
        ...
        scn["test-array"] = xr.DataArray(
            [1, 2, 3], attrs=dict(start_time=start_time, end_time=end_time, prerequisites=[make_dsq(name="hej")])
        )
    ...
    def test_save_array_coords(self):
        ...
        if CRS is not None:
            proj_str = "+proj=geos +lon_0=-95.0 +h=35786023.0 +a=6378137.0 +b=6356752.31414 +sweep=x +units=m +no_defs"
            coords["crs"] = CRS.from_string(proj_str)
        scn["test-array"] = xr.DataArray(
            [[1, 2, 3]],
            dims=("y", "x"),
            coords=coords,
            attrs=dict(start_time=start_time, end_time=end_time, prerequisites=[make_dsq(name="hej")]),
        )
        with TempFile() as filename:
            scn.save_datasets(filename=filename, writer="cf")
            with xr.open_dataset(filename) as f:
    ...
    def test_ancillary_variables(self):
        ...
        end_time = datetime(2018, 5, 30, 10, 15)
        da = xr.DataArray(
            [1, 2, 3], attrs=dict(start_time=start_time, end_time=end_time, prerequisites=[make_dataid(name="hej")])
        )
        scn["test-array-1"] = da
    ...
    def test_groups(self):
        ...
        with TempFile() as filename:
            scn.save_datasets(
                filename=filename, writer="cf", groups={"visir": ["IR_108", "VIS006"], "hrv": ["HRV"]}, pretty=True
            )
            ...
            assert set(nc_visir.variables.keys()) == {
                "VIS006",
                "IR_108",
                "y",
                "x",
            ...
            for tst, ref in zip(
                [nc_visir["VIS006"], nc_visir["IR_108"], nc_hrv["HRV"]], [scn["VIS006"], scn["IR_108"], scn["HRV"]]
            ):
    ...
    def test_bounds(self):
        ...
        with TempFile() as filename:
            ...
            scn.save_datasets(filename=filename, encoding={"time": {"units": time_units}}, writer="cf")
            with xr.open_dataset(filename, decode_cf=False) as f:
    ...
    def test_bounds_minimum(self):
        ...
        scn["test-arrayB"] = xr.DataArray(
            ...
            dims=["x", "y", "time"],
            coords={"time": [np.datetime64("2018-05-30T10:05:00")]},
            attrs=dict(start_time=start_timeB, end_time=end_timeB),
        )
        with TempFile() as filename:
            scn.save_datasets(filename=filename, writer="cf")
            with xr.open_dataset(filename, decode_cf=True) as f:
    ...
    def test_bounds_missing_time_info(self):
        ...
        scn["test-arrayB"] = xr.DataArray(
            test_arrayB, dims=["x", "y", "time"], coords={"time": [np.datetime64("2018-05-30T10:05:00")]}
        )
        with TempFile() as filename:
            scn.save_datasets(filename=filename, writer="cf")
            with xr.open_dataset(filename, decode_cf=True) as f:
    ...
    def test_header_attrs(self):
        ...
        start_time = datetime(2018, 5, 30, 10, 0)
        end_time = datetime(2018, 5, 30, 10, 15)
        scn["test-array"] = xr.DataArray([1, 2, 3], attrs=dict(start_time=start_time, end_time=end_time))
        with TempFile() as filename:
            header_attrs = {
                "sensor": "SEVIRI",
                "orbit": 99999,
            ...
            scn.save_datasets(filename=filename, header_attrs=header_attrs, flatten_attrs=True, writer="cf")
            with xr.open_dataset(filename) as f:
    ...
    def get_test_attrs(self):
        ...
        encoded = {
            ...
            "raw_metadata": '{"recarray": [[0, 0], [0, 0], [0, 0]], "flag": "true", "dict": {"a": 1, "b": [1, 2, 3]}}',
        }
        encoded_flat = {
            "name": "IR_108",
            "start_time": "2018-01-01 00:00:00",
    ...
    def test_encode_attrs_nc(self):
        ...
        raw_md_roundtrip = {"recarray": [[0, 0], [0, 0], [0, 0]], "flag": "true", "dict": {"a": 1, "b": [1, 2, 3]}}
    ...
    def test_da2cf_one_dimensional_array(self):
        ...
        arr = xr.DataArray(
            np.array([1, 2, 3, 4]), attrs={}, dims=("y",), coords={"y": [0, 1, 2, 3], "acq_time": ("y", [0, 1, 2, 3])}
        )
    ...
    def test_collect_cf_dataarrays(self):
        ...
        geos = pyresample.geometry.AreaDefinition(
            ...
            description="geos",
            proj_id="geos",
            projection={"proj": "geos", "h": 35785831.0, "a": 6378169.0, "b": 6356583.8},
            width=2,
            height=2,
            area_extent=[-1, -1, 1, 1],
        )
        ...
        list_dataarrays = [
            xr.DataArray(
                data=data,
                dims=("y", "x"),
                coords={"y": y, "x": x, "acq_time": ("y", time)},
    ...
    def test_assert_xy_unique(self):
        ...
        dummy = [[1, 2], [3, 4]]
        datas = {
            "a": xr.DataArray(data=dummy, dims=("y", "x"), coords={"y": [1, 2], "x": [3, 4]}),
    ...
    def test_make_alt_coords_unique(self):
        ...
        time2 = [3, 4]
        datasets = {
            "var1": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x, "acq_time": ("y", time1)}),
            "var2": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x, "acq_time": ("y", time2)}),
        }
    ...
    def test_area2cf(self):
        ...
        ds_base = xr.DataArray(
            data=[[1, 2], [3, 4]], dims=("y", "x"), coords={"y": [1, 2], "x": [3, 4]}, attrs={"name": "var1"}
        )
    ...
    def test__add_grid_mapping(self):
        ...
        geos = pyresample.geometry.AreaDefinition(
            ...
            description="geos",
            proj_id="geos",
            projection={"proj": "geos", "h": h, "a": a, "b": b, "lat_0": 0, "lon_0": 0},
            width=2,
            height=2,
            area_extent=[-1, -1, 1, 1],
        )
        geos_expected = xr.DataArray(
            data=0,
            attrs={
        ...
        geos = pyresample.geometry.AreaDefinition(
            ...
            description="geos",
            proj_id="geos",
            projection={"proj": "geos", "h": h, "datum": "WGS84", "ellps": "GRS80", "lat_0": 0, "lon_0": 0},
            width=2,
            height=2,
            area_extent=[-1, -1, 1, 1],
        )
...
class TestCFWriterData:
    ...
    def datasets(self):
        ...
        datasets = {
            "var1": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x}),
            "var2": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x}),
            "lat": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x}),
            "lon": xr.DataArray(data=data, dims=("y", "x"), coords={"y": y, "x": x}),
        }
...
class EncodingUpdateTest:
    ...
    def fake_ds(self):
        ...
        ds = xr.Dataset(
            {"foo": (("y", "x"), [[1, 2], [3, 4]]), "bar": (("y", "x"), [[3, 4], [5, 6]])},
            coords={"y": [1, 2], "x": [3, 4], "lon": (("y", "x"), [[7, 8], [9, 10]])},
        )
        return ds
    ...
    def fake_ds_digit(self):
        """Create fake data for testing."""
        ds_digit = xr.Dataset(
            {"CHANNEL_1": (("y", "x"), [[1, 2], [3, 4]]), "CHANNEL_2": (("y", "x"), [[3, 4], [5, 6]])},
            coords={"y": [1, 2], "x": [3, 4], "lon": (("y", "x"), [[7, 8], [9, 10]])},
        )
        return ds_digit
    ...
    def test_dataset_name_digit(self, fake_ds_digit):
        ...
        kwargs = {"encoding": {"1": {"dtype": "float32"}, "2": {"dtype": "float32"}}, "other": "kwargs"}
    ...
    def test_with_time(self, fake_ds):
        ...
        kwargs = {"encoding": {"bar": {"chunksizes": (1, 1, 1)}}, "other": "kwargs"}
        enc, other_kwargs = update_encoding(ds, kwargs)
        expected_dict = {
            "y": {"_FillValue": None},
            "x": {"_FillValue": None},
            ...
            "time": {"_FillValue": None, "calendar": "proleptic_gregorian", "units": "days since 2009-07-01 12:15:00"},
            "time_bnds": {
                "_FillValue": None,
...
def _should_use_compression_keyword():
    ...
    return versions["libnetcdf"] >= Version("4.9.0") and versions["xarray"] >= Version("2023.10")
