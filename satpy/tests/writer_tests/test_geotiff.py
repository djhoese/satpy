...
def _get_test_datasets_2d():
    ...
    ds1 = xr.DataArray(
        ...
        dims=("y", "x"),
        attrs={"name": "test", "start_time": datetime.utcnow(), "units": "K"},
    )
    return [ds1]
...
def _get_test_datasets_3d():
    ...
    ds1 = xr.DataArray(
        ...
        dims=("bands", "y", "x"),
        coords={"bands": ["R", "G", "B"]},
        attrs={"name": "test", "start_time": datetime.utcnow()},
    )
    return [ds1]
...
class TestGeoTIFFWriter:
    ...
    @pytest.mark.parametrize("input_func", [_get_test_datasets_2d, _get_test_datasets_3d])
    ...
    def test_float_write_with_unit_conversion(self, tmp_path):
        ...
        dtype = np.float32
        w = GeoTIFFWriter(base_dir=tmp_path, enhance=False, dtype=dtype)
