...
class TestNinjoTIFFWriter(unittest.TestCase):
    ...
    def test_dataset_skip_unit_conversion(self, iwsd):
        ...
        dataset = xr.DataArray([1, 2, 3], attrs={"units": "K"})
        with mock.patch("satpy.writers.ninjotiff.convert_units") as uconv:
            ntw.save_dataset(dataset, physic_unit="CELSIUS", convert_temperature_units=False)
    ...
    def test_convert_units_self(self):
        ...
        sc = make_fake_scene({"VIS006": np.arange(25, dtype="f4").reshape(5, 5)}, common_attrs={"units": "%"})
        ds_in = sc["VIS006"]
    ...
    def test_convert_units_temp(self):
        ...
        sc = make_fake_scene({"IR108": np.arange(25, dtype="f4").reshape(5, 5)}, common_attrs={"units": "K"})
        ds_in_k = sc["IR108"]
    ...
    def test_convert_units_other(self):
        ...
        sc = make_fake_scene(
            {"rain_rate": np.arange(25, dtype="f8").reshape(5, 5)}, common_attrs={"units": "millimeter/hour"}
        )
