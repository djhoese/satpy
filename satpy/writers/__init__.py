...
def read_writer_config(config_files, loader=UnsafeLoader):
    ...
    try:
        ...
        raise KeyError("Malformed config file {}: missing writer 'writer'".format(config_files))
...
def load_writer_configs(writer_configs, **writer_kwargs):
    ...
    writer = writer_class(config_files=writer_configs, **init_kwargs)
...
def load_writer(writer, **writer_kwargs):
    ...
    try:
        return load_writer_configs(writer_kwargs["config_files"], **writer_kwargs)
    except ValueError:
        raise ValueError("Writer '{}' does not exist or could not be loaded".format(writer))
...
def add_overlay(
    ...
    if hasattr(orig_img, "convert"):
        ...
        raise RuntimeError("'trollimage' 1.6+ required to support adding overlays/decorations to non-RGB data.")

    if overlays is None:
        overlays = _create_overlays_dict(color, width, grid, level_coast, level_borders)

    cw_ = ContourWriterAGG(coast_dir)
    new_image = orig_img.apply_pil(
        _burn_overlay, res_mode, None, {"fill_value": fill_value}, (area, cw_, overlays), None
    )
    return new_image
...
def add_text(orig, dc, img, text):
    ...
    new_data = xr.DataArray(
        arr,
        dims=["y", "x", "bands"],
        coords={"y": orig.data.coords["y"], "x": orig.data.coords["x"], "bands": list(img.mode)},
...
def get_enhanced_image(dataset, enhance=None, overlay=None, decorate=None, fill_value=None):
...
class Writer(Plugin, DataDownloadMixin):
    ...
    def __init__(self, name=None, filename=None, base_dir=None, **kwargs):
        ...
        if "file_pattern" in self.info:
            warnings.warn(
                "Writer YAML config is using 'file_pattern' which has been deprecated, use 'filename' instead.",
                stacklevel=2,
            )
        ...
        if "file_pattern" in kwargs:
            warnings.warn(
                "'file_pattern' has been deprecated, use 'filename' instead.", DeprecationWarning, stacklevel=2
            )
    ...
    def save_dataset(self, dataset, filename=None, fill_value=None, compute=True, units=None, **kwargs):
        ...
        """
        raise NotImplementedError("Writer '%s' has not implemented dataset saving" % (self.name,))
...
class ImageWriter(Writer):
    ...
    def save_dataset(
        self, dataset, filename=None, fill_value=None, overlay=None, decorate=None, compute=True, units=None, **kwargs
    ):
        """Save the ``dataset`` to a given ``filename``.
    ...
    def save_image(self, img: XRImage, filename: Optional[str] = None, compute: bool = True, **kwargs):
...
class DecisionTree(object):
    ...
    def _find_match_if_known(self, curr_level, remaining_match_keys, query_dict):
        ...
        for query_val in query_vals:
            if query_val not in curr_level:
                continue
            match = self._find_match(curr_level[query_val], remaining_match_keys[1:], query_dict)
    ...
    def _find_match(self, curr_level, remaining_match_keys, query_dict):
        ...
        match = self._find_match_if_known(curr_level, remaining_match_keys, query_dict)
        ...
        if match is None and self.any_key in curr_level:
            ...
            match = self._find_match(curr_level[self.any_key], remaining_match_keys[1:], query_dict)
        return match
    ...
    def find_match(self, **query_dict):
        ...
        if match is None:
            ...
            raise KeyError("No decision section found for %s" % (query_dict.get("uid", None),))
        return match
...
class EnhancementDecisionTree(DecisionTree):
    ...
    def __init__(self, *decision_dicts, **kwargs):
        ...
        super(EnhancementDecisionTree, self).__init__(decision_dicts, match_keys, multival_keys)
    ...
    def find_match(self, **query_dict):
        ...
        try:
            ...
            raise KeyError("No enhancement configuration found for %s" % (query_dict.get("uid", None),))
