...
class MITIFFWriter(ImageWriter):
    ...
    def save_datasets(self, datasets, filename=None, fill_value=None, compute=True, **kwargs):
            ...
            for ch, ds in enumerate(datasets):
                if isinstance(ds.attrs["prerequisites"][ch], (DataQuery, DataID)):
                    if ds.attrs["prerequisites"][ch]["name"] == cn:
                        channels.append(ds.attrs["prerequisites"][ch]["name"])
                        break
                else:
                    if ds.attrs["prerequisites"][ch] == cn:
                        channels.append(ds.attrs["prerequisites"][ch])
                        break
        return channels
    ...
    def _append_projection_center(self, proj4_string, datasets, first_dataset, x_0, y_0):
        ...
        if "x_0" not in proj4_string:
            proj4_string += " +x_0=%.6f" % (
                (-dataset.attrs["area"].area_extent[0] + dataset.attrs["area"].pixel_size_x) + x_0
            )
            proj4_string += " +y_0=%.6f" % (
                (-dataset.attrs["area"].area_extent[1] + dataset.attrs["area"].pixel_size_y) + y_0
            )
        elif "+x_0=0" in proj4_string and "+y_0=0" in proj4_string:
            proj4_string = proj4_string.replace(
                "+x_0=0",
                "+x_0=%.6f" % ((-dataset.attrs["area"].area_extent[0] + dataset.attrs["area"].pixel_size_x) + x_0),
            )
            proj4_string = proj4_string.replace(
                "+y_0=0",
                "+y_0=%.6f" % ((-dataset.attrs["area"].area_extent[1] + dataset.attrs["area"].pixel_size_y) + y_0),
            )
        return proj4_string
    ...
    def _convert_epsg_to_proj(self, proj4_string, x_0):
        if "EPSG:32631" in proj4_string:
            proj4_string = proj4_string.replace(
                "+init=EPSG:32631", "+proj=etmerc +lat_0=0 +lon_0=3 +k=0.9996 +ellps=WGS84 +datum=WGS84"
            )
            x_0 = 500000
        elif "EPSG:32632" in proj4_string:
            proj4_string = proj4_string.replace(
                "+init=EPSG:32632", "+proj=etmerc +lat_0=0 +lon_0=9 +k=0.9996 +ellps=WGS84 +datum=WGS84"
            )
            x_0 = 500000
        elif "EPSG:32633" in proj4_string:
            proj4_string = proj4_string.replace(
                "+init=EPSG:32633", "+proj=etmerc +lat_0=0 +lon_0=15 +k=0.9996 +ellps=WGS84 +datum=WGS84"
            )
            x_0 = 500000
        elif "EPSG:32634" in proj4_string:
            proj4_string = proj4_string.replace(
                "+init=EPSG:32634", "+proj=etmerc +lat_0=0 +lon_0=21 +k=0.9996 +ellps=WGS84 +datum=WGS84"
            )
            x_0 = 500000
        elif "EPSG:32635" in proj4_string:
            proj4_string = proj4_string.replace(
                "+init=EPSG:32635", "+proj=etmerc +lat_0=0 +lon_0=27 +k=0.9996 +ellps=WGS84 +datum=WGS84"
            )
            x_0 = 500000
    ...
    def _add_pixel_sizes(self, datasets, first_dataset):
        _image_description = ""
        if isinstance(datasets, list):
            _image_description += " Ax: %.6f" % (first_dataset.attrs["area"].pixel_size_x / 1000.0)
            _image_description += " Ay: %.6f" % (first_dataset.attrs["area"].pixel_size_y / 1000.0)
        else:
            _image_description += " Ax: %.6f" % (datasets.attrs["area"].pixel_size_x / 1000.0)
            _image_description += " Ay: %.6f" % (datasets.attrs["area"].pixel_size_y / 1000.0)
    ...
    def _add_corners(self, datasets, first_dataset):
        ...
        if isinstance(datasets, list):
            _image_description += " Bx: %.6f" % (
                first_dataset.attrs["area"].area_extent[0] / 1000.0
                + first_dataset.attrs["area"].pixel_size_x / 1000.0 / 2.0
            )  # LL_x
            _image_description += " By: %.6f" % (
                first_dataset.attrs["area"].area_extent[3] / 1000.0
                - first_dataset.attrs["area"].pixel_size_y / 1000.0 / 2.0
            )  # UR_y
        else:
            _image_description += " Bx: %.6f" % (
                datasets.attrs["area"].area_extent[0] / 1000.0 + datasets.attrs["area"].pixel_size_x / 1000.0 / 2.0
            )  # LL_x
            _image_description += " By: %.6f" % (
                datasets.attrs["area"].area_extent[3] / 1000.0 - datasets.attrs["area"].pixel_size_y / 1000.0 / 2.0
            )  # UR_y
    ...
    def _add_calibration_datasets(self, ch, datasets, reverse_offset, reverse_scale, decimals):
        ...
        for i, ds in enumerate(ds_list):
            if (
                "prerequisites" in ds.attrs
                and isinstance(ds.attrs["prerequisites"], list)
                and len(ds.attrs["prerequisites"]) >= i + 1
                and isinstance(ds.attrs["prerequisites"][i], (DataQuery, DataID))
            ):
                ...
                if not skip_calibration:
                    ...
                    for val in range(0, 256):
                        ...
                        _table_calibration += "{0:.{1}f} ".format(
                            (
                                float(self.mitiff_config[kwargs["sensor"]][cns.get(ch, ch)]["min-val"])
                                + (
    ...
    def _calibrate_data(self, dataset, calibration, min_val, max_val):
        ...
        _data = (
            reverse_offset
            + reverse_scale * ((dataset.data - float(min_val)) / (float(max_val) - float(min_val))) * 255.0
        )
    ...
    def _save_as_enhanced(self, datasets, tmp_gen_filename, **kwargs):
        ...
        for band in img.data["bands"]:
            ...
            data = chn.values.clip(0, 1) * 254.0 + 1
            data = data.clip(0, 255)
            mitiff_frames.append(Image.fromarray(data.astype(np.uint8), mode="L"))
        mitiff_frames[0].save(
            tmp_gen_filename,
            save_all=True,
            append_images=mitiff_frames[1:],
            compression="raw",
            compress_level=9,
            tiffinfo=tiffinfo,
        )
    ...
    def _save_single_dataset(self, datasets, cns, tmp_gen_filename, tiffinfo, kwargs):
        ...
        if len(datasets.dims) == 2 and (all("bands" not in i for i in datasets.dims)):
            ...
            cn = cns.get(datasets.attrs["prerequisites"][0]["name"], datasets.attrs["prerequisites"][0]["name"])
            data = self._calibrate_data(
                datasets,
            ...
            Image.fromarray(data.astype(np.uint8)).save(
                tmp_gen_filename, compression="raw", compress_level=9, tiffinfo=tiffinfo
            )
        else:
            mitiff_frames = []
            for _cn_i, _cn in enumerate(self.channel_order[kwargs["sensor"]]):
                for band in datasets["bands"]:
                    if band == _cn:
                        ...
                        # Note the last index is a tuple index.
                        cn = cns.get(
                            chn.attrs["prerequisites"][_cn_i]["name"], chn.attrs["prerequisites"][_cn_i]["name"]
                        )
                        data = self._calibrate_data(
                            chn,
                            chn.attrs["prerequisites"][_cn_i].get("calibration"),
                            self.mitiff_config[kwargs["sensor"]][cn]["min-val"],
                            self.mitiff_config[kwargs["sensor"]][cn]["max-val"],
                        )
